...
#
# Every check below matches only against the LEADING word(s) of a command
# segment (split on &&/||/;/|), never against the raw command text as a
# whole -- matching the whole text would also match plain prose mentioning
# a tool name inside an unrelated argument (e.g. a git commit message
# describing this very hook), a real false positive confirmed live the
# first time this script matched its own commit message's body text.
#
...
#     composer test*/pest) while one is still running IN THIS SAME
#     WORKTREE: confirmed 7+ times to silently under-report counts,
#     cascade unrelated failures, or get a process OOM-killed -- not just
...
while IFS= read -r seg; do
    ...
    if printf '%s' "$trimmed" | grep -qE '^(\./)?vendor/bin/phpstan[[:space:]]+clear-result-cache([[:space:]]|$)'; then
    ...
    if printf '%s' "$trimmed" | grep -qE '^git[[:space:]]+log([[:space:]].*)?[[:space:]]--all([[:space:]]|$)'; then
