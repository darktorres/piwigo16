...
| 3.1  | CSS design tokens + Stylelint | 🟢 **Active** ▸ 9 / 13 | M         | 4 live steps remaining (5, 11, 12, 15)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 5    | Collapse search CSS variants. `search.css` + `clear-search.css` + `dark-search.css` → single variable-driven `search.css`. The two variant files differ not just in color but also in some structural rules, so the collapse is token-based (each variant becomes a thin `:root {}`) while `search.css` absorbs the shared selectors. Net savings: ~500 lines.                                                                                                                                       |
...
| 11   | Split `themes/admin/_base/theme.css` (9,521 lines) along its 60+ `/* name.css */` section markers into base/components/pages/features. `theme.css` becomes an `@import` list. Utility classes `.u-*` land in `base/utilities.css`. Page-specific sections merge into the existing `css/pages/*.css` files (which templates already load individually).                                                                                                                                               |
