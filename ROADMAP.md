...
| 1.2 | Templates pipeline            | 🟢 Active ▸ W2 D.admin 24/70 · 25/137 .latte | XL | wave 1 hygiene done → wave 2 Latte foundation + plugin port + converter + admin first batch done → wave 3 precompile |
...
| C     | `tools/smarty-to-latte/convert.php` mechanical rewrite tool — covers: foreach, dot-access, if-not, escape filters, combine_script/css, define_derivative, include path, printed-literal filter prefix, assign, section→foreach, capture, literal, html_head/style/footer_script blocks, regex_replace→replaceRe, cat→`~`. 31 unit tests pin behavior. **Residue surfaced from D.admin run** that needs converter improvements: Smarty's `empty()`, `eq`/`neq`/`gt`/`lt` operator keywords, `{html_options}` plugin tag, `{counter}`, `{function}` (Latte uses `{define}`), `{else if}` with space, multi-arg pipe boundary issues. Each lifts another batch of conversions. | 🟢 Foundation + iteration | TBD |
| D.admin | Convert ~55 admin templates (`themes/admin/_base/template/`) | 🟡 Active · **24 / 70 lint-clean** (46 broken on residue patterns above; will lift as Phase C iterates) | TBD |
