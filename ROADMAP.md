...
| 1.2 | Templates pipeline            | 🟢 Active ▸ W2 D.admin 38/70 · 39/137 .latte | XL | wave 1 hygiene done → wave 2 Latte foundation + plugin port + converter iterating + admin half-converted → wave 3 precompile |
...
| D.admin | Convert ~55 admin templates (`themes/admin/_base/template/`) | 🟡 Active · **38 / 70 lint-clean** (32 broken on residue patterns below; will lift as Phase C iterates further). Residue still in admin: `{html_options}` / `{html_radios}` / `{math}` Smarty plugin tags (~14 files; need PiwigoExtension functions); `{counter}` (2 files); calls to user-defined `{define}` functions (`{tagContent}`, `{groupContent}` — Smarty syntax `{NAME}`, Latte syntax `{include NAME}`, 2 files); Smarty backtick string interpolation (3 files); HTML attribute syntax issues (queue, maintenance_actions); `Cannot use isset() on expression` (configuration_sizes — concatenation in isset arg); pipe in `{if}` expression (picture_modify). | TBD |
