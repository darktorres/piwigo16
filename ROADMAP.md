...
**Status:** 🟢 Active ▸ Phases A + B + C + D.\* + E + F.0 + F done · 133 / 133 .latte · `smarty/smarty` removed; `Template` is now a Latte-backed page-rendering coordinator · F.1 (caller migration off the handle-based API) is the only remaining cleanup · **Effort:** XL · depends on Wave 1
...
| F.1              | Migrate callers off `Template`'s handle-based API to direct `LatteEngine` calls; rename `Template` → `PageRenderer` (or similar) once it's a pure page-coordinator                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 🟡 Not started · ~85 `setFilename` + ~26 `parse` callsites; output buffer + scriptLoader/cssLoader + button accumulators stay where they are (page-coordination state, not engine-specific). Cosmetic refactor; defer until a coherent driver lands. |
