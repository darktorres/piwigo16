<?php

declare(strict_types=1);

// +-----------------------------------------------------------------------+
// | This file is part of Piwigo.                                          |
// |                                                                       |
// | For copyright and license information, please view the COPYING.txt    |
// | file that was distributed with this source code.                      |
// +-----------------------------------------------------------------------+

use Piwigo\admin\inc\functions_admin;
use Piwigo\inc\functions;
use Piwigo\inc\functions_category;
use Piwigo\inc\functions_plugins;
use Piwigo\inc\functions_url;
use Piwigo\inc\functions_user;

if (! defined('PHPWG_ROOT_PATH')) {
    exit('Hacking attempt!');
}

$query = <<<SQL
    SELECT COUNT(*) AS "COUNT(*)"
    FROM categories;
    SQL;
[$albums_counter] = $conf->sql_backend::pwg_db_fetch_row($conf->sql_backend::pwg_query($query));

// +-----------------------------------------------------------------------+
// | Check Access and exit when user status is not ok                      |
// +-----------------------------------------------------------------------+
functions_user::check_status(ACCESS_ADMINISTRATOR);

functions::check_input_parameter('parent_id', $_GET, false, PATTERN_ID);

// +-----------------------------------------------------------------------+
// | tabs                                                                  |
// +-----------------------------------------------------------------------+

$page['tab'] = 'list';
require __DIR__ . '/../admin/inc/albums_tab.php';

// +-----------------------------------------------------------------------+
// |                         categories auto order                         |
// +-----------------------------------------------------------------------+

$open_cat = $_GET['parent_id'] ?? -1;

$sort_orders = [
    'name ASC',
    'name DESC',
    'date_creation DESC',
    'date_creation ASC',
    'date_available DESC',
    'date_available ASC',
    'natural_order DESC',
    'natural_order ASC',
];

if (isset($_POST['simpleAutoOrder']) ||
    isset($_POST['recursiveAutoOrder'])
) {

    if (! in_array($_POST['order'], $sort_orders)) {
        exit('Invalid sort order');
    }

    functions::check_input_parameter('id', $_POST, false, '/^-?\d+$/');

    $id_condition = ($_POST['id'] === '-1') ? 'IS NULL' : "= {$_POST['id']}";
    $query = <<<SQL
        SELECT id
        FROM categories
        WHERE id_uppercat {$id_condition};
        SQL;
    $category_ids = $conf->sql_backend::query2array($query, null, 'id');

    if (isset($_POST['recursiveAutoOrder'])) {
        $category_ids = functions_category::get_subcat_ids($category_ids);
    }

    $categories = [];
    $sort = [];

    [$order_by_field, $order_by_asc] = explode(' ', $_POST['order']);

    $order_by_date = false;

    if (str_starts_with($order_by_field, 'date_')) {
        $order_by_date = true;

        $ref_dates = functions_admin::get_categories_ref_date(
            $category_ids,
            $order_by_field,
            $order_by_asc === 'ASC' ? 'min' : 'max'
        );
    }

    $category_ids_str = implode(', ', $category_ids);
    $query = <<<SQL
        SELECT id, name, id_uppercat
        FROM categories
        WHERE id IN ({$category_ids_str});
        SQL;
    $result = $conf->sql_backend::pwg_query($query);

    while ($row = $conf->sql_backend::pwg_db_fetch_assoc($result)) {
        $row['name'] = functions_plugins::trigger_change('render_category_name', $row['name'], 'admin_cat_list');

        $sort[] = $order_by_date ? $ref_dates[$row['id']] : functions::remove_accents($row['name']);

        $categories[] = [
            'id' => $row['id'],
            'id_uppercat' => $row['id_uppercat'],
        ];
    }

    array_multisort(
        $sort,
        $order_by_field === 'natural_order' ? SORT_NATURAL : SORT_REGULAR,
        $order_by_asc === 'ASC' ? SORT_ASC : SORT_DESC,
        $categories
    );

    functions_admin::save_categories_order($categories);

    $open_cat = $_POST['id'];
}

$template->assign('open_cat', $open_cat);

// +-----------------------------------------------------------------------+
// |                       template initialization                         |
// +-----------------------------------------------------------------------+
$template->set_filename('albums', 'albums.tpl');

$template->assign(
    [
        'F_ACTION' => functions_url::get_root_url() . 'admin.php?page=albums',
    ]
);

$template->assign('delay_before_autoOpen', $conf->album_move_delay_before_auto_opening);

$template->assign('POS_PREF', $conf->newcat_default_position); //TODO use user pref if it exists

// +-----------------------------------------------------------------------+
// |                          Album display                                |
// +-----------------------------------------------------------------------+

//Get all albums
$query = <<<SQL
    SELECT id, name, sort_rank, status, visible, uppercats, lastmodified
    FROM categories;
    SQL;

$allAlbum = $conf->sql_backend::query2array($query);

//Make an id tree
$associatedTree = [];

foreach ($allAlbum as $album) {
    $album['name'] = functions_plugins::trigger_change('render_category_name', $album['name'], 'admin_cat_list');
    $album['lastmodified'] = functions::time_since($album['lastmodified'], 'year');

    $parents = explode(',', $album['uppercats']);
    $the_place = &$associatedTree[$parents[0]];
    $counter = count($parents);

    for ($i = 1; $i < $counter; $i++) {
        $the_place = &$the_place['children'][$parents[$i]];
    }

    $the_place['cat'] = $album;
}

// WARNING $user['forbidden_categories'] is 100% reliable only on gallery side because
// it's a cache variable. On administration side, if you modify public/private status
// of an album or change permissions, this variable is reset and not recalculated until
// you open the gallery. As this situation doesn't occur each time you use the
// administration, it's quite reliable but not as much as on gallery side.
$is_forbidden = array_fill_keys(explode(',', ($user['forbidden_categories'] ?? '')), 1);

//Make an ordered tree
$query = <<<SQL
    SELECT category_id, COUNT(*) AS nb_photos
    FROM image_category
    GROUP BY category_id;
    SQL;

$nb_photos_in = $conf->sql_backend::query2array($query, 'category_id', 'nb_photos');

$query = <<<SQL
    SELECT id, uppercats
    FROM categories;
    SQL;
$all_categories = $conf->sql_backend::query2array($query, 'id', 'uppercats');

$subcats_of = [];

foreach ($all_categories as $id => $uppercats) {
    foreach (array_slice(explode(',', $uppercats), 0, -1) as $uppercat_id) {
        $subcats_of[$uppercat_id][] = $id;
    }
}

$nb_sub_photos = [];

foreach ($subcats_of as $cat_id => $subcat_ids) {
    $nb_photos = 0;

    foreach ($subcat_ids as $id) {
        if (isset($nb_photos_in[$id])) {
            $nb_photos += $nb_photos_in[$id];
        }
    }

    $nb_sub_photos[$cat_id] = $nb_photos;
}

$page_data = [
    'data' => functions_admin::assocToOrderedTree($associatedTree),
    'pwgToken' => functions::get_pwg_token(),
    'strShowSub' => functions::l10n('Show sub-albums'),
    'strHideSub' => functions::l10n('Hide sub-albums'),
    'strManageSubAlbum' => functions::l10n('Manage sub-albums'),
    'strApplyOrderRaw' => functions::l10n('apply automatic sort order'),
    'strEdit' => functions::l10n('Edit album'),
    'strAreYouSure' => functions::l10n("The status of the album '%s' and its sub-albums will change to private. Are you sure?"),
    'strYesChangeParent' => functions::l10n('Yes change parent anyway'),
    'strNoChangeParent' => functions::l10n("No, don't move this album here"),
    'strRoot' => functions::l10n('Root'),
    'openCat' => $open_cat,
    'nbAlbums' => count($allAlbum),
    'lightAlbumManager' => ($albums_counter > $conf->light_album_manager_threshold) ? 1 : 0,
    'xNbSubcats' => functions::l10n('%d sub-albums'),
    'xNbImages' => functions::l10n('%d photos'),
    'xNbSubPhotos' => functions::l10n('%d pictures in sub-albums'),
    'delayAutoOpen' => $conf->album_move_delay_before_auto_opening,
    'deleteAlbumWithName' => functions::l10n('Delete album "%s".'),
    'deleteAlbumWithSubs' => functions::l10n('Delete album "%s" and its %d sub-albums.'),
    'hasImagesAssociatedOutside' => functions::l10n('delete album and all %d photos, even the %d associated to other albums'),
    'hasImagesBecomingOrphans' => functions::l10n('delete album and the %d orphan photos'),
    'hasImagesRecursives' => functions::l10n('delete only album, not photos'),
    'renameItem' => functions::l10n('Rename "%s"'),
    'strAddAlbum' => functions::l10n('Add Album'),
    'strEditAlbum' => functions::l10n('Edit album'),
    'strAddPhoto' => functions::l10n('Add Photos'),
    'strVisitGallery' => functions::l10n('Visit Gallery'),
    'strSortOrder' => functions::l10n('Automatic sort order'),
    'strDeleteAlbum' => functions::l10n('Delete album'),
    'strRootOrder' => functions::l10n('Apply to root albums'),
    'strSubAlbumOrder' => functions::l10n('Apply to direct sub-albums'),
    'strAlbumNameEmpty' => functions::l10n('Album name must not be empty'),
    'addAlbumRootTitle' => functions::l10n('Create a new album at root'),
    'addSubAlbumOf' => functions::l10n('Create a sub-album of "%s"'),
    'tiptipLockedAlbum' => functions::l10n('Locked album'),
];

$template->assign(
    [
        'page_data_json' => json_encode($page_data),
        'nb_albums' => count($allAlbum),
        'ADMIN_PAGE_TITLE' => functions::l10n('Albums'),
        'light_album_manager' => ($albums_counter > $conf->light_album_manager_threshold) ? 1 : 0,
    ]
);

// +-----------------------------------------------------------------------+
// |                          sending html code                            |
// +-----------------------------------------------------------------------+

require_once __DIR__ . '/../inc/vite_helper.php';
\Piwigo\Vite\vite_assign_modules($template, ['albums', 'pwgConfirm']);

$template->assign_var_from_handle('ADMIN_CONTENT', 'albums');
