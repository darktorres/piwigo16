...
if (isset($_FILES['watermarkImage']) and
            ...
            if ($glob !== false) {
                foreach ($glob as $file) {
                    $watermark_files[] = functions::get_filename_wo_extension(
                        substr($file, strlen('./local/watermarks/'))
                    );
                }
            }
...
if (count($errors) == 0) {
    ...
    foreach (ImageStdParams::get_defined_type_map() as $type => $params) {
        ...
        if (! $changed and
            ...
            $changed |= $watermark->min_size[0] != $old_watermark->min_size[0] and
                                                  ($watermark->min_size[0] < $params->max_width() or $old_watermark->min_size[0] < $params->max_width());

            $changed |= $watermark->min_size[1] != $old_watermark->min_size[1] and
                                                  ($watermark->min_size[1] < $params->max_height() or $old_watermark->min_size[1] < $params->max_height());
        }
