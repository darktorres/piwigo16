...
class functions_admin
{
   ...
   */
  static function delete_categories($ids, $photo_deletion_mode='no_delete')
  {
    if (count($ids) == 0)
    {
      return;
   ...
   */
  static function delete_elements($ids, $physical_deletion=false)
  {
    if (count($ids) == 0)
    {
      return 0;
   ...
   */
  static function delete_orphan_tags()
  {
   ...
   */
  static function get_orphan_tags()
  {
    $query = '
  SELECT
      id,
   ...
   */
  static function update_category($ids = 'all')
  {
    global $conf;
   ...
   */
  static function images_integrity()
  {
    $query = '
  SELECT
      image_id
   ...
   */
  static function categories_integrity()
  {
   ...
   */
  static function update_global_rank()
  {
    $query = '
   ...
   * @param string $value
   */
  static function set_cat_status($categories, $value)
  {
  ...
  ;';
      ...
      foreach ($top_categories as $top_category)
      {
        ...
        $subcats = functions_category::get_subcat_ids(array($top_category['id']));
   ...
   * @return int[]
   */
  static function get_uppercat_ids($cat_ids)
  {
   ...
   * @return array
   */
  static function get_fs($path, $recursive = true)
  {
    global $conf;
   ...
   */
  static function update_uppercats()
  {
    $query = '
  ...
  ;';
    ...
    foreach ($cat_ids as $cat_id)
    {
      $query = '
  UPDATE '.IMAGES_TABLE.'
    SET path = '.functions_mysqli::pwg_db_concat(array("'".$fulldirs[$cat_id]."/'",'file')).'
   ...
   */
  static function move_categories($category_ids, $new_parent = -1)
  {
    global $page;
   ...
   * @param int $image_id
   */
  static function set_tags($tags, $image_id)
  {
    self::set_tags_of( array($image_id=>$tags) );
  }
   ...
   * @param int[] $images
   */
  static function add_tags($tags, $images)
  {
   ...
   * @param int[] $tag_ids
   */
  static function delete_tags($tag_ids)
  {
   ...
   * @return int
   */
  static function tag_id_from_tag_name($tag_name)
  {
    global $page;
   ...
   */
  static function set_tags_of($tags_of)
  {
    if (count($tags_of) > 0)
    {
   ...
   */
  static function compare_image_tag_lists($taglist_before, $taglist_after)
  {
   ...
   */
  static function fill_lounge($images, $categories)
  {
    $inserts = array();
    foreach ($categories as $category_id)
    {
  ...
  ;';
    ...
    foreach ($rows as $idx => $row)
    {
      ...
      if (!isset($rows[$idx+1]) or $rows[$idx+1]['category_id'] != $row['category_id'])
      {
        ...
        self::associate_images_to_categories($images, array($row['category_id']));
        $images = array();
      }
    }
   ...
   * @param int[] $categories
   */
  static function associate_images_to_categories($images, $categories)
  {
    if (count($images) == 0
   ...
   * @param int[] $categories
   */
  static function move_images_to_categories($images, $categories)
  {
    if (count($images) == 0)
    {
      return false;
   ...
   */
  static function associate_categories_to_categories($sources, $destinations)
  {
    if (count($sources) == 0)
    {
      return false;
   ...
   */
  static function invalidate_user_cache($full = true)
  {
    if ($full)
    {
      $query = '
   ...
   */
  static function invalidate_user_cache_nb_tags()
  {
    global $user;
   ...
   * @return string
   */
  static function create_table_add_character_set($query)
  {
   ...
   * @return array
   */
  static function get_user_access_level_html_options($MinLevelAccess = ACCESS_FREE, $MaxLevelAccess = ACCESS_CLOSED)
  {
    $tpl_options = array();
   ...
   * @return string[]
   */
  static function get_extents($start='')
  {
   ...
   */
  static function create_tag($tag_name)
  {
   ...
   * @return bool
   */
  static function cat_admin_access($category_id)
  {
    global $user;
   ...
   * @return bool
   */
  static function fetchRemote($src, &$dest, $get_data=array(), $post_data=array(), $user_agent='Piwigo', $step=0)
  {
    global $conf;
    ...
    if (function_exists('curl_init') && function_exists('curl_exec'))
    {
      ...
      if ($content !== false and $status >= 200 and $status < 400)
      {
        ...
        {
          return self::fetchRemote($m[1], $dest, array(), array(), $user_agent, $step+1);
        }
    ...
    while (!feof($s))
    {
      ...
      if (!$in_content)
      {
        ...
        {
          fclose($s);
          return self::fetchRemote(trim($m[1]),$dest,array(),array(),$user_agent,$step+1);
        }
        $i++;
        continue;
      }
   ...
   * @return string|false
   */
  static function get_groupname($group_id)
  {
    $query = '
  SELECT name
    FROM `'.GROUPS_TABLE.'`
  ...
  static function delete_groups($group_ids) 
  {
   ...
   * @return string
   */
  static function get_newsletter_subscribe_base_url($language='en_UK')
  {
   ...
   * @return int
   */
  static function get_active_menu($menu_page)
  {
    global $page;
   ...
   */
  static function get_taglist($query, $only_user_language=true)
  {
    $result = functions_mysqli::pwg_query($query);
   ...
   * @return int[]
   */
  static function order_by_name($element_ids, $name)
  {
   ...
   * @param int[] $user_ids
   */
  static function add_permission_on_category($category_ids, $user_ids)
  {
   ...
   * @param 'all'|int[] $types
   */
  static function clear_derivative_cache($types='all')
  {
    if ($types === 'all')
    {
   ...
   * @param 'all'|int $type
   */
  static function delete_element_derivatives($infos, $type='all')
  {
    $path = $infos['path'];
   ...
   * @return string[]
   */
  static function get_dirs($directory)
  {
    $sub_dirs = array();
   ...
   */
  static function deltree($path, $trash_path=null)
  {
    if (is_dir($path))
    {
      $fh = opendir($path);
   ...
   */
  static function get_photos_no_md5sum()
  {
    $query = '
  SELECT id
    FROM '.IMAGES_TABLE.'
   ...
   */
  static function get_image_infos($image_id, $die_on_missing=false)
  {
  ...
  static function order_by_is_local()
  {
    $conf = array();
    ...
    if (functions_mysqli::pwg_db_num_rows($result) > 0)
    {
      ...
      if ($env_nbm['is_sendmail_timeout'])
      {
        ...
        if (count($quoted_check_key_list) != 0 )
        {
          ...
          functions::redirect($base_url.functions_url::get_query_string_diff(array(), false), functions::l10n('Operation in progress')."\n".functions::l10n('Please wait...'));
        }
      }
    }
  }
