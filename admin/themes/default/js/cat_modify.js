        ...
        jQuery.ajax({
            ...
            success: function (data) {
                if (data.stat == "ok") {
                    ...
                    $(".info-message").show();
                    $(".cat-modification .cat-modify-info-subcontent").html(
                        str_just_now,
                    );
                    $(".cat-modification .cat-modify-info-content").html(
                        str_just_now,
                    );

                    is_visible = $("#cat-locked").is(":checked")
                        ? "false"
                        : "true";
                    checkAlbumLock();
            ...
            content: function () {
                ...
                <label for="delete_orphans">${str_delete_orphans.replace("%d", data.nb_images_becoming_orphan)}</label>
                    ...
                    error: function (message) {
                        console.log(message);
                        self.setContent(
                            "An error has occurred while calculating orphans",
                        );
                    },
                });
            },
