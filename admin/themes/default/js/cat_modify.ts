...
function pwgGet(method: string, data: Record<string, any>): Promise<any> {
    return fetch(`ws.php?format=json&method=${method}&` + new URLSearchParams(
        Object.fromEntries(Object.entries(data).map(([k, v]) => [k, String(v ?? '')]))
    )).then(r => r.json());
}

function showInfo(show: boolean) {
    document.querySelectorAll<HTMLElement>('.info-message').forEach(el => { el.style.display = show ? '' : 'none'; });
    if (show) setTimeout(() => document.querySelectorAll<HTMLElement>('.info-message').forEach(el => { el.style.display = 'none'; }), 5000);
}

function showError(show = true) {
    document.querySelectorAll<HTMLElement>('.info-error').forEach(el => { el.style.display = show ? '' : 'none'; });
    if (show) setTimeout(() => document.querySelectorAll<HTMLElement>('.info-error').forEach(el => { el.style.display = 'none'; }), 5000);
}
...
function add_related_category({ album, newSelectedAlbum, getSelectedAlbum }: { album: any; newSelectedAlbum: any; getSelectedAlbum: any }) {
    if (parent_album != album.id) {
        document.getElementById('cat-parent')!.innerHTML = album.full_name_with_admin_links ?? album.root;
        document.querySelector<HTMLElement>(`#${album.id}.search-result-item > *`)?.classList.add('notClickable');
        document.querySelector('.invisible-related-categories-select')
...
function delete_album(photo_deletion_mode: any): Promise<void> {
    return pwgPost('pwg.categories.delete', { category_id: album_id, photo_deletion_mode, pwg_token })
        .then(data => {
...
async function showDeleteAlbumDialog(): Promise<string | null> {
    ...
    return new Promise(resolve => {
        ...
        dialog.querySelector('.confirm-btn')?.addEventListener('click', async () => {
            const mode = dialog.querySelector<HTMLInputElement>('input[name=deletion-mode]:checked')?.value;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('cat-properties-save')?.addEventListener('click', () => {
        ...
        document.querySelectorAll<HTMLElement>('.info-error,.info-message').forEach(el => { el.style.display = 'none'; });
        const catLocked = (document.getElementById('cat-locked') as HTMLInputElement).checked;
        const catCommentable = (document.getElementById('cat-commentable') as HTMLInputElement).checked;
        ...
        pwgPost('pwg.categories.setInfo', {
            ...
            if (data.stat == 'ok') {
                ...
                document.querySelectorAll<HTMLElement>('.cat-modification .cat-modify-info-subcontent,.cat-modification .cat-modify-info-content')
                    .forEach(el => { el.innerHTML = str_just_now; });
                is_visible = catLocked ? 'false' : 'true';
                checkAlbumLock();
                showInfo(true);
            } else showError();
        }).catch(err => { save_button_set_loading(false); showError(); console.log(err); });

        if (parent_album != default_parent_album) {
            pwgPost('pwg.categories.move', { category_id: album_id, parent: parent_album, pwg_token })
                .then(data => {
                    if (data.stat === 'ok') {
        ...
        pwgPost('pwg.categories.refreshRepresentative', { category_id: album_id })
            .then(data => {
                if (data.stat == 'ok') {
                    ...
                    if (rep) { rep.style.backgroundImage = `url('${data.result.src}')`; rep.classList.remove('icon-dice-solid'); }
                } else console.error(data);
                icon.classList.replace('icon-spin6', 'icon-ccw');
                icon.classList.remove('animate-spin');
            })
            .catch(err => { console.error(err); icon.classList.replace('icon-spin6', 'icon-ccw'); icon.classList.remove('animate-spin'); });
    });
        ...
        pwgPost('pwg.categories.deleteRepresentative', { category_id: album_id })
            .then(data => {
                if (data.stat == 'ok') {
                    ...
                    const rep = document.querySelector<HTMLElement>('.cat-modify-representative');
                    if (rep) { rep.removeAttribute('style'); rep.classList.add('icon-dice-solid'); }
                } else console.error(data);
                icon.classList.replace('icon-spin6', 'icon-cancel');
                icon.classList.remove('animate-spin');
            })
            .catch(err => { console.error(err); icon.classList.replace('icon-spin6', 'icon-cancel'); icon.classList.remove('animate-spin'); });
    });

    document.getElementById('cat-parent')?.querySelector('.icon-pencil')?.addEventListener('click', (e) => {
        if ((e.target as HTMLElement).localName !== 'a') ab.open();
    });

    function handleCommentable(commentable: boolean) {
        save_button_set_loading(true);
        pwgPost('pwg.categories.setInfo', { category_id: album_id, commentable: String(commentable), apply_commentable_to_subalbums: 'true' })
            .then(data => {
                if (data.stat == 'ok') {
                    save_button_set_loading(false);
                    ...
                    if (msg) {
                        ...
                        msg.textContent = commentable ? str_album_comment_allow : str_album_comment_disallow;
                        msg.style.display = '';
                        setTimeout(() => { msg.style.display = 'none'; msg.textContent = temp_txt; }, 5000);
                    }
                } else showError();
            })
            .catch(e => { console.log(e); save_button_set_loading(false); });
    }

    document.querySelector('.allow-comments')?.addEventListener('click', () => handleCommentable(true));
    document.querySelector('.disallow-comments')?.addEventListener('click', () => handleCommentable(false));
    ...
    textareas.forEach(ta => {
        ta.addEventListener('keyup', function(this: HTMLTextAreaElement) {
            textareas.forEach(other => { other.value = this.value; });
        });
    });
    ...
    document.addEventListener('keyup', (e: KeyboardEvent) => {
        if (e.key === 'Escape' && descModal.style.display !== 'none') descModal.style.display = 'none';
    });
});
