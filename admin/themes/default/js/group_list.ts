...
type PanelState = 'NoSelection' | 'OneSelected' | 'ConfirmDeletion' | 'Selection' | 'OptionMerge';
...
export function init(cfgRaw: Record<string, unknown>): void {
    const cfg = cfgRaw as unknown as GroupListConfig;
    ...
    function updateBadge(): void {
        ...
        if (badge) badge.innerHTML = String(document.querySelectorAll(".GroupContainer").length - 2); //Less the add group div and the template
    }
    ...
    document.getElementById("form-btn")?.addEventListener("click", function () {
        ...
        const addUserLabel = document.getElementById("addUserLabel");
        if (addUserLabel) addUserLabel.style.display = '';
        const userSearch = document.querySelector<HTMLElement>(".UserSearch");
        if (userSearch) userSearch.style.display = '';
        const userSubmit = document.getElementById("UserSubmit");
        if (userSubmit) userSubmit.style.display = '';
    ...
    document.getElementById("cancel")?.addEventListener("click", function () {
        ...
        const addUserLabel = document.getElementById("addUserLabel");
        if (addUserLabel) addUserLabel.style.display = 'none';
        const userSearch = document.querySelector<HTMLElement>(".UserSearch");
        if (userSearch) userSearch.style.display = 'none';
        const userSubmit = document.getElementById("UserSubmit");
        if (userSubmit) userSubmit.style.display = 'none';
        const formBtn = document.getElementById("form-btn");
    ...
    document.querySelector("#addGroupForm form")?.addEventListener("submit", function (e) {
        e.preventDefault();
        const nameInput = document.querySelector<HTMLInputElement>("#addGroupForm input[type=text]");
        ...
        if (name.replace(/\s/g, "").length !== 0) {
            ...
            .then(raw_data => {
                ...
                if (data.stat === "ok") {
                    const nameInput2 = document.querySelector<HTMLInputElement>(".addGroupFormLabelAndInput input");
    ...
    function setupGroupBox(groupBox: HTMLElement): void {
        ...
        if (validateBtn) {
            validateBtn.addEventListener("click", function () {
                const form = groupBox.querySelector<HTMLFormElement>(".group-rename form");
                if (form) form.dispatchEvent(new Event("submit"));
            });
        }

        const renameForm = groupBox.querySelector<HTMLFormElement>(".group-rename form");
        if (renameForm) {
        ...
        document.addEventListener("mouseup", function (e) {
            ...
            if (!(option_is_clicked as boolean)) {
                const opts = groupBox.querySelector<HTMLElement>("#GroupOptions");
                if (opts) opts.style.display = 'none';
            }
        });
    ...
    function toggleSelection(group_id: string, toggle: boolean): void {
        ...
        if (toggle) {
            ...
            item.querySelector("a")?.addEventListener("click", function () {
                const checkbox2 = groupBox.querySelector<HTMLInputElement>(".Group-checkbox input");
            ...
            document.querySelectorAll<HTMLElement>("#MergeOptionsChoices option").forEach(el => {
                if ((el as HTMLOptionElement).value === group_id) el.remove();
            });
        }
    };
    ...
    function deleteGroup(id: string): void {
        const groupName = document.querySelector("#group-" + id + " #group_name")?.innerHTML ?? "";
        pwgConfirm({
    ...
    function renameGroup(id: string, newName: string): void {
        ...
        if (newName.replace(/\s/g, "").length !== 0) {
            ...
            .then(raw_data => {
                ...
                loadState.reverse();
                if (data.stat === "ok") {
                    const updatedName = data.result.groups[0]?.name ?? newName;
    ...
    function setupDefaultActions(id: string, is_default: boolean): void {
        ...
        if (is_default) {
            if (btn) {
                ...
                const oldHandler = () => { setDefaultGroup(id, false); };
                btn.replaceWith(btn.cloneNode(true));
                const newBtn = document.querySelector<HTMLElement>("#group-" + id + " #GroupDefault");
                if (newBtn) newBtn.addEventListener("click", oldHandler);
            }
            ...
            if (token) {
                ...
                const newToken = document.querySelector<HTMLElement>("#group-" + id + " .is-default-token");
                if (newToken) newToken.addEventListener("click", oldHandler);
            }
        } else {
            if (btn) {
    ...
    function duplicateAction(id: string): void {
        ...
        let copy_name = (document.querySelector("#group-" + id + " #group_name")?.innerHTML ?? "") + str_copy;
        ...
        while (name_exist(copy_name)) {
            copy_name = (document.querySelector("#group-" + id + " #group_name")?.innerHTML ?? "") + str_other_copy.replace("%s", String(i++));
        }
    ...
    -------*/
    let state: PanelState = "NoSelection";

    function updateSelectionPanel(changedState: PanelState | "" = ""): void {
        ...
        const numEl = document.querySelector(".number-Selected");
        if (numEl) numEl.innerHTML = numSelect + "";
    };

    function updateStatePanel(newState: PanelState = "Selection"): void {
        state = newState;
    ...
    function buttonAvailable(button: HTMLElement | null, onClick: string): void {
        if (button) {
    ...
    function buttonUnavailable(button: HTMLElement | null): void {
        if (button) {
    ...
    document.querySelector(".ConfirmMergeButton")?.addEventListener("click", function () {
        ...
        const dest_grp: string = destSelect?.value ?? "";
        ...
        .then(raw_data => {
            ...
            if (data.stat === "ok") {
                ...
                const list = document.querySelector(".DeleteGroupList");
                ...
                .then(response => response.text())
                .then(raw_data => {
                    const data = JSON.parse(raw_data) as { result: { users: UserData[] } };
                    const number = data.result.users.length;
    ...
    let usersCache: UsersCache | undefined;
    let usersInGroup: UserData[] = [];
    const maxOffsetUserCont = 322;
    let idSearch = "";

    function updateUserSearch(): void {
        ...
        try {
            usersCache.getFromStorage()?.forEach(function (u) {
        ...
        if (selectize) {
            for (const [, value] of Object.entries(selectize.options as Record<string, { username?: string; id?: string | number }>)) {
                if (value.username === "guest") {
                    selectize.removeOption(value.id as string);
                }
            }
        }
    ...
    function openUserManager(grp_id: string): void {
        ...
        .then(response => response.text())
        .then(raw_data => {
            const data = JSON.parse(raw_data) as { stat: string; result: { users: UserData[] } };
            loadState.reverse();
            if (data.stat === "ok") {
                ...
                const userList = document.querySelector(".UsersInGroupList");
                ...
                while (userListEl && userListEl.offsetHeight <= maxOffsetUserCont && usersInGroup[i] !== undefined) {
                    const u = usersInGroup[i];
                    if (u) {
                        const userEl = getUserDisplay(u.username, String(u.id), grp_id);
    ...
    function getUserDisplay(username: string, user_id: string, grp_id: string): HTMLElement {
            ...
            cancelBtn.addEventListener("click", function () {
                ...
                .then(raw_data => {
                    ...
                    if (data.stat === "ok") {
                        ...
                        const dissP = dissociateUserInfo.querySelector("p");
                        if (dissP) dissP.innerHTML = str;
                        ...
                        const userList2 = document.querySelector<HTMLElement>(".UsersInGroupList");
                        while (userList2 && userList2.offsetHeight > maxOffsetUserCont) {
                            userList2.querySelector(".UsernameBlock:last-child")?.remove();
                        }

                        usersInGroup = usersInGroup.filter((u) => u.id !== user_id);

                        const badgeNum = document.querySelector(".UserNumberBadge")?.innerHTML ?? "0";
    ...
    function updateMembernumber(number: number, grp_id: string): void {
        ...
        const badge = document.querySelector(".UserNumberBadge");
        if (badge) badge.innerHTML = String(number);

        const shown2 = document.querySelector(".AmountOfUsersShown strong:nth-child(2)");
        if (shown2) shown2.innerHTML = String(number);

        const shown1 = document.querySelector(".AmountOfUsersShown strong:nth-child(1)");
        ...
        if (id !== "") {
            ...
            .then(raw_data => {
                ...
                if (data.stat === "ok") {
                    ...
                    const usersList = document.querySelector<HTMLElement>(".UsersInGroupList");
                    usersList?.insertBefore(userBlock, usersList.firstChild);
                    ...
                    const assocP = associateUserInfo.querySelector("p");
    ...
    document.querySelector<HTMLInputElement>(".input-user-name")?.addEventListener("input", function () {
        ...
        if (searchString !== "") {
            ...
            usersInGroup.forEach(function (u) {
                ...
                if (userBlock && !isSearched) {
                    ...
                    const newBlock = getUserDisplay(u.username, String(u.id), grp_id);
                    const list = document.querySelector<HTMLElement>(".UsersInGroupList");
                    list?.insertBefore(newBlock, list.firstChild);
                }
            });
        } else {
            const container = document.querySelector<HTMLElement>(".UsersInGroupListContainer");
            ...
            const userList2 = document.querySelector<HTMLElement>(".UsersInGroupList");
            if (userList2) userList2.innerHTML = "";

            let i = 0;
            while (userList2 && userList2.offsetHeight <= maxOffsetUserCont && usersInGroup[i] !== undefined) {
                const u = usersInGroup[i];
                if (u) {
                    const userEl = getUserDisplay(u.username, String(u.id), grp_id);
                    userList2.appendChild(userEl);
                }
                i++;
            }
        }
        const shown = document.querySelector(".AmountOfUsersShown strong:nth-child(1)");
        if (shown) shown.innerHTML = String(document.querySelectorAll(".UsernameBlock").length);

        const userList3 = document.querySelector<HTMLElement>(".UsersInGroupList");
        while (userList3 && userList3.offsetHeight > maxOffsetUserCont) {
            userList3.querySelector(".UsernameBlock:last-child")?.remove();
        }
    });
    ...
    document.addEventListener('click', function(e) {
        if (!(e.target as HTMLElement).closest(".UserListPopInContainer")) {
            const userListEl = document.getElementById("UserList");
            if (userListEl) {
                userListEl.style.display = 'none';
            }
