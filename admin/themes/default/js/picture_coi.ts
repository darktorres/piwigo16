document.addEventListener('DOMContentLoaded', () => {
    ...
    container.style.cssText = 'position:relative;display:inline-block;cursor:crosshair;user-select:none;';
    ...
    overlay.style.cssText = 'position:absolute;border:2px solid #fff;box-shadow:0 0 0 9999px rgba(0,0,0,.5);pointer-events:none;display:none;box-sizing:border-box;';
    ...
    function applyOverlay(x: number, y: number, w: number, h: number) {
        Object.assign(overlay.style, { left: x + 'px', top: y + 'px', width: w + 'px', height: h + 'px', display: 'block' });
    }
    ...
    document.addEventListener('mousemove', (e) => {
        if (!dragging) return;
        const p = relCoords(e);
        applyOverlay(Math.min(startX, p.x), Math.min(startY, p.y), Math.abs(p.x - startX), Math.abs(p.y - startY));
    });
    ...
    function restoreCoi() {
        ...
        const r = imgEl!.getBoundingClientRect();
        if (!r.width) return;
        const l = parseFloat(lEl.value), t = parseFloat(tEl.value);
        const ri = parseFloat(rEl.value), b = parseFloat(bEl.value);
