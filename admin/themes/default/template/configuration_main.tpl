    ...
    <fieldset class="mainConf">
      ...
      <ul>
        <li>
          ...
          <br>
          <input type="text" maxlength="255" size="50" name="gallery_title" id="gallery_title"
            value="{$main.CONF_GALLERY_TITLE}">
        </li>

        <li>
          <label for="page_banner">{'Page banner'|translate}</label>
          <br>
          <textarea rows="5" cols="50" class="description" name="page_banner"
            id="page_banner">{$main.CONF_PAGE_BANNER}</textarea>
        </li>
        ...
        <li id="order_filters">
          ...
          {else}
            <span
              class="order_by_is_custom">{'You can\'t define a default photo order because you have a custom setting in your local configuration.'|translate}</span>
          {/if}
        </li>
      </ul>
    </fieldset>
    ...
    <fieldset class="mainConf">
      ...
      <ul>
        ...
        <li>
          <label class="font-checkbox">
            <span class="icon-check"></span>
            <input type="checkbox" name="allow_user_registration" {if ($main.allow_user_registration)}checked="checked"
              {/if}>
          ...
          <div id="email_admin_on_new_user" class="sub-setting">
            <label class="font-checkbox no-bold">
              <span class="icon-check"></span>
              <input type="checkbox" name="email_admin_on_new_user"
                {if ($main.email_admin_on_new_user)}checked="checked" {/if}>
              {'Email admins when a new user registers'|translate}
            </label>

            <div id="email_admin_on_new_user_filter" class="sub-setting" {if (!$main.email_admin_on_new_user)}
              style="display:none" {/if}>
              <label class="font-checkbox no-bold">
                <span class="icon-dot-circled"></span>
                <input type="radio" name="email_admin_on_new_user_filter" value="all"
                  {if ($main.email_admin_on_new_user_filter eq 'all')}checked{/if}>
                {'All admins'|translate}
              </label>
              <br>
              <label class="font-checkbox no-bold">
                <span class="icon-dot-circled"></span>
                <input type="radio" name="email_admin_on_new_user_filter" value="group"
                  {if ($main.email_admin_on_new_user_filter eq 'group')}checked{/if}>
                {'Only admins in a specific group'|translate}
              </label>

              <span id="email_admin_on_new_user_filter_group_options"
                {if ($main.email_admin_on_new_user_filter ne 'group')} style="display:none" {/if}>
        ...
        <li>
          <label class="font-checkbox">
            <span class="icon-check"></span>
            <input type="checkbox" name="allow_user_customization"
              {if ($main.allow_user_customization)}checked="checked" {/if}>
        ...
        <li>
          <label class="font-checkbox">
            <span class="icon-check"></span>
            <input type="checkbox" name="obligatory_user_mail_address"
              {if ($main.obligatory_user_mail_address)}checked="checked" {/if}>
    ...
    <fieldset class="mainConf">
      ...
      <ul>
        ...
        <li>
          ...
          <label class="font-checkbox no-bold">
            <span class="icon-check"></span>
            <input type="checkbox" name="show_mobile_app_banner_in_gallery"
              {if ($main.show_mobile_app_banner_in_gallery)}checked="checked" {/if}>
            {'in gallery'|translate}
          </label>

          <label class="font-checkbox no-bold">
            <span class="icon-check"></span>
            <input type="checkbox" name="show_mobile_app_banner_in_admin"
              {if ($main.show_mobile_app_banner_in_admin)}checked="checked" {/if}>
            {'in administration'|translate}
          </label>

          <span class="icon-help-circled tiptip-with-img show-mobile-app-banner-tooltip"
            title="{'Displays a banner to install or open the official Piwigo app'|translate}<br><img src='admin/themes/default/images/piwigo_app_banner.jpg' style='width:100%;margin-top:5px;'>"
        ...
        <li>
          <label class="font-checkbox">
            <span class="icon-check"></span>
            <input type="checkbox" name="upload_detect_duplicate" {if ($main.upload_detect_duplicate)}checked="checked"
              {/if}>
            {'Detect and avoid duplicates during upload'|translate}
          </label>

          <span class="icon-help-circled tiptip"
          ...
          <div class="themeBoxes font-checkbox">
            {foreach from=$main.mail_theme_options item=name key=theme}
              <div class="themeSelect {if $main.mail_theme==$theme}themeDefault{/if}">
                ...
                <a href="{$ROOT_URL}themes/default/template/mail/screenshot-{$theme}.png"
                  class="icon-zoom-in">{'Preview'|translate}</a>
              </div>
            {/foreach}
          </div>
        </li>
