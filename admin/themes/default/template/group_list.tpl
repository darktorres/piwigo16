...
{function name=groupContent}
  {function groupContent}
    <div id="group-{$grp_id}" class="GroupContainer" data-id={$grp_id} {if $grp_is_default}data-default=1
      {else}data-default=0 
      {/if}>
    ...
    {/if}">
      ...
      <a id="UserListTrigger"
        class="icon-user-1 manage-users not-in-selection-mode GroupManagerButtons">{'Manage the members'|translate}</a>
      <a class="icon-lock manage-permissions not-in-selection-mode GroupManagerButtons"
        href="admin.php?page=group_perm&group_id={$grp_id}">{'Manage Permissions'|translate}</a>
    </div>
  {/function}
{/function}
      ...
      <form>
        <fieldset>
          <div class="addGroupFormLabelAndInput">
            ...
            <input type="text" id="addGroupNameInput" name="groupname" maxlength="50" size="20"
