    ...
    <fieldset>
      ...
      <table>
        <tr>
          ...
          <td>
            <label><input type="radio" name="nbm_send_html_mail" value="true" {if $param.SEND_HTML_MAIL}checked="checked"
                {/if}>{'Yes'|translate}</label>
            <label><input type="radio" name="nbm_send_html_mail" value="false"
                {if $param.SEND_HTML_MAIL === "false" || !$param.SEND_HTML_MAIL}checked="checked"
                {/if}>{'No'|translate}</label>
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td><input type="text" maxlength="35" size="35" name="nbm_send_mail_as" id="send_mail_as"
              value="{$param.SEND_MAIL_AS}"></td>
        </tr>
        <tr>
          <td><label>{'Add detailed content'|translate}</label></td>
          <td>
            <label><input type="radio" name="nbm_send_detailed_content" value="true"
                {if $param.SEND_DETAILED_CONTENT}checked="checked" {/if}>{'Yes'|translate}</label>
            <label><input type="radio" name="nbm_send_detailed_content" value="false"
                {if !$param.SEND_DETAILED_CONTENT || $param.SEND_DETAILED_CONTENT === "false"}checked="checked"
                {/if}>{'No'|translate}</label>
          </td>
        </tr>
        <tr>
          <td><label for="complementary_mail_content">{'Complementary mail content'|translate}</label></td>
          <td><textarea cols="50" rows="5" name="nbm_complementary_mail_content"
              id="complementary_mail_content">{$param.COMPLEMENTARY_MAIL_CONTENT}</textarea></td>
        </tr>
        <tr>
          <td>
          ...
          <td>
            <label><input type="radio" name="nbm_send_recent_post_dates" value="true"
                {if $param.SEND_RECENT_POST_DATES}checked="checked" {/if}>{'Yes'|translate}</label>
            <label><input type="radio" name="nbm_send_recent_post_dates" value="false"
                {if !$param.SEND_RECENT_POST_DATES || $param.SEND_RECENT_POST_DATES === "false"}checked="checked"
                {/if}>{'No'|translate}</label>
          </td>
        </tr>
      </table>
  ...
  {if isset($send)}
    ...
    {else}
        ...
        <table class="table2">
          ...
          {foreach from=$send.users item=u name=user_loop}
            <tr class="{if $smarty.foreach.user_loop.index is odd}row1{else}row2{/if}">
              ...
              <td><input type="checkbox" name="send_selection[]" value="{$u.ID}" {$u.CHECKED} id="send_selection-{$u.ID}">
              </td>
            </tr>
          {/foreach}
        </table>
      ...
      <fieldset>
        ...
        <table>
          <tr>
            ...
            <td><textarea cols="50" rows="5" name="send_customize_mail_content"
                id="send_customize_mail_content">{$send.CUSTOMIZE_MAIL_CONTENT}</textarea></td>
          </tr>
        </table>
      </fieldset>
      ...
      {if isset($auth_key_duration)}
        <fieldset>
          <legend><span class="icon-info-circled-1 icon-blue"></span>{'Information'|translate}</legend>
          <p>
