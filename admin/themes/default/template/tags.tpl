...
<div class='tag-header'>
  ...
  {/if}
  <div class='info-message tag-info  icon-ok not-in-selection-mode' {if $message_tags != ""}style='display:flex' {/if}>
    <p> {$message_tags} </p>
  </div>
  <div class='info-error tag-info icon-cancel not-in-selection-mode'>
    <p> </p>
  </div>
</div>
<div class="pageLoad">
...
<div class="tag-pagination">
  <div class="pagination-per-page">
    ...
    <a id="200" {if isset($smarty.cookies.pwg_tags_per_page) && $smarty.cookies.pwg_tags_per_page == 200}
      class="selected" {/if}>200</a>
    <a id="500" {if isset($smarty.cookies.pwg_tags_per_page) && $smarty.cookies.pwg_tags_per_page == 500}
      class="selected" {/if}>500</a>
    <a id="1000" {if isset($smarty.cookies.pwg_tags_per_page) && $smarty.cookies.pwg_tags_per_page == 1000}
      class="selected" {/if}>1000</a>
  </div>
...
<div class='tag-template' style='display:none'>
  {tagContent tag_name='%name%' tag_U_VIEW='%U_VIEW%' tag_U_EDIT='%U_EDIT%' has_image=false tag_count='%count%' tag_raw_name = '%raw_name%'}
</div>