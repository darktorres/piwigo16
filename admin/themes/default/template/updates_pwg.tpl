...
{if $STEP == 1}
  <fieldset>
    ...
    <p class="release release-minor">
      ...
      {else}
        <a href="admin.php?page=updates&amp;step=2&amp;to={$MINOR_VERSION}"
          class="icon-arrows-cw goto-update-page">{'Update to Piwigo %s'|@translate:$MINOR_VERSION}</a>
      {/if}
    </p>
    ...
    <p class="release release-major">
      ...
      {else}
        <a href="admin.php?page=updates&amp;step=3&amp;to={$MAJOR_VERSION}"
          class="icon-arrows-cw goto-update-page">{'Update to Piwigo %s'|@translate:$MAJOR_VERSION}</a>
      {/if}
    </p>
...
{if $STEP == 2}
  ...
  <form action="" method="post">
    <p>
      <input type="submit" name="submit" value="{'Update to Piwigo %s'|@translate:$UPGRADE_TO}"
        {if isset($MINOR_RELEASE_PHP_REQUIRED)} disabled{/if}>
      {if isset($MINOR_RELEASE_PHP_REQUIRED)}
        <span class="errors icon-block">{'Requires PHP %s'|translate:$MINOR_RELEASE_PHP_REQUIRED}</span>
      {/if}
    </p>
    <p class="autoupdate_bar" style="display:none;">&nbsp; {'Update in progress...'|@translate}<br><img
...
{if $STEP == 3}
  ...
  <form action="" method="post">
    ...
    <fieldset>
      ...
      {/if}
      <p><input type="submit" name="submit" value="{'Update to Piwigo %s'|@translate:$UPGRADE_TO}"
          {if !empty($missing.plugins) or !empty($missing.themes) or isset($MAJOR_RELEASE_PHP_REQUIRED)}disabled="disabled"
          {/if}>
        {if isset($MAJOR_RELEASE_PHP_REQUIRED)}
          <span class="errors icon-block">{'Requires PHP %s'|translate:$MAJOR_RELEASE_PHP_REQUIRED}</span>
        {/if}
