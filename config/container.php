...
return [
    ...
    UserService::class         => factory(static fn (UserRepository $u, UserFavoriteRepository $uf, HistoryRepository $h, ActivityRepository $a, GroupRepository $g, AuthKeyRepository $ak, ActivityLogger $al, ExecutionMutex $mutex, LangService $lang, UrlGenerator $ug, MailService $mail, MessageBusInterface $bus, HtmlService $html, DateService $date, CategoryService $cat, ImageRepository $i, UserAdminService $uas, Session $session, SessionService $sess, AuthService $auth, PreferencesService $pref, PermissionService $perm, LanguageService $langSvc, ThemeService $theme, EventDispatcherInterface $dispatcher): UserService => new UserService($u, $uf, $h, $a, $g, $ak, $al, $mutex, $lang, $ug, $mail, $bus, $html, $date, $cat, $i, $uas, $session, $sess, $auth, $pref, $perm, $langSvc, $theme, $dispatcher)),
    ...
    HtmlService::class         => factory(static fn (CategoryRepository $catR, Session $session, EventDispatcherInterface $d): HtmlService => new HtmlService($catR, $session, $d)),
    ...
    UrlService::class          => factory(static fn (CategoryService $cat, HtmlService $h, TagService $tag, PermissionService $perm, UserFavoriteRepository $uf, EventDispatcherInterface $dispatcher): UrlService => new UrlService($cat, $h, $tag, $perm, $uf, $dispatcher)),
    ...
    ExtensionsEndpoints::class       => factory(static fn (PermissionService $perm, UrlGenerator $ug, ActivityLogger $al, CsrfService $csrf, RedirectResponder $redirect, IgnoredUpdatesRepository $iu, Session $session): ExtensionsEndpoints => new ExtensionsEndpoints($perm, $ug, $al, $csrf, $redirect, $iu, $session)),
    ...
    FilterResolver::class            => factory(static fn (CategoryRepository $catR, HtmlService $h, TagAdminService $tA, CsrfService $csrf, LangService $lang, Session $session, UrlService $us, EventDispatcherInterface $dispatcher): FilterResolver => new FilterResolver($catR, $h, $tA, $csrf, $lang, $session, $us, $dispatcher)),
    ...
    CategoryDefaultRenderer::class   => factory(static fn (CategoryService $cat, CommentRepository $cr, HtmlService $h, ImageRepository $i, Session $session, UrlService $u, DebugCollector $dbg, EventDispatcherInterface $dispatcher): CategoryDefaultRenderer => new CategoryDefaultRenderer($cat, $cr, $h, $i, $session, $u, $dbg, $dispatcher)),
    ...
    NoPhotoYetRenderer::class        => factory(static fn (ConfigService $cfg, ImageRepository $i, UrlGenerator $ug, RedirectResponder $redirect, UrlService $us, PermissionService $perm, Session $session, EventDispatcherInterface $dispatcher, Paths $paths): NoPhotoYetRenderer => new NoPhotoYetRenderer($cfg, $i, $ug, $redirect, $us, $perm, $session, $dispatcher, $paths)),
    ...
    PictureCommentRenderer::class    => factory(static fn (CommentRepository $cR, CommentService $com, DateService $d, HtmlService $h, PermissionService $perm, Session $session, UrlService $u, CsrfService $csrf, EphemeralKeyService $eks, PaginationService $pag, EventDispatcherInterface $dispatcher): PictureCommentRenderer => new PictureCommentRenderer($cR, $com, $d, $h, $perm, $session, $u, $csrf, $eks, $pag, $dispatcher)),
    ...
    ActivityLogger::class      => factory(static fn (ActivityRepository $aR, HistoryRepository $hist, HistoryAdminService $histA, PermissionService $perm, Session $session, UserRepository $u, EventDispatcherInterface $d): ActivityLogger => new ActivityLogger($aR, $hist, $histA, $perm, $session, $u, $d)),
    ...
    DeviceDetectionService::class => factory(static fn (Session $session): DeviceDetectionService => new DeviceDetectionService($session)),
    ...
    FilterMiddleware::class           => factory(static fn (CategoryService $cat, ImageRepository $i, Session $session, FilterService $f): FilterMiddleware => new FilterMiddleware($cat, $i, $session, $f)),
