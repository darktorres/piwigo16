...
return [
    ...
    SectionInitializer::class    => factory(static fn (CalendarService $cal, CategoryRepository $catR, CategoryService $cat, HtmlService $h, ImageRepository $i, PermissionService $perm, SearchService $sr, Session $session, TagService $tag, UrlService $u, UserRepository $ur, UserService $us, RedirectResponder $redirect, CacheItemPoolInterface $pool, EventDispatcherInterface $dispatcher, OrderByService $orderBy): SectionInitializer => new SectionInitializer($cal, $catR, $cat, $h, $i, $perm, $sr, $session, $tag, $u, $ur, $us, $redirect, $pool, $dispatcher, $orderBy)),
