...
return [
    ...
    CalendarService::class     => factory(static fn (CategoryService $cat, Connection $conn, Util $util): CalendarService => new CalendarService($cat, $conn, $util)),
    MailService::class         => factory(static fn (Connection $conn, StringUtil $s, UrlGenerator $ug, Util $util): MailService => new MailService($conn, $s, $ug, $util)),
    ...
    CategoryService::class     => factory(static fn (CategoryRepository $cat, Connection $conn, FilterService $f): CategoryService => new CategoryService($cat, $conn, $f)),
    HtmlService::class         => factory(static fn (Connection $conn, StringUtil $s): HtmlService => new HtmlService($conn, $s)),
    NotificationService::class => factory(static fn (Connection $conn, HtmlService $h, UrlGenerator $ug): NotificationService => new NotificationService($conn, $h, $ug)),
    ...
    UserAdminService::class          => factory(static fn (Connection $conn, ConfigService $cfg, GroupRepository $g, PermissionRepository $perm, SessionService $sess, UserRepository $u, UserService $us, Util $util): UserAdminService => new UserAdminService($conn, $cfg, $g, $perm, $sess, $u, $us, $util)),
    ...
    NoPhotoYetRenderer::class        => factory(static fn (ConfigService $cfg, ImageRepository $i, StringUtil $s, UrlGenerator $ug): NoPhotoYetRenderer => new NoPhotoYetRenderer($cfg, $i, $s, $ug)),
    PictureRateRenderer::class       => factory(static fn (RateRepository $rR): PictureRateRenderer => new PictureRateRenderer($rR)),
