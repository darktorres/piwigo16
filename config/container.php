...
return [
    ...
    MetadataService::class => factory(static fn (LoggerInterface $log, StringUtil $s, EventDispatcherInterface $d): MetadataService => new MetadataService($log, $s, $d)),
    ...
    MailService::class         => factory(static fn (Connection $conn, StringUtil $s, UrlGenerator $ug, UserRepository $u, LangService $lang, AuthService $auth, UrlService $us, EventDispatcherInterface $dispatcher): MailService => new MailService($conn, $s, $ug, $u, $lang, $auth, $us, $dispatcher)),
    ...
    UrlService::class          => factory(static fn (Connection $conn, StringUtil $s, CategoryService $cat, HtmlService $h, TagService $tag, PermissionService $perm, EventDispatcherInterface $dispatcher): UrlService => new UrlService($conn, $s, $cat, $h, $tag, $perm, $dispatcher)),
    UrlGenerator::class          => factory(static fn (Router $r, UrlService $u): UrlGenerator => new UrlGenerator($r, $u)),
    SectionInitializer::class    => factory(static fn (Connection $conn, CalendarService $cal, CategoryService $cat, HtmlService $h, PermissionService $perm, SearchService $sr, SessionService $sess, StringUtil $s, TagService $tag, UrlService $u, UserRepository $ur, UserService $us, RedirectResponder $redirect, CacheItemPoolInterface $pool, EventDispatcherInterface $dispatcher): SectionInitializer => new SectionInitializer($conn, $cal, $cat, $h, $perm, $sr, $sess, $s, $tag, $u, $ur, $us, $redirect, $pool, $dispatcher)),
    GeneralEndpoints::class          => factory(static fn (Connection $conn, AuthService $auth, CategoryRepository $catR, CommentRepository $com, ConfigService $cfg, CookieService $cookie, DateService $d, HistoryAdminService $hA, HtmlService $h, ImageAdminService $iA, ImageRepository $i, PermissionService $perm, PictureService $pic, RateService $rate, SearchRepository $sR, StringUtil $s, TagRepository $tR, UrlGenerator $ug, UrlService $us, UserAdminService $uA, UserRepository $u, WsHelper $ws, CacheItemPoolInterface $pool, ActivityLogger $al, CsrfService $csrf, InputValidator $iv, EventDispatcherInterface $dispatcher): GeneralEndpoints => new GeneralEndpoints($conn, $auth, $catR, $com, $cfg, $cookie, $d, $hA, $h, $iA, $i, $perm, $pic, $rate, $sR, $s, $tR, $ug, $us, $uA, $u, $ws, $pool, $al, $csrf, $iv, $dispatcher)),
    TagsEndpoints::class             => factory(static fn (Connection $conn, CategoryService $cat, HtmlService $h, ImageRepository $i, TagAdminService $tA, TagRepository $tR, TagService $tag, UrlService $u, ActivityLogger $al, CsrfService $csrf, WsHelper $ws, EventDispatcherInterface $d): TagsEndpoints => new TagsEndpoints($conn, $cat, $h, $i, $tA, $tR, $tag, $u, $al, $csrf, $ws, $d)),
    CommentsEndpoints::class         => factory(static fn (Connection $conn, CommentService $com, DateService $d, UrlGenerator $ug, CsrfService $csrf, EventDispatcherInterface $dispatcher): CommentsEndpoints => new CommentsEndpoints($conn, $com, $d, $ug, $csrf, $dispatcher)),
    ...
    CategoryDefaultRenderer::class   => factory(static fn (Connection $conn, CategoryService $cat, HtmlService $h, ImageRepository $i, SessionService $sess, StringUtil $s, UrlService $u, DebugCollector $dbg, EventDispatcherInterface $dispatcher): CategoryDefaultRenderer => new CategoryDefaultRenderer($conn, $cat, $h, $i, $sess, $s, $u, $dbg, $dispatcher)),
    ...
    NoPhotoYetRenderer::class        => factory(static fn (ConfigService $cfg, ImageRepository $i, StringUtil $s, UrlGenerator $ug, RedirectResponder $redirect, UrlService $us, PermissionService $perm, EventDispatcherInterface $dispatcher): NoPhotoYetRenderer => new NoPhotoYetRenderer($cfg, $i, $s, $ug, $redirect, $us, $perm, $dispatcher)),
