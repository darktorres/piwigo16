...
**Remaining 2 widgets (not started)** — `selectize` (12 views; must
preserve the real, live `$this->colorscheme`-driven light/dark file
swap, ideally via `light-dark()` instead of two parallel files;
tokens.css companion diff and 1 fabricated rule need fixing), `jgrowl`
(1 view; reconcile against `theme.css`'s own ~70 existing lines of
first-party overrides before landing — one proposed rule would lose a
specificity fight the current override wins). After all 8 land, wrap
