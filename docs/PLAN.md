...
**P51-N (scoped, not started)** — eliminate the inline `onclick=`/
`javascript:`-attribute `window.X = X` coupling P51-H's own
`build/ambient-globals.d.ts` audit item found and confirmed real but
didn't itself remove. Found live during P51-G's own planning (a
`window.X = Y` cross-check against the snake_case rename set): **15
real sites across 6 files** are `window.X = X` assignments whose only
real reader is a Latte template's own inline `onclick=`/`javascript:`
HTML attribute, not any JS `import` — `scripts.ts` (`popuphelp`,
`pwg_tryFocus`), `updates_ext.ts` (`ignoreAll`/`resetIgnored`/
`updateExtension`/`ignoreExtension`), `batchManagerGlobal.ts`
(`selectGenerateDerivAll`/`selectGenerateDerivNone`/`selectDelDerivAll`/
`selectDelDerivNone`), `footer.ts` (`hide_user_whats_new`/
`show_user_whats_new`), `user_list.ts` (`plugin_add_tab_in_user_modal`),
`group_list.ts` (`hideAddGroupForm`/`updateSelectionPanel`). Real ES
modules have no implicit global scope, so `window.X = X` is currently
the *only* bridge from an inline HTML attribute to a module-scoped
function -- confirmed necessary as long as the attribute stays inline,
not an oversight P48/P51-H merely hadn't gotten to yet.

The actual fix is a template edit, not a JS-only one: for each site,
rewrite the corresponding `.latte` template to drop the inline
`onclick=`/`javascript:` attribute (replacing it with a real CSS
class/data-attribute the script can select) and have the script bind
the handler itself via this codebase's own already-dominant `on()`/
`addEventListener` convention (the same convention P51-H's own "2
remaining DOM0-style `document.onkeydown =` handlers" item already
moves those 2 sites onto) -- then delete the now-dead `window.X = X`
line and its corresponding `build/ambient-globals.d.ts` declaration for
real, closing out what P51-H's own audit item could only confirm, not
remove. Genuinely cross-cutting (PHP template + JS per site, not a
mechanical TS-only conversion like the rest of P51), so distinct from
P51-A through M's own JS/TS-only scope -- sequencing relative to them
is open, not fixed by this entry. Needs Visual Regression / the real
