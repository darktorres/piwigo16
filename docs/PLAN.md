...
| P51 | TS modernization | In progress — P51-A through A5 done (hardened ESLint ruleset + tsconfig + 3 further "how can we harden" rounds); P51-B through L scoped, not started; P51-M (third-party plugin exploration) measured and deferred until P51-A–L land | 17 |
...
a real `AjaxThenable` (`await ajax(...)` already works everywhere); this
is purely converting callers. Recounted fresh against current source (P51-A5
touched `ajax.ts`/`tags.ts`/`user_list.ts`/`albums.ts`/`group_list.ts`/
`plugins_installated.ts`/`cat_modify.ts` today, so this needed a real
recount, not the plan's original estimate): **118 real `ajax(`/`ajax<T>(`
call sites, 5 already `await`/`.then()`-based, 113 still on the old
callback-options shape**, concentrated in `user_list.ts` (20),
`group_list.ts` (11), `cat_modify.ts` (9), `plugins_installated.ts` (7),
`tags.ts`/`albums.ts`/`album_selector.ts` (6 each), `photos_add_direct.ts`
(5), and ~22 more files with 1-4 sites each. Convert each to
`try { const data = await ajax<T>(...); ... } catch (e) { ... }`,
narrowing the caught error to `AjaxError` where the handler reads
`e.responseText`/`e.status`; an `error` handler that chains UI updates
through `complete` regardless of success/failure becomes a `finally`
block, not just a `catch`. **Real, already-verified exception carved out
by P51-A5's own investigation**: `cat_modify.ts`'s delete-album confirm,
`group_list.ts`'s single-group-delete confirm, `plugins_installated.ts`'s
`deletePlugin`, and `tags.ts`'s `removeTag`/mergeTags confirms all have a
`content: function () { return ajax({...}) }` callback that must keep
returning `ajax()`'s own raw `AjaxThenable` object, unconverted --
jconfirm.ts's own `isThenable()` check needs its real `.always()` method
to drive the loading-spinner-then-`setContent()` UX, which an
`await`/`.then()` conversion would break the same way `async` would (see
P51-A5's own `eslint-disable-next-line` comments at each of those 5
sites). Once every other real call site converts, narrow
`AjaxOptions`/`ajax()` itself: drop `success`/`error`/`complete` from the
options interface (dead once no caller uses them), keeping only
`beforeSend`/`url`/`method`/`data`/`dataType`/`contentType`/`headers`/
`timeout`. Same pass, same call sites: add a real `json?: unknown` option
to `AjaxOptions` that stringifies and sets `contentType` once inside
`ajax()` itself, and convert the ~70 call sites across ~21 files that
currently hand-pair `contentType: "application/json"` with
`data: JSON.stringify({...})` to the shorthand (`json: {...}`).

**P51-D (scoped, not started)** — frontend id-typing boundary. Add one
typed accessor to `vendor/dom.ts` (alongside the existing `attrOf`/`attr`
helpers) that reads a DOM attribute/dataset/select value expected to be a
row id and returns a real `number`, single parse point. Replace the ~85
scattered `Number()`/`parseInt()`/`String()` coercions around id-named
variables across `album_selector.ts`, `group_list.ts`, `tags.ts`,
`history.ts`, `user_activity.ts`, `albums.ts`, `jqtree.ts`,
`photos_add_direct.ts`, `rating.ts`, and others found via the original
close read. Change function signatures currently typed `string | number`
for an id (`album_selector.ts`'s `#catIdParam`/`#prefill_search_subcats`
being the concrete example) to plain `number`, parsed once at the real
DOM boundary. Should make `eqeqeq` require zero suppressions for id
comparisons, confirming the type fix is real.

**P51-E (scoped, not started)** — real hard-private class fields.
Recounted, unchanged from the original scope: **97 TS-keyword
`private`/`protected` member declarations across 6 files** --
`vendor/ajax.ts` (1), `vendor/dom.ts` (9), `vendor/uploadQueue.ts` (17),
`vendor/ajaxQueue.ts` (4), `vendor/lineChart.ts` (29),
`vendor/jqtree.ts` (37) -- converting each to real `#field` syntax,
matching the convention the other 64 `.ts` files already use. Read each
class before converting: a `protected` member meant for a real subclass
to override needs checking whether any real subclass exists in this
codebase at all (`#field` has no subclass-access story -- if a real
subclass genuinely needs it, that member stays `protected`, the one
legitimate reason not to convert); a `private` member read from a unit
test via a workaround (casting to `any`/reflection) needs that test
fixed to go through the class's real public API instead.
