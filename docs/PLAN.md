...
**Remaining 5 widgets (not started)** — `selectize` (12 views; must
...
read), `jgrowl` (1 view; reconcile against `theme.css`'s own ~70
existing lines of first-party overrides before landing — one proposed
rule would lose a specificity fight the current override wins),
`colorbox` (1 view but 14 consuming pages via `new ColorboxView()`;
re-derive the redesigned nav-button offsets for the real new icon
size, preserve the `id: 'jquery.colorbox'` dedup key), `jquery-ui`+timepicker
(7 views; add the missed `SearchFiltersView.php` site, fix the
`--color-text-primary`/`--color-danger` token mismatches and the
`.ui-corner-all` → 4px not `--radius-sm`/3px mapping). After all 8 land, wrap
