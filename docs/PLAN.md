...
**Remaining 1 widget (not started)** — `jgrowl` (1 view; reconcile
against `theme.css`'s own ~70 existing lines of first-party overrides
before landing — one proposed rule would lose a specificity fight the
current override wins). After it lands, wrap
`admin/default`'s remaining CSS in `@layer theme-chain` (safe with
zero vendor-override mitigation needed at that point), then the rest
