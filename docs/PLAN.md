...
**Remaining 4 widgets (not started)** — `selectize` (12 views; must
...
specificity fight the current override wins), `jquery-ui`+timepicker
(7 views; add the missed `SearchFiltersView.php` site, fix the
`--color-text-primary`/`--color-danger` token mismatches and the
`.ui-corner-all` → 4px not `--radius-sm`/3px mapping; its own binary
assets already vendored, see above). After all 8 land, wrap
