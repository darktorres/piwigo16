...
| P52 | CSS architecture modernization | In progress — Tailwind call resolved (not adopted); full design scoped below (P52-A through J); P52-A done (`.browserslistrc` floor bump), P52-B done (tokens + reset + base layers, theme-independent — see its own entry below), P52-C partially done (font consolidation, `:focus-visible` audit, `@container`/`@media` conversion, jqtree-RTL resolution all done; the full ID→class detox + selector-modernization sweep re-scoped out as its own not-yet-started follow-up once its real cross-Latte-template footprint became clear — see its own entry below), P52-D through J not started | 1 |
...
`#ui-datepicker-div`/jqtree's selectors and any genuine JS-hook IDs), a
...
**P52-C follow-up (not started)** — the full ID→class detox +
selector-modernization sweep: lowercase HTML tag selectors,
physical→logical properties (including jqtree's own, deferred from
item 5 above), float/`display:table`→flex/grid, `:is()`/`:where()`,
native nesting, vendor-prefix cleanup, and migrating all 45 `theme.css`/
`print.css`/`css/{pages,components}` files into the `components`/
`pages`/`utilities` layers — plus, newly discovered as this entry's
own real scope, adding a markup class in every `.latte` template
that has one of the ~489 styled IDs with no class of its own to reuse.
Needs its own real count of that template-side footprint (a script-
based audit, not a guess) before it can be sized the way this entry
was.
