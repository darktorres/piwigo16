...
**P52-C follow-up, second batch (not started)** — the 144 IDs needing
a genuinely new class, each in its own real `.latte` template. Real
per-template distribution from the pre-workflow audit (14 in
`user_list.latte`, 10 in `batch_manager_filter.inc.latte`, 9 in
`layout.latte`, tapering to 1 in most of the rest) undercounts the
true total now that the workflow found dozens more "the only existing
class is generic/reused" cases than the manual audit did — needs a
fresh per-file tally at execution time. The workflow's own per-ID
evidence (which template, which line, why any existing class is
unsafe) is the starting checklist for whoever executes this, not
something to re-derive from scratch. Folding in the rest of "item 6"
(lowercase HTML tag selectors, physical→logical properties including
jqtree's own, float/`display:table`→flex/grid, `:is()`/`:where()`,
native nesting, vendor-prefix cleanup, and layer-wrapping all 45 CSS
files) into the same per-file pass avoids a second full pass over the
same files, but is real, separate, substantial work of its own.
