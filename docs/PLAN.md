...
| P51 | TS modernization | In progress — P51-A + P51-A2 + P51-A3 done; P51-M (third-party plugin exploration) measured and deferred until P51-A–L land | 17 |
...
Also since P51-A3: two more built-in `@typescript-eslint` hardening
...
not masking an Annex-B hazard). Then an "add all of them" pass over
every remaining unconfigured `@typescript-eslint/*` rule with a real
violation count worth having: 5 zero-violation rules enabled as pure
forward-guards (`no-unused-private-class-members`,
`consistent-type-assertions`, `array-type`, `class-literal-property-style`,
`no-restricted-imports` guarding against jQuery ever coming back post-P49),
plus `consistent-generic-constructors`/`consistent-indexed-object-style`
(1 site each), `no-inferrable-types` (10/7 files), `dot-notation` (11/5
...
Remaining sub-phases: **P51-B** a `build/collectScriptEntries.ts` scanner
so `vite.config.ts`/`knip.config.ts` derive their entry lists from the
real PHP `AssetContribution::script()`/`::css()` registrations instead of
a hand-maintained list; **P51-C** convert `ajax()` call sites to
async/await plus a `json:` options shorthand; **P51-D** a typed id
accessor at the DOM boundary, closing the frontend `string | number` id
churn; **P51-E** real `#field` hard-private class members (6 files, 97
members) replacing the TS-keyword `private`/`protected` form;
**P51-F** `LocalStorageCache.ts`'s prototype-based "classes" to real ES6
`class`/`extends`; **P51-G** rename 628 snake_case identifiers across 41
files to camelCase; **P51-H** dead code/unsafe-cast/animation-engine
(`setInterval` → `requestAnimationFrame`)/global-coordination cleanup;
**P51-I** file-level reorganization (split/merge/rename/move, directory
regrouping, filename-casing normalization); **P51-J** backend
`Comment`-family id-typing (4 real targets); **P51-K** backend
`CategoryRepository`'s remaining 13 raw-int methods; **P51-L** a closing
re-read of everything P51-A–K touched plus the full-suite gate.
