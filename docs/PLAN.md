...
**Remaining 7 widgets (not started)** — `selectize` (12 views; must
...
size, preserve the `id: 'jquery.colorbox'` dedup key), `jcrop` (1
view; golden-html fixture for `admin-picture-coi` hardcodes the old
CDN link, expected to change), `jquery-ui`+timepicker (7 views; add
the missed `SearchFiltersView.php` site, fix the `--color-text-
primary`/`--color-danger` token mismatches and the `.ui-corner-all`
→ 4px not `--radius-sm`/3px mapping). After all 8 land, wrap
