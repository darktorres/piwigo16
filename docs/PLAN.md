...
**Update**: the layer-wrapping portion is now Done for the admin theme
(P52-D + "the `@layer` wrap itself" entries below) — a handful of
lowercase-selector/logical-property fixes landed opportunistically
along the way wherever a specific bug fix already required touching
that exact declaration (e.g. `ul.jqtree_common`'s `margin-inline`), but
the systematic sweep across every remaining file is still not started.
`default`/`standard_pages` haven't had this specific item 6 sweep
either — their own `@layer` wrap is done (see P52-E/F), this
lowercase/logical-property/vendor-prefix sweep is a separate, still-
open item for both chains.
