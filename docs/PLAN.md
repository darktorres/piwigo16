...
**Remaining 3 widgets (not started)** — `selectize` (12 views; must
preserve the real, live `$this->colorscheme`-driven light/dark file
swap, ideally via `light-dark()` instead of two parallel files;
tokens.css companion diff and 1 fabricated rule need fixing),
`plupload` (3 views; audit `roma`/`clear`'s own overrides from the
start, not admin/default alone — both already have independent,
working, `!important`-protected rules the original scoping never
read; its own binary assets already vendored, see above), `jgrowl` (1
view; reconcile against `theme.css`'s own ~70 existing lines of
