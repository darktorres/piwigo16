...
## Backlog — Deferred / partial work from #1–#22

Tasks pulled out of phases otherwise marked Done. Grouped by source phase. Most cluster on three downstream tracks: #23 (Latte) unblocks #18 step 6 and #19 Latte partials; #26 (plugin/theme) unblocks #21 plugin migration and #18 templates; the rest are small follow-ups.

### From #4 — Composer audit + Renovate

- [ ] Port dev-dep auto-merge workflow from the original Renovate spec. Dependabot ships no built-in auto-merge; add a small workflow only if dependency churn warrants it.

### From #5 — Config schema + `.env`

- [ ] Add `'required' => true` SCHEMA field + `MissingRequiredConfigException`; validate in `ConfigLoader`.
- [ ] Add `'description'` SCHEMA field; populate `docs/CONFIG-REFERENCE.md` with descriptions.
- [ ] Add `'sensitive'` SCHEMA field; implement `Config::dumpForLog(): array` (sensitive-masked) for logging.
- [ ] Add namespace-prefix support to `ConfigStorage` for plugin keys.
- [ ] Add `--target=<path>` flag to `tools/build-config-accessors.php` so per-plugin Configs can be regenerated by the same tool.
...
- [ ] Resolve the 6 in-code `DEFERRED` markers: `Updates.php` plugin-era redirect target; search cat-id access gap; admin URL in gallery filter; category position persistence; stub `cache_size`; image_id/filename precedence.
- [ ] Address the 2 `PERF` notes: redundant `get_available_tags()` call; all-rows-before-PHP-count in WS history.

### From #11 — PSR-3 Logger

- [ ] Swap Piwigo's logger backend to Monolog. Interface is the contract today; deferred until demand surfaces.
...
- [ ] Wire S3 and/or SFTP adapters in `config/storage.php` when a deployment needs them (currently local-only; adapters are a `composer require` + closure swap).

### From #18 — i18n

- [ ] Wire the Smarty `{translate}` / Latte `{|translate}` filter (Step 6). Blocked on #23 (Latte) and #26 (plugin/theme compat).

### From #19 — `functions_*` → typed services

- [ ] Convert the pure rendering includes (page_header, page_tail, picture_comment, …) — already migrated to typed PHP renderers — to Latte partials under #23 Wave 0.
- [ ] Populate the five page-context DTOs (`AlbumPageContext`, `PicturePageContext`, `SearchPageContext`, `TagsPageContext`, `AdminPageContext`) from controllers as each `.latte` partial is written.

### From #20 — Symfony Messenger

- [ ] Package supervisor/systemd worker config alongside the documented `bin/piwigo messenger:consume async` command (currently ops-by-doc only).

### From #21 — OpenAPI spec

- [ ] Teach `SpecBuilder` to read the `#[ApiMethod]` attribute for per-method `summary` / `responseClass` / `tags` enrichment. Requires WS handlers to be reflection-accessible controller classes.
- [ ] Add a CI gate that validates the generated spec on every run. Options: PHPUnit structural test (no new dep), `cebe/php-openapi` dev dep, or external `redocly lint` / `openapi-spec-validator`.
- [ ] Migrate plugins still calling the removed `PwgServer::addMethod()` to `register(MethodDefinition $def)`.
...
- `.github/workflows/ci.yml` runs four jobs on push/PR: `style` (Pint `--test`), `phpstan` (`composer dump-autoload --strict-psr` + `vendor/bin/phpstan analyse`), `unit` (`vendor/bin/phpunit --testsuite Unit`), and `audit` (composer audit + `npm audit`). Direct actions pinned to current majors (`actions/checkout@v6`, `shivammathur/setup-php@v2`, `ramsey/composer-install@v4`, `actions/setup-node@v6`); all run on Node 24. Infection (#28) and TS/CSS jobs land here later.
...
Every file under `src/` declares `strict_types=1`. A PHPStan rule fails CI if a new file is added without it. (`include/` and `admin/` are fully deleted.)
...
A PR introducing a vulnerable dep is blocked. Dependabot opens grouped weekly PRs.
...
- `Piwigo\Config\Config::SCHEMA` is the single source of truth for all 287
...
- `Piwigo\Config\ConfigLoader` runs at every entry point's boot:
  `applyDefaults()` walks SCHEMA, then `loadEnv($repoRoot)` reads
  `.env` (only — `.env.local` is opt-in for the test runner via
  `tests/bootstrap.php`), then `applyEnvOverrides()` applies the
  curated `ENV_MAPPING` (5 DB vars: HOST/USER/PASSWORD/BASE/PREFIX).
  (The `$conf` parameter was removed from both methods in the bridge-removal pass.)
...
- PHPUnit tests pass (378+ at time of writing; count has grown since #5 shipped)
...
PIWIGO_DB_HOST=remote.example.com php -r "
  ...
  Piwigo\Config\ConfigLoader::loadEnv('.');
...
php -r "
  require 'vendor/autoload.php';
  var_dump(\Piwigo\Plugins\LocalFilesEditor\Config::tabs());
"
```
...
```bash
grep -rn "TODO\|FIXME" src/ --include="*.php" | grep -v "vendor\|install/db"
...
5. **`unserialize` (~15 sites).** All `@unserialize(...)` calls now go
   through `safe_unserialize()` (introduced in `include/functions.inc.php`,
   subsequently migrated to `StringUtil::safeUnserialize()` and deleted
...
- `pwg_safe_getimagesize()`, `pwg_safe_exif_read_data()` — introduced in `include/functions.inc.php`; subsequently migrated to `StringUtil::pwgSafeGetimagesize()` / `StringUtil::pwgSafeExifReadData()` and deleted with the include file.
...
vendor/bin/phpstan analyse           # green at level 10 (NoErrorSuppressionRule active)
vendor/bin/phpunit                   # 378+ tests, all green
```
...
- **7 generic `throw new \Exception(...)`** sites: `include/dblayer/functions_mysqli.inc.php` (2), `admin/include/functions_install.inc.php` (1), `src/Piwigo/Admin/Image/PwgImage.php` (4).
...
grep -rnE 'throw new \\?Exception\(' src/ \
  --include='*.php' | wc -l                                       # 0
vendor/bin/phpstan analyse                                        # green
```
...
- Logger class lives at `src/Piwigo/Core/Logger.php` (the original plan said `src/Piwigo/Log/Logger.php`, but the move went to `Core/`). `include/Logger.class.php` was one of the empty stubs — deleted in #31.
...
- Callers under `src/Piwigo/Admin/{Languages,Plugins,Themes}.php` and `src/Piwigo/Admin/Image/ImageExtImagick.php` used `global $logger` (now resolved — all globals removed per #6).

### Steps

1. **Add `psr/log`** to `composer.json` (`require: { "psr/log": "^3.0" }`). `composer update`.

2. **Implement `LoggerInterface`.** `Piwigo\Core\Logger` extends `Psr\Log\AbstractLogger` (provides leveled methods routed to a single `log()`). Implement `log(string $level, string|\Stringable $message, array $context = []): void`.
...
```bash
grep -rn '->add(' src/ --include='*.php' | grep -i logger   # empty
...
4. **Define services.** In `config/container.php`:
   ...
   ```php
   return [
       Piwigo\Config\Config::class    => DI\autowire(),
       Piwigo\Core\PageState::class   => DI\autowire(),
       Piwigo\Core\Logger::class      => DI\factory(/* file/db backend factory */),
       Psr\Log\LoggerInterface::class => DI\get(Piwigo\Core\Logger::class),
       // …
   ];
   ```
...
```php
$config = $container->get(Piwigo\Config\Config::class);
$this->assertInstanceOf(Piwigo\Config\Config::class, $config);
```
...
- `src/` had 68 classes/interfaces at the start of this item; `include/` free functions were out of scope (covered by item #19, now deleted).
- 3 `readonly` declarations existed in `src/` at the start; 0 enums, no `match` adoption beyond a few isolated uses.
- `include/ws_core.inc.php` had 10 `define()` bitmask constants — enum candidates (linked to #8). File subsequently deleted; constants migrated to `WsType`/`WsParam`.
...
- **`include/dblayer/functions_mysqli.inc.php` deleted** (`b2429e1c5`) — after migration, all remaining helpers (`mass_updates`, `mass_inserts`, `single_update`, `single_insert`, `protect_column_name`, `get_dbal_connection`) were inlined into `Piwigo\Db\Dml` and `Piwigo\Db\DbConnection`; the constants migrated to `Piwigo\Db\DbConstants`.
...
grep -rc 'pwg_query\|pwg_db_\|query2array\|get_enums\|do_maintenance_all_tables' \
  src/ --include='*.php' | awk -F: '{s+=$2} END {print s}'

vendor/bin/phpstan analyse   # 0 errors at level 10
```
...
- **Template syntax (step 6):** Smarty `{translate}` / Latte `{|translate}` filter wiring depends on #23 (Latte migration) and #26 (plugin/theme compat). `l10n()` as a free function is deleted; templates use the Smarty `translate` modifier (backed by `Translator`) or `Lang::t()` in PHP code.
...
**Status:** ✅ Done — 44 service classes, ~525 functions migrated; all delegate shell files deleted &nbsp;|&nbsp; **Size:** XL

### Goal

Move all 366 free functions across the 19 `functions_*.inc.php` modules into typed, namespaced classes under `src/Piwigo/<domain>/`. Each migrated function becomes a static or instance method on a domain class. Free-function wrappers were kept as one-line delegates during the transition, then deleted along with the source files once all callers were updated.
...
the free function becomes a one-line ServiceLocator delegate. PHPStan level 10
...
A small number of **legitimate** pre-boot standalones were identified as
requiring special handling — functions called before any boot was possible
(e.g. from `fatal_error`, `i.php` fast-path, or `common.inc.php` itself):
`mkgetdir`, `script_basename`, `load_conf_from_db`, `get_languages`,
`convert_charset`, `cookie_path`, `generate_key`, `fatal_error`,
`get_root_url`, `l10n`, `redirect_html`, `get_branch_from_version`.
All were subsequently migrated to typed static methods and deleted with
`include/functions.inc.php` (`be344673b`).
...
| `functions_plugins.inc.php`                            | 458   | 10+2  | ✅ done                  | `Piwigo\Plugin\PluginService`; factory helpers inlined with `NoDynamicNew` suppression   |
...
| `admin/include/functions_upgrade.php`                  | —     | 9     | ✅ deleted               | Upgrade-flow helpers migrated to `UpgradeController` / `UpgradeService`                  |
| `admin/include/functions_install.inc.php`              | —     | 4     | ✅ deleted               | Install-flow helpers migrated to `InstallController`                                     |
| `include/derivative_params.inc.php`                    | —     | 5     | ✅ deleted (`378fa5e8e`) | Migrated to `Piwigo\Image\DerivativeEncoding`                                            |
...
| File                                                                         | Becomes                                                                       | Notes                                                                     |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `include/section_init.inc.php`                                               | `Piwigo\Section\SectionInitializer::initialize()`                             | ✅ done — #22 phase 6c                                                    |
| `include/user.inc.php`                                                       | `Piwigo\Users\UserBootstrap`                                                  | ✅ done — #22 phase 6c                                                    |
| `include/filter.inc.php`                                                     | `Piwigo\Filter\FilterResolver`                                                | ✅ done — `FilterMiddleware` in middleware pipeline                       |
| `include/ws_core.inc.php`                                                    | `Piwigo\Ws\WsType` / `Piwigo\Ws\WsParam`                                      | ✅ deleted — constants migrated; file fully removed after #8              |
...
| `include/constants.php`                                                      | `Piwigo\Core\Config::dbPrefix()`                                              | ✅ deleted (`9642e31f4`) — constants migrated to `Config` typed accessors |

Pure rendering includes (`include/page_header.php`, `include/page_tail.php`, `include/picture_comment.inc.php`, etc.) were migrated to typed PHP renderer classes (`PageHeaderRenderer`, `PageTailRenderer`, `PictureCommentRenderer`, etc.) and deleted. The Latte partial conversion step remains pending under item #23 Wave 0.
...
| `include/ws_protocols/xmlrpc_encoder.php:xmlrpc_encode`                           | 1         | ✅ deleted — migrated to `PwgXmlRpcEncoder` in `src/Piwigo/Ws/Protocol/`                                         |
...
```bash
# include/ fully deleted — all modules migrated
...
```bash
# include/ fully deleted — zero free functions remain outside src/
ls include/ 2>/dev/null || echo "include/ deleted"
```

### Final step — delete the delegate shells

Functions from the separate `functions_*.inc.php` shells were inlined directly
into `functions.inc.php` (at the time, loaded for every request via
`common.inc.php`). This eliminated the separate files without changing any
call-site. `functions.inc.php` and `common.inc.php` were subsequently deleted
once all callers were migrated to typed service calls. The WS endpoint classes were also upgraded to
...
| `include/ws_functions.inc.php`           | ✅ deleted               | `ws_isInvokeAllowed` moved to `WsController`; 7 other delegates were dead code — endpoint classes now call `WsHelper` directly            |
| `include/functions_cookie.inc.php`       | ✅ deleted               | Migrated to `CookieService`; `i.php` refactored away                                                                                      |
| `include/functions_url.inc.php`          | ✅ deleted               | Pre-boot standalones migrated to `UrlService`; `i.php` deleted                                                                            |
| `include/functions_search.inc.php`       | ✅ deleted               | `QSearchScope` etc. migrated to `src/Piwigo/Search/`                                                                                      |
| `include/functions_filter.inc.php`       | ✅ deleted               | Migrated to `FilterMiddleware` / `FilterService`                                                                                          |
| `include/functions_calendar.inc.php`     | ✅ deleted               | Migrated to `CalendarService`; `SectionInitializer` uses typed class                                                                      |
| `include/functions_plugins.inc.php`      | ✅ deleted               | Event system → `EventDispatcher`/`LoadedPluginRegistry`; constants migrated                                                               |
| `include/functions_user.inc.php`         | ✅ deleted               | Migrated to `AuthService`/`UserService`/`PermissionService`/`PreferencesService`                                                          |
| `include/functions.inc.php`              | ✅ deleted (`be344673b`) | All pre-boot standalones migrated to typed static methods on `Util`, `UrlService`, `LangService`, `CommonBootstrap`, `ConfigService` etc. |
| `admin/include/functions.php` etc.       | ✅ deleted               | All admin helpers migrated to typed service classes; `admin/` directory fully removed                                                     |
...
Manual polling loop (no pcntl, cross-platform). Calls `ImageStdParams::loadFromDb()` before entering the loop so derivative generation has the gallery's size configuration. Dispatches each fetched envelope back through the bus with `ReceivedStamp`.

#### Admin queue UI (`BatchManagerController` queue page + `queue.tpl`)
...
All other sizes remain lazily generated by `ImageDerivativeController` (via the `index.php` fast-path) on first browser request.

#### Job dispatch call sites

| Location                                     | Job dispatched                | Notes                                                                              |
| -------------------------------------------- | ----------------------------- | ---------------------------------------------------------------------------------- |
| `MaintenanceController` (derivatives action) | `RegenerateAllDerivativesJob` | After synchronous cache clear; handler fans out per-image `GenerateDerivativeJob`s |
...
On shared hosting without persistent processes, the queue simply stays idle and `ImageDerivativeController` handles derivatives lazily on first request as always.
...
An OpenAPI 3.1 specification describes every method registered with `PwgServer::register()`. The spec is auto-generated from the registration metadata — no hand-maintained YAML. Served at `/ws/openapi.json`; Swagger UI at `/ws/docs` (via `WsController`). With the spec in place, client SDKs (TypeScript, Python, etc.) can be generated via `openapi-generator-cli`.
...
#### Routes (`WsController` / `ws.php` path)

Interception block in `WsController` runs **before** `PwgServerRegistry::current()->run()`:
...
- `http://localhost/piwigo16/index.php/ws/openapi.json`
- `http://localhost/piwigo16/index.php/ws/docs`

Fallback (no rewrite needed):

- `http://localhost/piwigo16/index.php?/ws?_openapi=json`
- `http://localhost/piwigo16/index.php?/ws?_openapi=ui`
...
`SpecBuilder` does not yet read the attribute — pending a future pass once WS handlers are proper controller classes with reflection-accessible attributes.
...
- `tests/Unit/Ws/PwgServerTest.php` — 12 tests covering `register()`, `hasMethod()`, `getMethodDescription()`, `getMethodSignature()`, `getMethodDefs()`, `optionalFlag()` semantics.
...
```bash
# Spec served via WsController (PATH_INFO approach):
curl -s 'http://localhost/piwigo16/index.php/ws/openapi.json' | php -r "echo json_decode(file_get_contents('php://stdin'))->info->title;"
# → Piwigo Web Services

# Fallback (works anywhere):
curl -s 'http://localhost/piwigo16/index.php?/ws?_openapi=json' | php -r "echo json_decode(file_get_contents('php://stdin'))->info->title;"

# SDK generation:
openapi-generator-cli generate -i 'index.php?/ws?_openapi=json' -g typescript-axios -o sdk/ts
```
...
`config/routes.php` returns a named `RouteCollection` with 32 routes (22 original + `about`, `nbm`, `popuphelp`, `gallery_search_paged` added in #22; `qsearch`, `action`, `upgrade_feed`, `install`, `upgrade` added when standalone shims were merged into routing). Route names are the keys for `Router::generate()`. Sub-token parsing (pagination, chronology, slugs) is done by `SectionInitializer` inside each controller — the router only identifies the section via a `{rest}` catch-all.
...
All root entry-point files converted. `index.php` now runs the PSR-15 pipeline. Each legacy file was a 7-line shim (`define + require common.inc.php + Kernel::boot() + call controller`) during the Wave A transition — these shims were subsequently deleted. No `ResponseEmitter` in shims — controllers that render HTML/XML output directly avoid the PHP 8 `http_response_code()`-after-`header()` warning.

Helper functions that were defined inside entry-point files were first extracted to `include/` as an intermediate step, then subsequently migrated to typed classes and deleted along with `include/` itself:

| Include file (deleted)           | Extracted from | Final home                               |
| -------------------------------- | -------------- | ---------------------------------------- |
| `include/ws_default_methods.php` | `ws.php`       | `WsMethodRegistrar`                      |
| `include/feed_functions.php`     | `feed.php`     | `src/Piwigo/Feed/`                       |
| `include/password_functions.php` | `password.php` | `PasswordController` / `PasswordService` |
| `include/profile_functions.php`  | `profile.php`  | `ProfileController` / `ProfileService`   |
| `include/picture_functions.php`  | `picture.php`  | `PictureController`                      |

| Old file             | Controller                  | Notes                                                                 |
| -------------------- | --------------------------- | --------------------------------------------------------------------- |
| `index.php`          | `GalleryController`         | Section routing via `SectionInitializer` (was `section_init.inc.php`) |
...
All three deferred controllers used shims with their own bootstrap sequences. They have since been merged into `index.php` routing or deleted (see ROUTING-MIGRATION-PLAN.md phases 7–10).

Wave A is **complete** — all 15 root entry-point files were converted to controllers; the shim files were subsequently deleted (see ROUTING-MIGRATION-PLAN.md phases 7–10).
...
All 63 admin page-body files are **deleted** — logic inlined into private class methods of each sub-controller. No `admin/*.php` page files are `require()`d from within the controller classes; the dynamic-filename dispatch is completely eliminated. `admin/` directory fully deleted — `site_reader_local.php` migrated to `src/Piwigo/Site/LocalSiteReader.php`; all `admin/include/*.php` helpers migrated to typed service classes.
...
| Route-based         | `admin(string $section)`, `ws(array $params)`                                                                                                 | Uses `Router::generate('admin'/'ws')` + appends query params |

`applyUrlMode()` currently appends `index.php?/` unconditionally (the question-mark form). Clean URLs and `index.php/` PATH_INFO form remain possible via web-server rewrite but are not auto-selected by `UrlGenerator`.
...
**All root shims subsequently deleted** — `i.php` merged into `index.php` fast-path (`9cb3d6087`); `install.php`/`upgrade.php` merged into route detection (`1907ab38f`); `action.php`/`qsearch.php`/`upgrade_feed.php` merged into routing (`e70eaaa38`). `migrations.php` remains as a standalone CLI entry point.
...
curl -s 'http://localhost:8765/?/ws?_openapi=json' | php -r "echo json_decode(file_get_contents('php://stdin'))->info->title;"
# → Piwigo Web Services

vendor/bin/phpunit --no-progress    # 378+ tests, all green
...
- **135 `.tpl` files** (reduced from 169 — plugin templates deleted when plugin directories were cleaned): `themes/admin/_base/template/` ~60, `themes/_base/template/` ~50, `themes/standard_pages/` 3, plus a handful in admin skin variants. Zero `.latte` files yet.
...
4. **Port Smarty extensions to Latte equivalents.** Map each registered Smarty plugin to a Latte filter/function/extension:
   ...
   - **`translate`/`translate_dec`** — Latte filter `|translate` backed by `Piwigo\Lang\Translator` (created in item #18).
...
5. **Convert templates in waves.** Order risk-low → risk-high:
   - **Wave 0 — extract layout partials from `include/`.** ✅ Partially done: `PageHeaderRenderer`, `PageTailRenderer`, `PictureCommentRenderer`, `PictureMetadataRenderer`, `PictureRateRenderer`, `NoPhotoYetRenderer`, `SearchFilterRenderer`, `SelectedTagsRenderer`, `CategoryCatsRenderer`, `CategoryDefaultRenderer` now exist as typed PHP services. The `include/` PHP files are deleted. The `.latte` partial conversion (replacing Smarty `.tpl` with typed-context Latte files) is still pending.
   - **Wave 1 — admin templates** (lowest risk, ~55 files in `themes/admin/_base/template/`). Each `.tpl` → `.latte`. Smarty syntax → Latte syntax. Run the page in the browser after each conversion.
...
# baseline: 135 today (reduced from 169); target: 0 (all converted to .latte)
...
**Status:** Partially started — step 4 (CSRF middleware) done; steps 1–3, 5–6 not started &nbsp;|&nbsp; **Size:** M
...
- **CSRF tokens (`pwg_token`)**: centralized in `CsrfMiddleware` (step 4 done); per-controller ad-hoc checks remain as belt-and-suspenders but middleware is the gate.
...
4. **CSRF middleware.** ✅ Done — `CsrfMiddleware` is active in the PSR-15 pipeline (#22 step 3). Validates `pwg_token` on POST (exempt: `/ws`, `/install`, `/upgrade`, `/identification`, `/register`). Per-controller ad-hoc checks remain but are now supplementary.
...
- `template_compile_check` is on by default — Smarty `stat`s every source on every render. `Piwigo\Config\Config::templateCompileCheck()` reads it from config; there is no CLI override and no production-time flip.
- No `tools/precompile_*` script exists today.

### Steps

1. **Add `tools/precompile_templates.php`.** Boot Piwigo (`CommonBootstrap::run()` + `Kernel::boot()`) in CLI mode without emitting output, then for each engine instance:
...
- **`include/functions_plugins.inc.php` deleted** — `add_event_handler`, `trigger_change`, `set/get_plugin_data` etc. no longer exist as free functions. `EventDispatcher` and `LoadedPluginRegistry` are the typed replacements. `$GLOBALS['pwg_event_handlers']`/`$GLOBALS['pwg_loaded_plugins']` remain reference-bridged for plugins that read the globals directly.
- **5 bundled plugins** under `plugins/`: `LocalFilesEditor`, `nbc_ThemeChanger`, `piwigo-openstreetmap`, `piwigo-videojs`, `user_tags`. Each still has a `main.inc.php` (loaded via `PluginService`) and a `maintain.inc.php` extending `PluginMaintain` / procedural hooks.
- **`src/Piwigo/Admin/PluginMaintain`** and `ThemeMaintain` are the only typed lifecycle contracts today.
- **3rd-party plugins** that called `add_event_handler()` directly will need to call `EventDispatcher::addListener()` — the free-function bridge is gone.
- **`global $conf` gone** — bundled plugins use per-plugin typed `Config` classes (item #5).
...
3. **Move side-effect code to `Theme::boot()`.** Today, `themes/standard_pages/themeconf.inc.php` runs `$this->assign(...)` and `ServiceLocator::get(ConfigService::class)->confGetParam(...)` at load. That code moves into the `boot()` method, where it receives the container and can pull `Config`, the template registry, etc. via DI. Event handlers registered from `themeconf.inc.php` move into `subscribedEvents()`.
...
php -r '
  ...
  echo $sp->getParentId() === "_base" ? "ok" : "fail";
'
...
PHPStan analyse passes at level 10 with no baseline file. Level 10 enforces fully-typed `mixed` propagation — the strictest analysis level. Was gated by globals removal (#6) and `include/` migration (#17).
...
test ! -f phpstan-baseline.neon              # no baseline file
composer dump-autoload --strict-psr          # zero warnings
```
...
4. **Port E2E tests from TypeScript to PHP.** Move each `tests/e2e/*.spec.ts` to `tests/Browser/*.php` using Pest browser syntax:
   ...
   Port all 16 tests. Keep the same logical groupings — one PHP file per existing `.spec.ts` file.
...
- **378 test methods** across `tests/Unit/` (Auth, Cache, Core, Image, Menu, Search, Session, Template, Users, Ws, Config, Lang, and more). 48 test files.
- Largest untested areas in `src/`: `Admin/` (image backends, `plugins`, `themes`, `updates`), `Calendar/`, `Db/` (repositories).
...
**Status:** ✅ Done; `class_alias` decision still pending (no concrete breakage seen) &nbsp;|&nbsp; **Size:** XS
...
- `tools/list-classes.php` was trimmed (removing `*.class.php` patterns) then subsequently deleted with other obsolete dev tools.
...
4. **Manual** — spot-check that a representative bundled plugin still loads. ✅ Done — the #5 plugin-activation pass (`bc41a5379`) exercised `nbc_ThemeChanger`, `LocalFilesEditor`, `piwigo-videojs`, `piwigo-openstreetmap` end-to-end.
