...
filenames. `dist/.vite/manifest.json` is Vite's own manifest (not used by
Piwigo's `ScriptLoader`). **Not committed to git** (`.gitignore`d — regenerated
by `npm run build`).
...
obsolete.list                    Files removed in this release
