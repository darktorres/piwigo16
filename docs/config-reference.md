...
| `mail_theme` | `string` | `clear` | `Config::mailTheme()` | — |
...
| `php_extension_in_urls` | `bool` | `true` | `Config::phpExtensionInUrls()` | — |
...
| `question_mark_in_urls` | `bool` | `true` | `Config::questionMarkInUrls()` | — |
...
loaded by `Piwigo\Config\ConfigLoader` from `.env` and `.env.local`:
...
Env values win over `local/config/database.inc.php` (12-factor precedence).
Existing installs that rely solely on `database.inc.php` keep working
unchanged when no `.env` is present.
