...
- **Actual run**: PHPStan level 8, baseline 5016 errors (walked levels 0→8 via 8 incremental baseline commits); TYPE_DECLARATION rector applied across include/ (50 files), admin/ (32 files), root entry points (6 files); strict_types added to all 185+ tracked PHP files; 6 real bugs fixed by strict_types enforcement (unserialize null, ceil→int casts, str_replace false arg, str2DateTime type widening, gc() return type, version_compare int args); Conf @phpstan-type alias committed to tools/phpstan-types.php; input_int/input_string/input_bool coercion helpers added. CI run 24975924111: lint ✓ 2m01s, e2e ✓ 1m49s, unit ✓ 14s. Baseline generated on Windows; database.inc.php + config.inc.php cross-platform include.fileNotFound handled via reportUnmatched:false.
