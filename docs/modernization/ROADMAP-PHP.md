...
### What was done (phases 1–10 + cleanup)

All modules fully migrated. Each function moved to a typed service class;
...
| 11 | MetadataAdminService, HistoryAdminService, WsHelper | 21 |
| **Total** | **44 service classes** | **~525 functions** |
...
| `admin/include/functions_metadata.php`                 | ~390  | 7     | ✅ done | `Piwigo\Admin\Metadata\MetadataAdminService` |
| `admin/include/functions_history.inc.php`              | ~250  | 6     | ✅ done | `Piwigo\Admin\History\HistoryAdminService` |
| `include/ws_functions.inc.php`                         | ~260  | 8     | ✅ done | `Piwigo\Ws\WsHelper` |
...
| File                                                                         | Becomes                                                                       | Notes                                                                         |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `include/section_init.inc.php`                                               | `Piwigo\Section\SectionInitializer::initialize()`                             | Deferred → **#22** (MVC controllers; PATH_INFO routing tightly coupled)       |
| `include/user.inc.php`                                                       | `Piwigo\Users\UserBootstrap`                                                  | Deferred → **#22** (request lifecycle bootstrap)                              |
| `include/filter.inc.php`                                                     | `Piwigo\Filter\FilterResolver`                                                | Deferred → **#22** (filter state from session/URL)                            |
| `include/ws_core.inc.php`                                                    | merge into `Piwigo\Ws\PwgServer`                                              | Skipped — already a 27-line backward-compat shim (done in #8)                |
| `include/ws_init.inc.php`                                                    | `Piwigo\Ws\PwgServer::boot()`                                                 | ✅ done                                                                        |
| `include/ws_functions/pwg.{categories,extensions,images,php,tags,users}.php` | `Piwigo\Ws\Method\{Categories,Extensions,Images,General,Tags,Users}Endpoints` | ✅ done                                                                        |
| `admin/include/albums_tab.inc.php`                                           | `Piwigo\Admin\Album\AlbumsTabRenderer`                                        | ✅ done                                                                        |
| `admin/include/batch_manager_filters.inc.php`                                | `Piwigo\Admin\BatchManager\FilterResolver`                                    | ✅ done                                                                        |
| `admin/include/configuration_sizes_process.inc.php`                          | `Piwigo\Admin\Config\SizesProcessor`                                          | ✅ done                                                                        |
| `admin/include/configuration_watermark_process.inc.php`                      | `Piwigo\Admin\Config\WatermarkProcessor`                                      | ✅ done                                                                        |
| `admin/include/photos_add_direct_prepare.inc.php`                            | `Piwigo\Admin\Upload\DirectPreparer`                                          | ✅ done                                                                        |
| `admin/include/user_tabs.inc.php`                                            | `Piwigo\Admin\Users\UserTabRenderer`                                          | ✅ done                                                                        |
| `include/constants.php`                                                      | `Piwigo\Core\Config::dbPrefix()`                                              | Deferred → **#18** (pre-Composer bootstrap; removing table constants breaks everything) |
...
#### `DerivativeService` direct call site

`UploadService::addUploadedFile()` calls `DerivativeService::generate($imageInfos, IMG_MEDIUM)` **synchronously** after every upload. This replaces the old `AdminService::fetchRemote()` HTTP self-request that triggered `i.php` to warm the medium derivative. Result: same timing as before, no worker dependency, no external HTTP round-trip.

All other sizes remain lazily generated by `i.php` on first browser request.

#### Job dispatch call sites

| Location | Job dispatched | Notes |
|---|---|---|
| `admin/maintenance_actions.php` `derivatives` | `RegenerateAllDerivativesJob` | After synchronous cache clear; handler fans out per-image `GenerateDerivativeJob`s |
| `UserService::registerUser()` | `SendNotificationEmailJob` | Pre-renders subject+content at dispatch time for correct language |

#### Worker dependency

The job queue requires a long-running worker process to consume messages. Without one, dispatched jobs accumulate in `{prefix}messenger_messages` and are never processed — which is safe because all worker-dispatched work is either advisory (derivative pre-warming beyond IMG_MEDIUM) or tolerates delay (registration email). Nothing in the request path blocks on worker completion.

The worker is intended for deployments that can run a daemon:

```bash
# supervisord / systemd / screen
bin/piwigo messenger:consume async --time-limit=3600 --memory-limit=256M
```

On shared hosting without persistent processes, the queue simply stays idle and `i.php` handles derivatives lazily as always.

### Notes

- `ReindexImagesJob` / `ReindexImagesHandler`: Piwigo uses SQL LIKE / MATCH for search — no external index to rebuild. Handler purges stale search history so it recomputes on next request. Extendable to a real FTS engine when search is upgraded.
- Supervisor/systemd config not included; worker command documented above.
