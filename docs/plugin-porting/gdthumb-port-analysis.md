...
| `data-image-id`/(new, gdThumb needs) per-category identifier | `themes/default/template/thumbnails.latte:19` (line corrected 2026-09-12, was cited as 18) `<li data-image-id="{$thumbnail->id}">`, `themes/default/template/mainpage_categories.latte:13-16` `<li data-category-id="{$cat->id}">` | Both confirmed present, both the only real grid-loop renderers of these two Views anywhere in `themes/` (no other theme renders a competing shape) |
| `jquery.ba-resize.min.js` (custom resize-event polyfill) | Native `ResizeObserver` | Confirmed already the house pattern (`themes/default/js/vendor/widgets/lineChart.ts`, path corrected 2026-09-12) — drop the legacy polyfill entirely |
