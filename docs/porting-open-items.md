...
- [ ] No `ExtensionContext` accessor for "is another named extension active" — `PluginRegistry::isActive(string $pluginId): bool` exists but isn't reachable from `ExtensionContext` at all. Real example: `block_search_13.0.a`'s own `SELECT state FROM PLUGINS_TABLE WHERE id = 'PWG_Stuffs'` check, found while investigating the raw-DB-surface item above (a separate, smaller gap, not folded into `db()`).
