...
| `pwg_set_session_var('show_metadata', true)` | `themeconf.inc.php` | **Closed (§7, Q14).** The naive `ExtensionContext::session()` route was wrong — it would have silently written an unread key, since core's `PictureController` reads/writes the bare, un-namespaced `pwg_show_metadata` key directly via `SessionService`, bypassing `ExtensionContext` entirely. `ExtensionContext::isShowMetadataEnabled()`/`setShowMetadataEnabled()` now exist for exactly this flag, added for this port. | trivial — landed as designed |
...
| `checkIfHomepage()` / `stripBreadcrumbs()` | `include/themecontroller.php` | **Closed (§7, Q16), via `currentSection()` rather than a new event.** `PageHeaderRendering`/`PageHeaderRendered` are still empty marker classes (confirmed unchanged) — but `ExtensionContext::currentSection(): ?SectionContext` now gives any handler direct pull access to `isHomepage`/section/category/items, no event payload needed. See §5.14. | small — closed without the event this row originally called for |
...
| PhotoSwipe (lightbox) | `node_modules/photoswipe@4.1.3` | **Resolved (§7, Q5):** PhotoSwipe v5, narrow-ported as `themes/default/js/vendor/widgets/photoswipe.ts` (path corrected 2026-09-12) following `colorbox.ts`'s exact real pattern (`WeakMap<Element,Options>` + registration list, lazy singleton overlay, `computeRelated()`/`relatedAt()` gallery grouping). Built as a **shared, non-darkroom-private** capability — any theme can adopt it. | medium |
...
See §3c — PhotoSwipe v5 as a shared `vendor/widgets/photoswipe.ts` capability (path corrected 2026-09-12), following `colorbox.ts`'s pattern. Scope confirmed: `index.tpl`/`thumbnails.tpl`/`_photoswipe_*.tpl`, plus an optional additive "view fullscreen" button on `picture.tpl`. `picture.ts`/`picture_nav_buttons.ts` confirmed structurally unrelated (real page-navigation, not lightbox state) — no rearchitecture needed there. Recommend landing this as its own task ahead of the darkroom-specific work — core's default theme can adopt it too. Effort: **medium**.
...
| `jquery.cookie.js` | theme's own JS | `themes/default/js/vendor/utils/cookie.ts` (already exists; path corrected 2026-09-12) | Fully superseded already |
...
| 11 | Theme/plugin l10n auto-load design | **Resolved, and landed 2026-09-12 exactly as designed** — confirmed directly in `ThemeRegistry`/`PluginRegistry`'s current source: direct load in `bootCurrent()`, `PluginRegistry` implementing `SubscriberInterface` via `LoadingLang` (§5.12) |
...
| 14 | Session-flag namespacing | **Closed 2026-09-12** — original proposal was wrong, but the new method it called for now exists: `isShowMetadataEnabled()`/`setShowMetadataEnabled()` (§3a) |
...
**Phase 1 — Shared infrastructure this port surfaces but isn't specific to it — done, landed 2026-09-12 (P61-A..D), confirmed against real source, one item shaped differently than planned:**
- Generalize theme/plugin translation auto-loading (§5.12): landed as designed — `LangService::loadLanguageForPlugin()`/`isInstalledLocale()` deleted, `ThemeRegistry`/`PluginRegistry` wired as designed.
- `themes/default/js/vendor/widgets/photoswipe.ts` (path corrected 2026-09-12) (§5.8) as a shared lightbox capability, following `colorbox.ts`'s pattern — landed (deferred to Phase 6 in the real execution, per this file's own P61 status elsewhere).
- ~~The new page-header-state PSR-14 event (§5.14)~~ — not built; superseded by the more general `ExtensionContext::currentSection()` instead (also covers `findByIdsOrdered()`'s own need).
- The one new narrow `ExtensionContext` session method for the legacy-shared `show_metadata` flag (§3a) — landed as `isShowMetadataEnabled()`/`setShowMetadataEnabled()`.
- `paths(): Paths` also landed in this phase (P61-A..D), for the theme-owned `custom_css` settings file — not separately called out in this section's original scope, but real and shipped alongside the above.
