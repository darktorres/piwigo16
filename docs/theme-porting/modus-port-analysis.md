...
- **`colorscheme: "dark"`** — the manifest, gap-audit, and CSS dimensions all independently raise the per-skin colorscheme mismatch as `needs_human_decision`, but all three land on compatible answers once you separate the two same-named-but-unrelated concepts (see §4-A). `"dark"` here is Option A (this schema field stays a static, secondary fact matching the *theme-level* legacy default). **Confirmed 2026-09-12**: the real, shipped `modus_17.0.0/theme.json` has exactly `"colorscheme": "dark"` — Option A is what shipped. Whether a human ever formally signed off on it (as opposed to it just being what got implemented) isn't something code can confirm either way.
...
| Smarty compile-hook/plugin registration (8 occurrences) | closed 2026-09-12, no generic mechanism built | The raw compile-hook mechanism itself still doesn't exist; each of the 3 payloads was resolved individually instead — cssGradient/cssResolution dropped as CSS-era shims (§3c), modus_thumbs via a View-layer data-prep step (`ModusThumbGridPageContext`/`ModusMasonryThumbItem`, confirmed in the real `modus_17.0.0/` source) |
| `modus_smarty_prefilter()` (raw-source rewriting) | closed 2026-09-12, via redesign not a new hook | No compile-time source-rewrite hook was built (still doesn't exist) — instead §4-B's Option B (`{block}`-refactor of `themes/default`, P29.6-L..T) let each of the prefilter's concrete DOM edits be hand-applied as a named-block override, confirmed landed per `docs/PLAN.md` |
...
| `pwg_get_session_var('index_deriv'/'picture_deriv')` (reads) | closed 2026-09-12 | `ExtensionContext::coreIndexDeriv()`/`corePictureDeriv()` now expose these two shared core session keys directly, confirmed in `ExtensionContext`'s current source |
...
| `caps`/`picture_deriv`/`phavsz` cookie read/write | closed 2026-09-12 | `ExtensionContext::cookies(): ExtensionCookie` now exists (namespaced `pwg_*` cookie read/write); `cookie_path()` still has no direct equivalent but is no longer needed to reach cookies at all |
...
| Direct mutation of `$conf['tag_letters_column_number']` per device | closed 2026-09-12 | Confirmed `CurrentConfig::$tagLettersColumnNumber` is now a plain mutable `public` property, not `private(set)` — `$context->config()->tagLettersColumnNumber = ...` works directly, no separate override mechanism needed |
...
> **`$conf['tag_letters_column_number']` per-device mutation** (`themeconf.inc.php:62-65`; `CurrentConfig.php:1440-1444`) — *unsolved_gap*. Verified directly: `CurrentConfig::$tagLettersColumnNumber` has `private(set)` visibility — external code, including a theme via `ExtensionContext::config()`, cannot write to it at all. Even if it could, `ExtensionContext::setSetting()` is the only settings-write path and persists to the DB for ALL future requests/users, not a per-request, per-device, non-persistent override — using it here would incorrectly leak a mobile visitor's override into every subsequent desktop visitor's session too. *Proposed approach:* No existing `ExtensionContext` mechanism supports a request-scoped (non-persistent) override of a typed `CurrentConfig` property. Requires either a new `ExtensionContext` capability for scoped/ephemeral config overrides, or accepting this device-responsive behavior as dropped in the port. **Closed 2026-09-12 — the premise was wrong, not the plan**: `$tagLettersColumnNumber` turned out to already be a plain mutable `public` property, not `private(set)` as this item assumed; no new capability was needed at all.
...
| `$_COOKIE['caps']` read + session promotion | closed 2026-09-12 | Session half was already clean; cookie-read half now has `cookies(): ExtensionCookie` |
...
| `modus_thumbs` custom function | closed 2026-09-12 | No registration mechanism was built (still doesn't exist) — resolved instead via a View-layer data-prep step, `ModusThumbGridPageContext`/`ModusMasonryThumbItem`, confirmed in the real `modus_17.0.0/` source |
...
| Settings page + `glob()` skin discovery | closed 2026-09-12, exactly per §2's proposed layout | `hasSettings:true` confirmed required; the static skin-catalog source landed as `SkinCatalog.php`/`Skin.php`, both real files in `modus_17.0.0/src/` |
...
| "modus mode" masonry layout (both files) | closed 2026-09-12 | Decided the harder way: both modes were built (§4-C Option 2), via the `ModusThumbGridPageContext` View-layer helper shape |
...
| menubar special-link hoisting | closed 2026-09-12 | `Piwigo\Menu\MenubarSpecialKind` (a real backed enum) now gives `MenubarSpecialsView` a stable discriminator |
...
> **"modus mode" custom masonry thumbnail/category layout** — *needs_human_decision*. The template-level shape (a runtime if/else branch) is directly portable. The categories side has a confirmed live event mapping. The thumbnails side's `{modus_thumbs}` is a Smarty function-plugin that emits raw HTML directly from inline PHP — `ExtensionContext` has no "register a custom template tag/function" mechanism, so this needs a different shape (a PHP helper called via the existing `$pwg`/`TemplateAdapter` bridge) rather than a direct translation. *Proposed approach:* See §4-C. **Closed 2026-09-12**: §4-C's Option 2 (both modes) was chosen; the thumbnails side landed as a View-layer helper (`ModusThumbGridPageContext`), not a template-tag registration.

> **menubar `mbMostVisited`/`mbBestRated`/`mbSpecials` hoisting** — *needs_human_decision*. Legacy modus's horizontal-menu UX hoists 3 specific "special" links out of the normal dropdown into always-visible items, keyed by array keys like `$blocks.mbSpecials->data.most_visited`. Today's `MenubarSpecialsView` exposes only a flat list of `{url,title,name,noFollow}` link objects with no stable, non-translated discriminator for which entry is "most visited" — confirmed by reading the whole file. There is no safe way to peel these out by matching translated label text. *Proposed approach:* Either (a) make a small, localized addition to `MenubarSpecialsView`'s item shape (a stable `id`/`kind` per well-known special link) — a small `adapt_in_piwigo17_rewrite` change — or (b) modus drops this specific hoisting behavior and folds all specials into the normal dropdown like every other theme. **Closed 2026-09-12, option (a)**: `Piwigo\Menu\MenubarSpecialKind`, confirmed in `src/Piwigo/Menu/MenubarSpecialKind.php`.
...
| `modus_thumbs()`'s `block_html_style()` CSS injection | closed 2026-09-12 | Landed as a real inline `<style>` block in `thumbnails.latte` with interpolated per-request values (`{$MODUS_THUMB_GRID_ROW_HEIGHT}` etc.), the same request-computed-value-into-static-CSS idiom this item called for |
...
| `photo.autosize.js` — `changeImgSrc` override | closed 2026-09-12 | `picture.ts` now has a real mutable `derivativeSwitchOverride` slot, `SwitchBoxQueue`-style, confirmed in its current source |
...
| Theme's own `boot()`-time translation loading | closed 2026-09-12, as designed | `ThemeRegistry`/`PluginRegistry` now wire this end-to-end (see §4-H) |
...
**A. Per-skin `colorscheme` variance vs. `theme.schema.json`'s static enum.** *(Raised independently, and in agreement, by the gap-audit, manifest/lifecycle, and CSS/skins dimensions.)* Net fact, reconciled across all three: 10 of 18 skins are effectively light ("clear"), 8 effectively dark, selected per-request/per-admin-config — but `theme.json`'s `colorscheme` is a single static fact read once from disk with no runtime override path anywhere in `ThemeChain`. The CSS dimension's framing is the cleanest resolution: **these are two same-named but functionally unrelated concepts** — the schema field's real consumers are narrow, secondary surfaces (`standard_pages`, admin batch-manager dark/light CSS variant), while modus's own 18-way palette is a wholly separate, theme-owned concern.
  - **Option A (recommended, and confirmed 2026-09-12 as what shipped):** Hardcode `theme.json`'s `colorscheme` to a static value (`"dark"`, matching the theme-level legacy default) and handle all 18 skins' actual light/dark palette entirely through the CSS-custom-property mechanism, completely decoupled from this schema field. Zero core change; accept the documented, minor cosmetic mismatch where `standard_pages`/batch-manager won't track whichever skin is actually selected. The real `modus_17.0.0/theme.json` has exactly `"colorscheme": "dark"`.
...
**B. How to replace Smarty's compile-time source-rewriting for `default`-theme fragments modus needs to alter, and how much shared/core surface to touch doing it.** *(Raised by the templates dimension as Option A/B for `picture.latte`/`index.latte`/`layout.latte`, and independently by the CSS dimension as the `Template::resolveLocalHeadOnce()` generalization question — both are the same underlying "invest in shared extensibility now, or fork wholesale" fork.)*
  ...
  - **Option B (chosen, landed 2026-09-12) — refactor `default`'s own templates to expose named `{block}` regions at the exact seams a child theme needs**, then have modus `{layout}`-extend and override only those blocks (`{block pageBanner}`, `{block imageHeaderBar}`, etc.). Confirmed via `docs/PLAN.md`'s own P29.6 status: Phase 2 (`{block}`-refactor of `themes/default`, 10 templates, P29.6-L..T) landed in this repo. Cross-theme `{layout}` block-inheritance chaining, this item's own open prototype question, works as hoped.
  - **Sub-decision, landed alongside B:** `Template::resolveLocalHeadOnce()` was generalized into a real per-theme-chain loop (confirmed live via `modus`'s own skin-driven `<style>:root{}` `localHead` block).
...
**C. Is "modus mode" (the `album_thumb_size`-driven custom masonry thumbnail/category layout — `modus_thumbs` Smarty function + `thumb.arrange.js`'s `RVGThumbs` client-side justified-row algorithm) in scope for the first port at all?** *(Raised by the templates dimension as a scope question, load-bearing for the gap-audit's hardest-flagged item, and material to the JS dimension's "genuinely new module" finding.)* This is the single largest chunk of genuinely new, non-trivial work in the whole port — it needs a new PHP-callable-from-template helper shape (no precedent anywhere in this fork) *and* a from-scratch TS port of a real layout algorithm with no existing equivalent.
  - **Option 1 (recommended for a first port):** Ship "standard mode" only for v1, defer "modus mode".
  - **Option 2 (chosen instead) — Port both modes together.** Confirmed directly against the real `modus_17.0.0/` source: `src/ModusMasonryThumbItem.php`, `src/ModusThumbGridPageContext.php`, `src/ModusAlbumThumbsPageContext.php`, and `js/thumb-arrange.ts` all exist — the harder option was the one actually built, not the recommended-safer deferral.
...
**D. Theme-owned JS/CSS delivery pipeline is functionally real but has zero production exercise.** *(Raised by the JS dimension for `GetPageAssets`/`AssetContribution`/`PageAssets`, and independently by the admin dimension for the Vite `collectScriptEntries()` scan gap — these are two halves of the same underlying "can a theme's own JS actually reach a real page" question.)* Concretely: (1) `GetPageAssets` is wired end-to-end in `Template::dispatchPageAssetsOnce()` but every real reference to it in the repo is a unit test; (2) even if a theme correctly implements it, `collectScriptEntries()` never scans `themes/**/src`, so the referenced `.ts` file would never become a real Vite entry and would never load in production.
  - **(1) Closed**: `Template.php`/`Renderer.php` are real, non-test `GetPageAssets` consumers now.
  - **(2) Closed for a different case than modus's own, worth being precise about**: `collectScriptEntries()` now scans `themes/**/src`/`plugins/**/src` too (`roots: { src, themes, plugins }`) — but that covers a theme/plugin bundled *inside* `piwigo17-rewrite`'s own tree, not a sibling-catalog-repo port like `modus_17.0.0/` in this repo. Modus's own real JS (`js/thumb-arrange.ts` etc.) ships through the separate, already-existing `tools/build-ported-extension-assets.mjs` (`bun run build:ported-extensions`, documented in `../piwigo16-plugins/CLAUDE.md`'s "Own JS" section) — confirmed that mechanism, not the `collectScriptEntries()` fix, is what actually gets modus's JS to a real page.

### Tier 2 — real, scoped gaps needing a small `ExtensionContext`/core capability, each independently confirmed absent

**E. `ExtensionContext` capability gaps — all 5 closed, revalidated 2026-09-12 directly against `ExtensionContext`'s current source:**
  1. **Closed.** `coreIndexDeriv()`/`corePictureDeriv()`/`setCorePictureDeriv()` now expose the shared, un-namespaced `pwg_index_deriv`/`pwg_picture_deriv` session keys directly — the narrow, explicitly-named accessor option was the one built.
  2. **Closed.** `cookies(): ExtensionCookie` (namespaced `pwg_*` cookie read/write) now exists.
  3. **Closed.** `CurrentConfig::$tagLettersColumnNumber` is a confirmed plain mutable public property (not `private(set)`) — `$context->config()->tagLettersColumnNumber = ...` works directly from `boot()`, no new mechanism needed after all.
  4. **Still open, unchanged.** No request/query-parameter accessor reachable from `boot()` or an early-firing event exists — still blocks the `?skin=` live preview override (Tier 3-J's own "leaning drop" call stands).
  5. **Closed.** `imageStdParams(): ImageStdParams` exists, the general-accessor option (not the narrower read-facade alternative).

**F. Closed.** `picture.ts` now has exactly the proposed override hook: a mutable `derivativeSwitchOverride` slot (`SwitchBoxQueue`-style) that `changeImgSrc()`'s own call sites consult before falling through to the default — option (a) is what was built, confirmed directly in `themes/default/js/picture.ts`.

**G. Closed.** `Piwigo\Menu\MenubarSpecialKind` (a backed enum: `Favorites`/`MostVisited`/`BestRated`/`RecentPics`/`RecentCats`/`Random`/`Calendar`) now gives `MenubarSpecialRow` a stable, non-translated discriminator — option (a) is what was built, confirmed directly in `src/Piwigo/Menu/MenubarSpecialKind.php`.

**H. Closed, exactly as designed.** `LangService::loadLanguageForPlugin()`/`isInstalledLocale()` are deleted; `ThemeRegistry::bootCurrent()` calls `$this->lang->load('theme.lang', ...)` directly, and `PluginRegistry` implements `Core\SubscriberInterface`, subscribing to `Lang\Event\LoadingLang` to load each booted plugin's own `plugin.lang` — confirmed directly in both classes' current source. This resolves questions (1) and (2); question (3) (a per-install local-language-override mechanism / `Paths`-backed accessor) is moot now that `paths(): Paths` also exists as a plain accessor, though no specific local-override-for-strings caller has exercised it yet.

**I. Still open, unchanged.** `ConfigService::confGetParam()`/`confUpdateParam()` are still a bare `json_decode()`/`json_encode()` round-trip with no legacy-`serialize()`-format fallback (confirmed: no `unserialize()`/format-detection logic anywhere in `ConfigService`). A real 16.x→17.x upgrade migrating pre-existing `config.value` rows remains unaddressed at the framework level — genuinely still a cross-cutting gap, not modus-specific, and not yet resolved by anything in P29.6/P61.
...
**K.** ~~Should `tags.latte`'s 2-item `categoryActions` list also get the mobile action-switcher button~~ — **moot, confirmed 2026-09-12**: `modus_17.0.0/template/` has no `tags.latte` override at all, so this question never had to be answered.

**L.** Should the CSS/skins dimension's assumption ... be confirmed with an actual visual check before those template overrides are dropped from the plan? **Not independently re-verified visually this session** (that needs a browser, not grep) — but `docs/PLAN.md`'s own P29.6 status records a real closing live-install verification (item 44) that "confirmed skin switching/masonry/settings-round-trip," which plausibly covers this; treat as likely-checked-already rather than confirmed here.

**M.** ~~Do `themes/default/icon/errors.png`, `infos.png`, and `rating-stars.gif` ... actually exist~~ — **confirmed 2026-09-12: yes**, all 3 exist at exactly those paths. No replacement/dropping needed.

**N.** ~~Does `hf_base.css` become unconditional...~~ — **decided, confirmed 2026-09-12**: unconditional. `hf_base.css`'s content is folded directly into the real `theme.css` (`/* Layout refinements shared by every skin (from css/hf_base.css ...`), not gated per-skin.

**O.** ~~Who dispatches the custom `RVTS_loaded` DOM event...~~ — **confirmed 2026-09-12**: the real `modus_17.0.0/js/thumb-arrange.ts` itself is the only file referencing it — self-contained, not an external PHP hook.
...
**Q.** ~~Should `hasSettings` be plain `true`...~~ — **decided, confirmed 2026-09-12**: the real `theme.json` has `"hasSettings": true`, the recommended value.

**R.** ~~Is a typed value object worth introducing...~~ — **decided, confirmed 2026-09-12**: yes — `ModusThemeSettings` is a real `final readonly class` with 5 typed properties (`$skin`, `$albumThumbSize`, `$indexPhotoDeriv`, `$indexPhotoDerivHdpi`, `$displayPageBanner`), not a plain array.

**S.** ~~Is `'local-modus-17.0.0'` acceptable...~~ — **decided, confirmed 2026-09-12**: yes, the real `manifest.json` entry uses exactly `"revision_id": "local-modus-17.0.0"`. The sparse-locale policy was also decided: `ar_SA`'s real `theme.po` shipped at only 22 lines, not backfilled — "don't backfill" is the policy in practice, no minimum-coverage bar enforced.

**T.** ~~Should `css/fontello/config.json` be kept...~~ — **decided, confirmed 2026-09-12**: kept — `modus_17.0.0/css/fontello/config.json` is real and present on disk.
