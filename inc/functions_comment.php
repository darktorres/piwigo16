...
class functions_comment
{
   ...
   */
  static function user_comment_check($action, $comment)
  {
    global $conf,$user;
  ...
  SELECT COUNT(*) AS user_exists
    ...
    if ($comment_action!='reject' and $conf['anti-flood_time']>0 and !functions_user::is_admin())
    { // anti-flood system
  ...
  ;';
    ...
    $comment_action = functions_plugins::trigger_change('user_comment_check',
        $comment_action, $comm
      );
  ...
  static function update_user_comment($comment, $post_key)
  {
    ...
    // perform more spam check
    $comment_action =
      functions_plugins::trigger_change('user_comment_check',
        $comment_action,
        array_merge($comment,
              array('author' => $GLOBALS['user']['username'])
              )
  ...
  ;';
      ...
      elseif ($result)
      {
        self::email_admin('edit', array('author' => $GLOBALS['user']['username'],
   ...
   * @return int
   */
  static function get_comment_author_id($comment_id, $die_on_error=true)
  {
    $query = '
  SELECT
      author_id
   ...
   * @param int|int[] $comment_id
   */
  static function validate_user_comment($comment_id)
  {
    if (is_array($comment_id))
      $where_clause = 'id IN('.implode(',', $comment_id).')';
    else
   ...
   */
  static function invalidate_user_cache_nb_comments()
  {
    global $user;
