...
class functions_metadata
{
   ...
   * @return array
   */
  static function get_iptc_data($filename, $map, $array_sep=',')
  {
    global $conf;
          ...
          if (isset($iptc[$iptc_key][0]))
          {
            if ($iptc_key == '2#025')
            {
              $value = implode($array_sep,
                              array_map(self::clean_iptc_value(...),$iptc[$iptc_key]));
            }
            else
            {
   ...
   * @return string
   */
  static function clean_iptc_value($value)
  {
    ...
    if ( preg_match('/[\x80-\xff]/', $value) )
    {
      ...
      if ( ($qual = functions::qualify_utf8($value)) != 0)
      {// has non ascii chars
        if ($qual>0)
        {
   ...
   * @return array
   */
  static function get_exif_data($filename, $map)
  {
    global $conf, $logger;
    ...
    if ($exif = @exif_read_data($filename) or $exif2 = functions_plugins::trigger_change('format_exif_data', $exif=null, $filename, $map))
    {
      ...
      {
        $exif = functions_plugins::trigger_change('format_exif_data', $exif, $filename, $map);
      }
  ...
  static function strip_html_in_metadata(&$v, $k)
  {
    $v = strip_tags($v);
  }
   ...
   */
  static function parse_exif_gps_data($raw, $ref)
  {
    foreach ($raw as &$i)
    {
      $i = explode('/', $i);
