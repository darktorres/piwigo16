...
class functions_user
{
   ...
   */
  static function validate_mail_address($user_id, $mail_address)
  {
    global $conf;

    if (empty($mail_address) and
        !($conf['obligatory_user_mail_address'] and
        in_array(functions::script_basename(), array('register', 'profile'))))
    {
      return '';
    }
   ...
   * @return string|void error message or nothing
   */
  static function validate_login_case($login)
  {
    global $conf;
   ...
   */
  static function search_case_username($username)
  {
    global $conf;
  ...
  ;';
      ...
      if (count($inserts) != 0)
      {
        functions_mysqli::mass_inserts(USER_GROUP_TABLE, array('user_id', 'group_id'), $inserts);
      }
  ...
  SELECT COUNT(DISTINCT(image_id)) as total
    ...
    WHERE category_id NOT IN ('.$userdata['forbidden_categories'].')
      AND image_id '.$userdata['image_access_type'].' ('.$userdata['image_access_list'].')';
        ...
        if ( !self::is_admin($userdata['status']) )
  ...
  INSERT IGNORE INTO '.USER_CACHE_TABLE.'
    ...
    VALUES
    ('.$userdata['id'].',\''.functions_mysqli::boolean_to_string($userdata['need_update']).'\','
   ...
   */
  static function check_user_favorites()
  {
    global $user;
   ...
   */
  static function calculate_permissions($user_id, $user_status)
  {
    $query = '
  SELECT id
   ...
   * @return mixed
   */
  static function get_default_user_value($value_name, $default)
  {
   ...
   */
  static function create_user_infos($user_ids, $override_values=null)
  {
    global $conf;
   ...
   */
  static function log_user($user_id, $remember_me)
  {
    global $conf, $user;
    ...
    if ($remember_me and $conf['authorize_remembering'])
    {
      ...
      if ($key!==false)
      {
        ...
        setcookie($conf['remember_me_name'],
          ...
          functions_cookie::cookie_path(),ini_get('session.cookie_domain'),ini_get('session.cookie_secure'),
   ...
   * @return string
   */
  static function pwg_password_hash($password)
  {
   ...
   * @return bool
   */
  static function pwg_password_verify($password, $hash)
  {
   ...
   * @return bool
   */
  static function try_log_user($username, $password, $remember_me)
  {
   ...
   * @return bool
   */
  static function pwg_login($success, $username, $password, $remember_me)
  {
    if ($success===true)
    {
      return true;
   ...
   * @return string
   */
  static function get_user_status($user_status='')
  {
    global $user;
   ...
   */
  static function get_access_type_status($user_status='')
  {
    global $conf;
   ...
   * @return bool
   */
  static function is_generic($user_status='')
  {
   ...
   * @return bool
   */
  static function is_a_guest($user_status='')
  {
   ...
   * @return bool
   */
  static function can_manage_comment($action, $comment_author_id)
  {
    global $user, $conf;
   ...
   * @return string
   */
  static function get_recent_photos_sql($db_field)
  {
    global $user;
   ...
   * @return null
   */
  static function deactivate_user_auth_keys($user_id)
  {
    $query = '
   ...
   * @return null
   */
  static function deactivate_password_reset_key($user_id)
  {
   ...
   */
  static function userprefs_delete_param($params)
  {
    global $user;
