...
class pwg_categories
{
  ...
  SELECT
    ...
    WHERE image_id IN ('.implode(',', $image_ids).')
      AND '.functions_user::get_sql_condition_FandF(array('forbidden_categories' => 'category_id'), null, true).'
  ;';
        $result = functions_mysqli::pwg_query($query);
        while ($row = functions_mysqli::pwg_db_fetch_assoc($result))
        {
  ...
  ;';
        ...
        foreach ($images as $idx => $image)
        {
          ...
          {
            $url = functions_url::make_index_url(array('category' => $details_for_category[$cat_id]));
   ...
   * account.
   */
  static function ws_categories_getAdminList($params, &$service)
  {
    global $conf;
  ...
  ;';
    ...
    {
      $subcats = functions_category::get_subcat_ids(array($params['category_id']));
      if (count($subcats) > 0)
      {
        $query = '
  ...
  ;';
    ...
    if (0 != $params['parent'])
    {
      $subcat_ids = functions_category::get_subcat_ids(array($params['parent']));
  ...
  ;';
    ...
    {
      $nb_sub_photos = 0;
      $sub_cat_without_parent = array_diff(functions_category::get_subcat_ids(array($update_cat)), array($update_cat));
  ...
  SELECT DISTINCT 
    ...
    $subcat_ids = functions_category::get_subcat_ids(array($category_id));
