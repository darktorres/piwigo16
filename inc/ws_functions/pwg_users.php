...
class pwg_users
{
  ...
  ;';
      ...
      foreach ($users as $cur_user)
      {
        ...
        if (isset($params['display']['registration_date_string'])) {
          $users[$cur_user['id']]['registration_date_string'] = functions::format_date($cur_user['registration_date'], array('day', 'month', 'year'));
        }
        ...
        if (isset($params['display']['last_visit'])) {
          ...
          {
            $users[ $cur_user['id'] ]['last_visit_string'] = functions::format_date($last_visit, array('day', 'month', 'year'));
          }
      ...
      if (isset($params['display']['registration_date_string']))
      {
        foreach ($users as $cur_user)
        {
          $users[$cur_user['id']]['registration_date_string'] = \Piwigo\inc\functions::format_date($cur_user['registration_date'], array('day', 'month', 'year'));
        }
      }
      ...
      if (isset($params['display']['registration_date_since']))
      {
        foreach ($users as $cur_user)
        {
          $users[ $cur_user['id'] ]['registration_date_since'] = \Piwigo\inc\functions::time_since($cur_user['registration_date'], 'month');
        }
      }
      ...
      if (isset($params['display']['last_visit']))
      {
        foreach ($users as $cur_user)
        {
          ...
          if (!\Piwigo\inc\dblayer\functions_mysqli::get_boolean($cur_user['last_visit_from_history']) and empty($last_visit))
          {
            $last_visit = \Piwigo\inc\functions_user::get_user_last_visit_from_history($cur_user['id'], true);
            $users[ $cur_user['id'] ]['last_visit'] = $last_visit;
          }

          if (isset($params['display']['last_visit_string']))
          {
            $users[ $cur_user['id'] ]['last_visit_string'] = \Piwigo\inc\functions::format_date($last_visit, array('day', 'month', 'year'));
          }
          
          if (isset($params['display']['last_visit_since']))
          {
            $users[ $cur_user['id'] ]['last_visit_since'] = \Piwigo\inc\functions::time_since($last_visit, 'day');
          }
        }*/ 
    }
  ...
  ;';
    ...
    if (!empty($params['status']))
    {
      if (in_array($params['status'], array('webmaster', 'admin')) and !functions_user::is_webmaster() )
      {
