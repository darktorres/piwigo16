...
// P22: extracted from profile.php's own top-level free functions --
// confirmed via a real PHPStan full-project run that both
// admin/configuration.php's "default" tab and admin/profile.php still
// call these by name (via their own `include_once
// PHPWG_ROOT_PATH . 'profile.php';`, now retargeted at this file), so
// they cannot become Piwigo\Controller\ProfileController private
// methods the way every other page-owned free function this phase has.
// Piwigo\Controller\ProfileController::__invoke() include_once's this
// same file and calls these as free functions too.
    ...
    if (count($errors) === 0) {
        ...
        if (isset($_POST['mail_address'])) {
            ...
            if (is_string($new_pwd_for_update) and $new_pwd_for_update !== '' and $new_pwd_for_update !== '0') {
                ...
                $data[$user_fields['password']] = pwg_password_hash($new_pwd_for_update);
