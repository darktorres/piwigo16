...
CREATE TABLE `activity` (
  ...
  `object_id` int(11) NOT NULL COMMENT 'id of the affected object, or the target user id on a logout action -- the durable record of which row this was about, kept when the typed reference column beside it is nulled by a deletion',
...
CREATE TABLE `derivative_settings` (
  `id` smallint(6) NOT NULL COMMENT 'settings row identifier',
  `default_quality` int(11) NOT NULL DEFAULT 95 COMMENT 'default JPEG compression quality, 0 to 100, for generated derivative images',
  `watermark_json` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL COMMENT 'encoded watermark configuration' CHECK (json_valid(`watermark_json`)),
  `custom_json` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL COMMENT 'encoded custom derivative-generation parameters' CHECK (json_valid(`custom_json`)),
  PRIMARY KEY (`id`)
...
CREATE TABLE `derivative_size` (
  ...
  `min_width` int(11) DEFAULT NULL COMMENT 'minimum output width required to allow cropping, see SizingParams::min_size',
  `min_height` int(11) DEFAULT NULL COMMENT 'minimum output height required to allow cropping, see SizingParams::min_size',
  `sharpen` decimal(5,4) NOT NULL DEFAULT 0.0000 COMMENT 'sharpening amount from 0, none, to 1, max, see DerivativeParams::sharpen',
  `last_mod_time` int(11) NOT NULL DEFAULT 0 COMMENT 'unix timestamp of the last parameter change, used to invalidate cached derivatives, see DerivativeParams::last_mod_time',
  PRIMARY KEY (`name`)
...
CREATE TABLE `history_summary` (
  ...
  `history_id_from` int(11) DEFAULT NULL COMMENT 'lowest history.id folded into this summary row -- a watermark, not a reference: autopurge deliberately deletes the history rows this range covers, so no foreign key can exist here',
  `history_id_to` int(11) DEFAULT NULL COMMENT 'highest history.id folded into this summary row, the next run resumes past this id -- a watermark, not a reference, and autopurge cursors on it, so it must survive the purge it drives',
...
CREATE TABLE `user_infos` (
  ...
  `lastmodified` timestamp NOT NULL DEFAULT current_timestamp() COMMENT 'row last-update timestamp',
  `preferences` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL COMMENT 'generic per-user key-value bag for preferences with no dedicated column' CHECK (json_valid(`preferences`)),
  PRIMARY KEY (`user_id`),
  KEY `lastmodified` (`lastmodified`),
