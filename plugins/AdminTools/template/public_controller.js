var AdminTools = (function ($) {
    ...
    function moveBody(dir, anim) {
        ...
        if (anim) {
            ...
            if (
                $('#the_page, [data-role="page"]').css("position") == "absolute"
            ) {
                $('#the_page, [data-role="page"]').animate({
                    top: operator + ato_height,
                });
            }
        } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ("sessionStorage" in window) {
                        window.sessionStorage.multiView = JSON.stringify(
                            data.result,
                        );
                    }
                },
    ...
    this.init = function (open) {
        ...
        $ato.find(".close-panel").on("click", function (e) {
            ...
            if ("localStorage" in window)
                window.localStorage.ato_panel_open = 0;
            e.preventDefault();
        });
        ...
        $ato_closed.on("click", function (e) {
            ...
            if ("localStorage" in window)
                window.localStorage.ato_panel_open = 1;
            e.preventDefault();
        });
    };
                ...
                if (is_picture) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for (
                                var i = 0, l = data.result.tags.length;
                                i < l;
                                i++
                            ) {
                                tags.push({
