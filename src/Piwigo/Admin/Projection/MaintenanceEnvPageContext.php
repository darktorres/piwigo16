 ...
 * {@see \Piwigo\Admin\MaintenanceEnvPageRenderer::render()}.
 * `$graphicsLibrary`, `$maintUnlockGallery`/`$maintLockGallery`, and
 * `$installedOn`/`$installedSince` are genuinely optional -- the original
 * code only ever assigns those template keys under their own runtime
 * condition, omitted here (not present as a null value) to match that
 * exact original behavior. `$maintUnlockGallery`/`$maintLockGallery` are
 * also mutually exclusive (gallery lock state).
 */
final readonly class MaintenanceEnvPageContext implements TemplatePageContext
{
    /**
    ...
    public function toArray(): array
    {
        $result = [
            ...
            'PWG_VERSION' => $this->pwgVersion,
            'U_CHECK_UPGRADE' => $this->checkUpgradeUrl,
            'OS' => $this->os,
            'CONTAINER_INFO' => $this->containerInfo,
        ...
        if ($this->installedOn !== null) {
            $result['INSTALLED_ON'] = $this->installedOn;
        }

        if ($this->installedSince !== null) {
            $result['INSTALLED_SINCE'] = $this->installedSince;
        }
