...
final class CookieService
{
    ...
    public function setCookieVar(string $var, mixed $value, ?int $expire = null): bool
    {
        if ($value == null or $expire === 0) {
            ...
            return setcookie('pwg_' . $var, '', ['expires' => 0, 'path' => $this->cookiePath() ?? '/', 'samesite' => 'Strict']);
        } else {
            $_COOKIE['pwg_' . $var] = $value;
            $expire = is_numeric($expire) ? $expire : strtotime('+10 years');
            return setcookie('pwg_' . $var, is_scalar($value) ? (string) $value : '', ['expires' => $expire, 'path' => $this->cookiePath() ?? '/', 'samesite' => 'Strict']);
        }
    }
