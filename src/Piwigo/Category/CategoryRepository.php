...
final class CategoryRepository extends AbstractRepository
{
    ...
    /**
     * id/name/permalink/nb_images/global_rank/date_last/max_date_last/
     * count_images/count_categories, loosely typed same as
     * {@see findComputedCategoriesRollup()}.
     *
     * @return list<array<string, mixed>>
     */
    public function findMenuCategories(int $userId, string $whereClause): array
    {
        return $this->conn->createQueryBuilder()
            ->select(
                'id',
            ...
            ->from(Tables::categories())
            ->innerJoin(Tables::categories(), Tables::userCacheCategories(), Tables::userCacheCategories(), 'id = cat_id AND user_id = :userId')
            ->where($whereClause)
