 ...
 * shape -- {@see \Piwigo\Ws\PwgCategories::getList()}'s real (and only)
 ...
 * `is_numeric()`/`is_string()` guards through the WS response-building
 * consumer.
 *
 ...
 * WS response, so it `toArray()`s the whole paginated result once, up
 * front.
 */
