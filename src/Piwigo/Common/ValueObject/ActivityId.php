...
/**
 * Positive integer identifier of a row in the `activity` table.
 *
 * Strict `from(int)` for known-int call sites (DB hydration, internal use);
 * lenient `tryFrom(mixed)` at parsing boundaries.
 */
final readonly class ActivityId implements NumericId
{
    ...
    {
        if ($value <= 0) {
            throw new InvalidArgumentException("ActivityId must be a positive integer, got {$value}");
        }
