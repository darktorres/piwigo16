...
/**
 * Positive integer identifier of a row in the `user_auth_keys` table.
 *
 * Strict `from(int)` for known-int call sites (DB hydration, internal use);
 * lenient `tryFrom(mixed)` at parsing boundaries.
 */
final readonly class AuthKeyId implements NumericId
{
    ...
    {
        if ($value <= 0) {
            throw new InvalidArgumentException("AuthKeyId must be a positive integer, got {$value}");
        }
