...
/**
 * Positive integer identifier of a row in the `image_format` table.
 *
 * Strict `from(int)` for known-int call sites (DB hydration, internal use);
 * lenient `tryFrom(mixed)` at parsing boundaries.
 */
final readonly class FormatId implements NumericId
{
    ...
    {
        if ($value <= 0) {
            throw new InvalidArgumentException("FormatId must be a positive integer, got {$value}");
        }
