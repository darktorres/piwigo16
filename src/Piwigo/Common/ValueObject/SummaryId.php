...
/**
 * Positive integer identifier of a row in the `history_summary` table.
 *
 * Strict `from(int)` for known-int call sites (DB hydration, internal use);
 * lenient `tryFrom(mixed)` at parsing boundaries.
 */
final readonly class SummaryId implements NumericId
{
    ...
    {
        if ($value <= 0) {
            throw new InvalidArgumentException("SummaryId must be a positive integer, got {$value}");
        }
