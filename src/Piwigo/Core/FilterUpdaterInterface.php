...
/**
 * `Piwigo\Filter\FilterService` is L2bExtendedDomain, but its
 * `updateCatsWithFilteredData()` method has real L2aCoreDomain callers
 * (`Category\CategoryService`) that deptrac's ruleset forbids from
 * depending upward on L2b directly. Lives in `Piwigo\Core`
 * (L1Infrastructure, same direction as `ActivityLoggerInterface`) so
 * those classes can depend downward on this instead of the concrete
 * class. `FilterService` implements it; bound in `config/container.php`.
 */
