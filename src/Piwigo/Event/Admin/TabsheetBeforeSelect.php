 ...
 * risk than a genuinely wrong TYPE, which dispatchChange() already
 * catches), so the one real consumer (`Tabsheet::select()`) always
 * re-validates each entry defensively regardless of what this property
 * declares; a precise shape here would only fight that -- and PHPStan
 * itself -- for both this file's own tests (which deliberately construct
 * malformed shapes to exercise that defense) and any real plugin.
 */
