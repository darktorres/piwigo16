...
/**
 * Maps the `derivative_settings` table (`piwigo_derivative_settings` once
 * Piwigo\Db\TablePrefixListener applies db_prefix at metadata-load time).
 * Single logical row, id=1 by convention -- the table itself has no
 * DB-level CHECK/default enforcing that (confirmed against
 * install/piwigo_structure-mysql.sql: `id` is a bare smallint PK, no
 ...
 * OBJECT_SERIALIZED_PARAMS`) -- `watermarkJson`/`customJson` hold the
 * same data `WatermarkParams`/`ImageStdParams::$custom` used to carry via
