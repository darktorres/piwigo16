...
final class Version20260804122301 extends AbstractMigration
{
    ...
    private function upPostgres(): void
    {
        ...
        $this->addSql("COMMENT ON COLUMN {$prefix}group_access.group_id IS 'granted group id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}group_access.cat_id IS 'private album the group is granted access to'");
        ...
        $this->addSql('ALTER TABLE ' . $prefix . 'groups ADD CONSTRAINT groups_ui1_unique UNIQUE (name)');
        $this->addSql('CREATE INDEX groups_lastmodified_idx ON ' . $prefix . 'groups (lastmodified)');
        $this->addSql('CREATE TRIGGER trg_groups_lastmodified BEFORE UPDATE ON ' . $prefix . 'groups FOR EACH ROW EXECUTE FUNCTION piwigo_set_lastmodified()');
        $this->addSql("COMMENT ON TABLE {$prefix}groups IS 'user groups for bulk permission and membership management'");
        $this->addSql("COMMENT ON COLUMN {$prefix}groups.id IS 'surrogate primary key'");
        $this->addSql("COMMENT ON COLUMN {$prefix}groups.name IS 'group display name, unique'");
        $this->addSql("COMMENT ON COLUMN {$prefix}groups.is_default IS 'every newly registered user is automatically added to groups marked default'");
        $this->addSql("COMMENT ON COLUMN {$prefix}groups.lastmodified IS 'row last-update timestamp, set on insert only'");
        ...
        $this->addSql("COMMENT ON TABLE {$prefix}sessions IS 'PHP session storage backend'");
        $this->addSql("COMMENT ON COLUMN {$prefix}sessions.id IS 'composite PHP session id, IP-hash-prefixed by SessionService'");
        $this->addSql("COMMENT ON COLUMN {$prefix}sessions.data IS 'serialized PHP session payload'");
        $this->addSql("COMMENT ON COLUMN {$prefix}sessions.expiration IS 'when this session becomes invalid'");

        $this->addSql('CREATE TABLE ' . $prefix . 'user_access (user_id integer NOT NULL, cat_id integer NOT NULL, PRIMARY KEY (user_id, cat_id))');
        $this->addSql("COMMENT ON TABLE {$prefix}user_access IS 'per-user private album permission grants'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_access.user_id IS 'granted user id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_access.cat_id IS 'private album the user is granted access to'");

        $this->addSql(
            'CREATE TABLE ' . $prefix . 'user_auth_keys (' .
        ...
        $this->addSql("COMMENT ON TABLE {$prefix}user_auth_keys IS 'persistent-login tokens and personal API keys, two row shapes sharing one table'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.auth_key_id IS 'surrogate primary key'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.auth_key IS 'the token value: a random persistent-login token for key_type=auth_key, or the public pkid-... identifier for key_type=api_key'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.apikey_secret IS 'hashed secret half of a key_type=api_key pair, null for auth_key rows'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.user_id IS 'owning user id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.created_on IS 'when the key was issued'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.duration IS 'requested key lifetime, seconds for auth_key rows or days for api_key rows, see expired_on for the actual cutoff'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.expired_on IS 'when the key stops being valid'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.apikey_name IS 'user-given label for a key_type=api_key row, null for auth_key rows'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.key_type IS 'auth_key for a persistent-login/URL-login token, api_key for a personal API key'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.revoked_on IS 'when the key was manually revoked, null if still live'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.last_used_on IS 'when the key last authenticated a request'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_auth_keys.last_notified_on IS 'when the owner was last emailed an expiration notice'");

        $this->addSql(
            'CREATE TABLE ' . $prefix . 'user_feed (' .
            "id varchar(50) COLLATE \"C\" NOT NULL DEFAULT '', " .
            'user_id integer NOT NULL, ' .
            'last_check timestamp DEFAULT NULL, ' .
            'PRIMARY KEY (id))'
        );
        $this->addSql("COMMENT ON TABLE {$prefix}user_feed IS 'per-user private RSS feed tokens'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_feed.id IS 'private feed token, passed as ?feed= to authenticate as the owning user without a login'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_feed.user_id IS 'user this feed token authenticates as'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_feed.last_check IS 'when this feed URL was last polled'");

        $this->addSql('CREATE TABLE ' . $prefix . 'user_group (user_id integer NOT NULL, group_id integer NOT NULL, PRIMARY KEY (group_id, user_id))');
        $this->addSql("COMMENT ON TABLE {$prefix}user_group IS 'user to group membership'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_group.user_id IS 'member user id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_group.group_id IS 'group the user belongs to'");

        $this->addSql(
            'CREATE TABLE ' . $prefix . 'user_infos (' .
            'user_id integer NOT NULL, ' .
        ...
        $this->addSql('CREATE INDEX user_infos_lastmodified_idx ON ' . $prefix . 'user_infos (lastmodified)');
        $this->addSql('CREATE TRIGGER trg_user_infos_lastmodified BEFORE UPDATE ON ' . $prefix . 'user_infos FOR EACH ROW EXECUTE FUNCTION piwigo_set_lastmodified()');
        $this->addSql("COMMENT ON TABLE {$prefix}user_infos IS 'per-user profile and preferences, one row per users.id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.user_id IS 'the owning users.id row, application-assigned, never auto-generated here'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.nb_image_page IS 'photos per page preference'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.status IS 'account role, gates admin access and permission checks'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.language IS 'interface language, references languages.id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.expand IS 'whether the album tree auto-expands in the menu'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.show_nb_comments IS 'whether comment counts are shown alongside thumbnails'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.show_nb_hits IS 'whether view counts are shown alongside thumbnails'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.recent_period IS 'number of days considered recent for the recent photos/albums views'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.theme IS 'interface theme, references themes.id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.registration_date IS 'account creation date'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.enabled_high IS 'whether the user may view/download the original, high-definition photo'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.level IS 'effective permission level, gates access to images.level-restricted photos'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.activation_key IS 'hashed password-reset token, see AuthService::setActivationKey and password.php'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.activation_key_expire IS 'when activation_key stops being valid'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.last_visit IS 'when the user was last seen, refreshed once per session length'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.last_visit_from_history IS 'whether last_visit was already backfilled from the history table, avoids repeating that lookup'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.lastmodified IS 'row last-update timestamp'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_infos.preferences IS 'generic per-user key-value bag for preferences with no dedicated column'");

        $this->addSql(
            'CREATE TABLE ' . $prefix . 'user_mail_notification (' .
            'user_id integer NOT NULL, ' .
        ...
        $this->addSql('ALTER TABLE ' . $prefix . 'user_mail_notification ADD CONSTRAINT user_mail_notification_ui1_unique UNIQUE (check_key)');
        $this->addSql("COMMENT ON TABLE {$prefix}user_mail_notification IS 'per-user new-photo email notification subscriptions'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_mail_notification.user_id IS 'subscribing user id'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_mail_notification.check_key IS 'private token used in subscribe/unsubscribe confirmation email links'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_mail_notification.enabled IS 'whether the user currently receives new-photo notification emails'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_mail_notification.last_send IS 'when a notification email was last sent to this user'");
        ...
        $this->addSql('ALTER TABLE ' . $prefix . 'users ADD CONSTRAINT users_ui1_unique UNIQUE (username)');
        $this->addSql("COMMENT ON TABLE {$prefix}users IS 'core login accounts, column names configurable via CurrentConfig::userFields for multi-auth integrations'");
        $this->addSql("COMMENT ON COLUMN {$prefix}users.id IS 'surrogate primary key, referenced by user_id everywhere else'");
        $this->addSql("COMMENT ON COLUMN {$prefix}users.username IS 'login name, unique'");
        $this->addSql("COMMENT ON COLUMN {$prefix}users.password IS 'hashed login password'");
        $this->addSql("COMMENT ON COLUMN {$prefix}users.mail_address IS 'account email address'");

        $this->addSql(
            'CREATE TABLE ' . $prefix . 'user_failed_logins (' .
            'id integer GENERATED BY DEFAULT AS IDENTITY, ' .
        ...
        $this->addSql('CREATE INDEX idx_user_failed_logins_user_time ON ' . $prefix . 'user_failed_logins (user_id, attempted_at)');
        $this->addSql('CREATE INDEX idx_user_failed_logins_ip_time ON ' . $prefix . 'user_failed_logins (ip, attempted_at)');
        $this->addSql("COMMENT ON TABLE {$prefix}user_failed_logins IS 'failed login attempts, read and written by AuthService::pwgLogin() via UserFailedLoginRepository to back its dual-scope (username + IP) lockout'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_failed_logins.id IS 'surrogate primary key'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_failed_logins.user_id IS 'targeted user id, if the attempted username resolved to a real account'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_failed_logins.ip IS 'REMOTE_ADDR the failed login attempt came from'");
        $this->addSql("COMMENT ON COLUMN {$prefix}user_failed_logins.attempted_at IS 'when the failed attempt occurred'");
    }

    /**
     * Same rationale as Version20260804122300::withMysqlPrefix() -- see
     * that method's own docblock. Each migration in this baseline set
     * needs its own copy since AbstractMigration has no shared-trait home
     * for it without a real second caller of its own.
     */
