...
final readonly class SearchService
{
    ...
    public function getRegularSearchResults(array $search, ?SqlCondition $imagesWhere = null): array
    {
        ...
        // A name -> bool map ("may THIS user use this filter"), separate
        // from FilterViewDefinition::$access ("who is it configured for").
        // This used to flatten each VO to an array purely so the computed
        // bool could be written back over the access-level string, which is
        // the same shape SearchFilterRenderer carried and the reason its
        // own template reads all typed as mixed (P58-B3). Two names beat
        // one slot holding two types.
        $displayFilters = [];
        ...
        if ($expertRule instanceof ExpertRule && $expertString !== null && $expertString !== '' && ($displayFilters['expert'] ?? false)) {
        ...
        if ($allwordsRule instanceof AllwordsRule && $allwordsRule->words !== [] && $allwordsRule->fields !== [] && ($displayFilters['words'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($authorRule instanceof AuthorRule && $authorWords !== [] && ($displayFilters['author'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($filetypes !== [] && ($displayFilters['file_type'] ?? false)) {
            $hasFiltersFilled = true;
            $clauses = [];
            $params = [];
        ...
        if ($addedByIds !== [] && ($displayFilters['added_by'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($catRule instanceof CategoryRule && $catWords !== [] && ($displayFilters['album'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($datePostedPreset !== null && $datePostedPreset !== '' && ($displayFilters['post_date'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($dateCreatedPreset !== null && $dateCreatedPreset !== '' && ($displayFilters['creation_date'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        // ratios
        $ratios = $rules->ratios ?? [];
        if ($ratios !== [] && ($displayFilters['ratio'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($this->currentConfig->rateEnabled && $ratings !== [] && ($displayFilters['rating'] ?? false)) {
            $hasFiltersFilled = true;
            $clauses = [];
            $ratingParams = [];
        ...
        if ($filesizeMinRaw !== null && $filesizeMinRaw !== 0 && $filesizeMaxRaw !== null && $filesizeMaxRaw !== 0 && is_numeric($filesizeMinRaw) && is_numeric($filesizeMaxRaw) && ($displayFilters['file_size'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($heightMinRaw !== null && $heightMinRaw !== 0 && $heightMaxRaw !== null && $heightMaxRaw !== 0 && ($displayFilters['height'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($widthMinRaw !== null && $widthMinRaw !== 0 && $widthMaxRaw !== null && $widthMaxRaw !== 0 && ($displayFilters['width'] ?? false)) {
            $hasFiltersFilled = true;
        ...
        if ($tagsRule instanceof TagsRule && $tagsWords !== [] && ($displayFilters['tags'] ?? false)) {
            $hasFiltersFilled = true;
