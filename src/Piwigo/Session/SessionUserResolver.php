 ...
 *
 * Goes through the real SessionRepository (DBAL) for the composite-key
 * lookup -- AuthService writes sessions keyed by
 * `getRemoteAddrSessionHash() . $sessionId`, not the raw cookie value
 ...
 * "bind sessions to the client IP?" policy bit is a parameter here
 * because the i.php fast-bootstrap path must source it from the legacy
 * global $conf (CurrentConfig::$data isn't authoritatively populated from
 * local config overrides on that path, only from ConfigLoader's own
 * defaults/env).
 */
