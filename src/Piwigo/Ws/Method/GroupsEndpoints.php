...
final readonly class GroupsEndpoints
{
    ...
    public function add(array $params, PwgServer &$service): mixed
    {
        ...
        $insertedId = $this->groupRepository->insertNew($params['name'], BoolUtil::toInt($isDefaultVal));
    ...
    public function setInfo(array $params, PwgServer &$service): mixed
    {
        ...
        $this->groupRepository->updateById($setinfoGroupId, $updates);
    ...
    public function addUser(array $params, PwgServer &$service): mixed
    {
        ...
        foreach ($userIds as $userId) {
            $inserts[] = ['group_id' => $adduserGroupId, 'user_id' => is_numeric($userId) ? (int) $userId : 0];
        }
    ...
    public function duplicate(array $params, PwgServer &$service): mixed
    {
        ...
        $insertedId = $this->groupRepository->insertNew($copyNameStr, BoolUtil::toInt($isDefault));
        $this->activityLogger->log(new ActivityEvent(ActivityObject::Group, $insertedId, 'add'));
        $users   = $this->groupRepository->findUserIdsByGroupId($dupGroupId);
        $inserts = [];
        ...
        foreach ($users as $userId) {
            $this->activityLogger->log(new ActivityEvent(ActivityObject::User, $userId, 'edit', ['associated' => $dupGroupId]));
        }
        return $service->invoke('pwg.groups.getList', ['group_id' => $insertedId]);
    }
