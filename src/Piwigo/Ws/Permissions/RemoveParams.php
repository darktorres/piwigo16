...
/**
 * `pwg.permissions.remove` input DTO. `cat_id` is mandatory
 * (`FORCE_ARRAY` + `WsParamType::ID`); `group_id`/`user_id` are the same
 * coercion but `OPTIONAL` (empty list when absent, matching
 * `RemoveHandler`'s original `isset($params[...]) && $params[...] !== []`
 * guard); `pwg_token` is mandatory, unchecked type.
 */
