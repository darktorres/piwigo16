...
it('resolves the representative-photo image query and injects an auth_key URL param for a normal-status recipient', function (): void {
    ...
    H::dbQuery($db, sprintf('UPDATE categories SET representative_picture_id = %d WHERE id = %d', $imageId, $albumId));
...
it('populates the private-album permission_url and direct/indirect notified-user queries when at least one group exists', function (): void {
    ...
    H::dbQuery($db, sprintf("UPDATE categories SET status = 'private' WHERE id = %d", $albumId));
    ...
    // empty one.
    H::dbQuery($db, sprintf('INSERT INTO group_access (group_id, cat_id) VALUES (%d, %d)', $groupId, $albumId));
    H::dbQuery($db, sprintf('INSERT INTO user_group (group_id, user_id) VALUES (%d, %d)', $groupId, $indirectUserId));
    ...
    try {
        ...
        H::dbQuery($db, sprintf('DELETE FROM user_group WHERE group_id = %d', $groupId));
        H::dbQuery($db, sprintf('DELETE FROM group_access WHERE group_id = %d', $groupId));
        H::dbQuery($db, sprintf('DELETE FROM user_infos WHERE user_id = %d', $indirectUserId));
        H::dbQuery($db, sprintf('DELETE FROM users WHERE id = %d', $indirectUserId));
        H::dbClose($db);
    }
});
