...
it('clears the default start date when any filter is applied', function (): void {
    ...
    // The ip filter itself is only echoed into the page's own page-data
    // JSON blob (Piwigo\Core\PageState's "Typed-page-data-exposure" P37
    // migration -- history.js reads it back via pwg_getPageData('ip')),
    // not a visible/DOM-attribute value, so a raw-content check is the
    // correct way to confirm it round-tripped.
    expect(H::pageData(H::rawWebpage($page)->content())['ip'] ?? null)->toBe('127.0.0.1');
    $page->assertNoJavaScriptErrors();
});
...
it('echoes a valid image_id filter back into the form', function (): void {
    ...
    expect(H::pageData(H::rawWebpage($page)->content())['image_id'] ?? null)->toBe('42');
});
