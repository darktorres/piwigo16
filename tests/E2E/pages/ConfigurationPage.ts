...
export class ConfigurationPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.submitButton = page.locator('button[type="submit"][name="submit"], input[type="submit"][name="submit"]');
   ...
   */
  async verifySuccess(messageText: string = 'Your configuration settings are saved'): Promise<void> {
  ...
  async getResetButton(): Promise<Locator | null> {
    const resetButton = this.page.locator('a[href*="action=restore_settings"], button:has-text("Restore"), a:has-text("Restore")');
  ...
  async getRestoreButton(): Promise<Locator | null> {
    const restoreButton = this.page.locator('a[href*="action=restore"], button:has-text("Restore"), a:has-text("Restore")');
  ...
  async countFields(name: string): Promise<number> {
    const fields = this.page.locator(`input[name="${name}"], textarea[name="${name}"], select[name="${name}"]`);
