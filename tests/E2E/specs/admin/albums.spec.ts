...
test.describe('Album UI Creation @admin @albums @critical', () => {
  ...
  test('should create a new album via UI @critical', async ({ adminPage }) => {
    ...
    const loginResp = await request.post(`${baseUrl}/ws.php?format=json`, {
      form: { method: 'pwg.session.login', username: TEST_DATA.admin.username, password: TEST_DATA.admin.password }
    });
    expect(loginResp.ok()).toBeTruthy();

    const statusResp = await request.post(`${baseUrl}/ws.php?format=json`, { form: { method: 'pwg.session.getStatus' } });
    ...
    const listResp = await request.post(`${baseUrl}/ws.php?format=json`, { form: { method: 'pwg.categories.getList' } });
    ...
    if (testAlbum) {
      const deleteResp = await request.post(`${baseUrl}/ws.php?format=json`, {
        form: { method: 'pwg.categories.delete', category_id: testAlbum.id, photo_deletion_mode: 'no_delete', pwg_token: token }
      });
