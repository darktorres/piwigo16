...
test.describe('Batch Manager UI @admin @batch', () => {
  ...
  test('should load batch manager page', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global', { waitUntil: 'networkidle' });
  ...
  test('should display photo selection controls', async ({ adminPage }) => {
    ...
    const hasSelection =
      ...
      (await isVisibleSafe(selectAll, 'select all button'));
  ...
  test('should display batch action options', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global', { waitUntil: 'networkidle' });
    const actions = await adminPage.locator('select[name*="action"], [class*="action"], [class*="operation"]');
...
test.describe('Batch Filters and Selection @admin @batch', () => {
  ...
  test('should have all photos filter option', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&filter=all_photos', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should filter photos by category', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&filter=category', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should filter photos by tag', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&filter=tag', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should filter photos by privacy level', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&filter=level', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should filter photos by dimensions', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&filter=dimension', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should filter photos by file size', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&filter=filesize', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should have caddie prefilter', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&prefilter=caddie', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should have favorites prefilter', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&prefilter=favorites', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should have last import prefilter', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&prefilter=last_import', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should have no album prefilter', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&prefilter=no_album', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should have no tag prefilter', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&prefilter=no_tag', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });

  test('should have duplicates prefilter', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=batch_manager&mode=global&prefilter=duplicates', { waitUntil: 'networkidle' });
    expect(response?.status()).toBeGreaterThan(0);
  });
});
...
test.describe('Batch Category Operations @api @batch', () => {
  ...
  test('should associate photos with categories', async ({ request }) => {
    ...
    if (photoJson.stat === 'ok' && catJson.stat === 'ok' &&
        photoJson.result?.images?.length > 0 && catJson.result?.categories?.length > 0) {
