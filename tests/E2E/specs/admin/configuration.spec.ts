...
test.describe('Admin Configuration @admin @config', () => {
  ...
  test('should load configuration main section', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration', { waitUntil: 'networkidle' });
  ...
  test('should have save button for configuration', async ({ adminPage }) => {
    ...
    const submitButton = await adminPage.locator('input[type="submit"], button[type="submit"]').first();
    expect(await submitButton.isVisible()).toBe(true);
  });

  test('should navigate to comments section', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=configuration&section=comments');
    expect(response?.ok()).toBeTruthy();
  });

  test('should navigate to display section', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=configuration&section=display');
    expect(response?.ok()).toBeTruthy();
  });

  test('should navigate to sizes section', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=configuration&section=sizes');
    expect(response?.ok()).toBeTruthy();
  });

  test('should navigate to image quality section', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=configuration&section=quality');
    expect([404, 500]).toContain(response?.status());
  });

  test('should navigate to watermark section', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=configuration&section=watermark');
    expect(response?.ok()).toBeTruthy();
  });

  test('should handle invalid section gracefully', async ({ adminPage }) => {
    const response = await adminPage.goto('/piwigo16/admin.php?page=configuration&section=invalid_section_name');
...
test.describe('Admin Configuration Main Settings @admin @config', () => {
  ...
  test('should display user registration checkbox', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration', { waitUntil: 'domcontentloaded' });
  ...
  test('should have mail configuration options', async ({ adminPage }) => {
    ...
    const hasMailSettings = await isVisibleSafe(obligatoryMail, 'obligatory mail address checkbox');
    if (hasMailSettings) {
  ...
  test('should have rating configuration', async ({ adminPage }) => {
    ...
    if (hasRating) {
      const firstRating = await isVisibleSafe(rating, 'rating option') ? rating : ratingAnon;
...
test.describe('Admin Configuration Comments Section @admin @config', () => {
  ...
  test('should have comment user controls', async ({ adminPage }) => {
    ...
    const formElements = await adminPage.locator('input[type="checkbox"], input[type="text"], select');
...
test.describe('Admin Configuration Display Section @admin @config', () => {
  ...
  test('should have metadata display settings', async ({ adminPage }) => {
    ...
    const allInputs = await adminPage.locator('input[type="checkbox"], input[type="text"], select');
...
test.describe('Admin Configuration Image Sizes @admin @config', () => {
  ...
  test('should load sizes configuration section', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=sizes', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form');
...
test.describe('Admin Configuration Watermark @admin @config', () => {
  ...
  test('should load watermark configuration section', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=watermark', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form');
...
test.describe('Admin Configuration Quality Settings @admin @config', () => {
  ...
  test('should load quality configuration section', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=quality', { waitUntil: 'networkidle' });
