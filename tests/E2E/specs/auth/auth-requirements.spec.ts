...
test.describe('Auth Requirements - Test 1: Fresh Browser (No Auth)', () => {
  ...
  test('profile.php without auth - check redirect behavior', async ({ page }) => {
    ...
    const hasLoginForm = (await page.locator('input[name="username"]').count() > 0) && (await page.locator('input[name="password"]').count() > 0);
    const hasProfileForm = (await page.locator('input[name="mail_address"]').count() > 0) || (await page.locator('input[name="use_new_pwd"]').count() > 0);

    console.log('TEST 1 - Fresh browser, no cookies:');
    console.log('  Final URL:', currentUrl);
    console.log('  Has login form:', hasLoginForm);
    console.log('  Has profile form:', hasProfileForm);
    console.log('  Is on login page:', currentUrl.includes('identification.php') || hasLoginForm);
...
test.describe('Auth Requirements - Test 3: Storage State Loading', () => {
  ...
  test('admin.php with storage state - verify access', async ({ page }) => {
    ...
    const hasAdminContent = bodyText.includes('Albums') || bodyText.includes('Photos') || bodyText.includes('Users');
...
test.describe('Auth Requirements - Test 4: API Session Validation', () => {
  ...
  test('verify admin credentials via API', async ({ request }) => {
    ...
    console.log('  Is admin:', json.result?.status === 'admin' || json.result?.status === 'webmaster');
...
test.describe('Auth Requirements - Test 5: Redirect Detection', () => {
  ...
  test('detect login redirect via URL', async ({ page }) => {
    ...
    const currentUrl = page.url();
    const isLoginPageByUrl = currentUrl.includes('identification.php') || currentUrl.includes('login');
  ...
  test('detect login redirect via locator', async ({ page }) => {
    ...
    const usernameInput = await isVisibleSafe(page.locator('input[name="username"]'), 'username input on login page');
    const passwordInput = await isVisibleSafe(page.locator('input[name="password"]'), 'password input on login page');
...
test.describe('Auth Requirements - Test 6: Authenticated Page Detection', () => {
  ...
  test('detect authenticated via locator methods', async ({ page }) => {
    ...
    const isMailVisible = await isVisibleSafe(mailAddressInput, 'mail address input on authenticated profile page');

    const loginForm = page.locator('input[name="username"]');
    const isLoginVisible = await isVisibleSafe(loginForm, 'login form on authenticated profile page');
...
test.describe('Auth Requirements - Test 7: Authentication Markers', () => {
  ...
  test('identify what elements appear when NOT logged in on profile.php', async ({ browser }) => {
    ...
    try {
      ...
      const markers = {
        hasMailField: (await isolatedPage.locator('input[name="mail_address"]').count() > 0),
        hasPasswordField: (await isolatedPage.locator('input[name="use_new_pwd"]').count() > 0),
        hasLoginForm: (await isolatedPage.locator('input[name="username"]').count() > 0) && (await isolatedPage.locator('input[name="password"]').count() > 0),
        hasLoginButton: (await isolatedPage.locator('input[type="submit"]').count() > 0) && (bodyText.includes('Log in') || bodyText.includes('login') || bodyText.includes('Connexion')),
      };
