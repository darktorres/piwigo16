...
test.describe('Configuration Email Settings @critical @admin @config', () => {
  ...
  test('should load email configuration settings', async ({ adminPage }) => {
    await adminPage.goto(`${baseUrl}/admin.php?page=configuration&section=main`, { waitUntil: 'domcontentloaded' });
  ...
  test('should change mail theme and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const submitBtn = adminPage.locator('button[type="submit"][name="submit"], input[type="submit"][name="submit"]');
      ...
      const persistedValue = await adminPage.locator('select[name="mail_theme"]').inputValue();
      expect(persistedValue).toBe(newValue);
    } finally {
      // Restore original
      await adminPage.locator('select[name="mail_theme"]').selectOption(originalValue);
      await adminPage.locator('button[type="submit"][name="submit"], input[type="submit"][name="submit"]').click();
    }
  });
  ...
  test('should toggle obligatory user mail address and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const newState = await adminPage.locator('input[name="obligatory_user_mail_address"]').isChecked();
      expect(newState).toBe(!originalState);
    } finally {
      // Restore original
      const current = await adminPage.locator('input[name="obligatory_user_mail_address"]').isChecked();
  ...
  test('should configure email notification on new user registration', async ({ adminPage }) => {
    ...
    try {
      ...
      const newState = await adminPage.locator('input[name="email_admin_on_new_user"]').isChecked();
      expect(newState).toBe(!originalState);
    } finally {
      // Restore original
      const current = await adminPage.locator('input[name="email_admin_on_new_user"]').isChecked();
      if (current !== originalState) {
  ...
  test('should configure week start day and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const persistedValue = await adminPage.locator('select[name="week_starts_on"]').inputValue();
      expect(persistedValue).toBe(newValue);
    } finally {
      // Restore original
...
test.describe('Configuration Comment Email Notifications @critical @admin @config', () => {
  ...
  test('should toggle email notification on new comment and persist', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=comments', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form[method="post"]');
    ...
    try {
      ...
      const newState = await adminPage.locator('input[name="email_admin_on_comment"]').isChecked();
      expect(newState).toBe(!originalState);
    } finally {
      // Restore original
      const current = await adminPage.locator('input[name="email_admin_on_comment"]').isChecked();
      if (current !== originalState) {
  ...
  test('should toggle email notification on comment validation and persist', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=comments', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form[method="post"]');
    ...
    try {
      ...
      const newState = await adminPage.locator('input[name="email_admin_on_comment_validation"]').isChecked();
      expect(newState).toBe(!originalState);
    } finally {
      // Restore original
      const current = await adminPage.locator('input[name="email_admin_on_comment_validation"]').isChecked();
      if (current !== originalState) {
  ...
  test('should toggle email notification on comment edition and persist', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=comments', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form[method="post"]');
    ...
    try {
      ...
      const newState = await adminPage.locator('input[name="email_admin_on_comment_edition"]').isChecked();
      expect(newState).toBe(!originalState);
    } finally {
      // Restore original
      const current = await adminPage.locator('input[name="email_admin_on_comment_edition"]').isChecked();
      if (current !== originalState) {
  ...
  test('should toggle email notification on comment deletion and persist', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=comments', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form[method="post"]');
    ...
    try {
      ...
      const newState = await adminPage.locator('input[name="email_admin_on_comment_deletion"]').isChecked();
      expect(newState).toBe(!originalState);
    } finally {
      // Restore original
      const current = await adminPage.locator('input[name="email_admin_on_comment_deletion"]').isChecked();
      if (current !== originalState) {
  ...
  test('should configure all comment notification options together', async ({ adminPage }) => {
    ...
    const emailOnValidation = adminPage.locator('input[name="email_admin_on_comment_validation"]');
    ...
    const originalValidationState = await isVisibleSafe(emailOnValidation) ? await emailOnValidation.isChecked() : false;
    const originalEditionState = await isVisibleSafe(emailOnEdition) ? await emailOnEdition.isChecked() : false;
    const originalDeletionState = await isVisibleSafe(emailOnDeletion) ? await emailOnDeletion.isChecked() : false;
    ...
    try {
      ...
      if (await isVisibleSafe(emailOnValidation) && !(await emailOnValidation.isChecked())) {
      ...
      expect(await adminPage.locator('input[name="email_admin_on_comment"]').isChecked()).toBeTruthy();
    } finally {
      // Restore original states
      const currentComment = await adminPage.locator('input[name="email_admin_on_comment"]').isChecked();
...
test.describe('Configuration Display and Default Settings @admin @config', () => {
  ...
  test('should load display configuration section', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=display', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form[method="post"]');
  ...
  test('should configure default guest/user settings', async ({ adminPage }) => {
    await adminPage.goto('/piwigo16/admin.php?page=configuration&section=default', { waitUntil: 'domcontentloaded' });
    await adminPage.waitForSelector('form[method="post"]');
