...
test.describe('Configuration General Settings @critical @admin @config', () => {
  ...
  test('should verify registration form appears when enabled', async ({ adminPage, adminContext }) => {
    const configPage = new ConfigurationPage(adminPage);
    await configPage.navigateToSection('main');
    ...
    try {
      ...
      await registrationPage.goto('/piwigo16/register.php', { waitUntil: 'domcontentloaded' });
  ...
  test('should require PWG token for form submission (CSRF protection)', async ({ adminPage }) => {
    const configPage = new ConfigurationPage(adminPage);
    await configPage.navigateToSection('main');
  ...
  test('should allow webmaster only to edit configuration', async ({ adminPage }) => {
    ...
    const hasWebmasterWarning = bodyText.includes('webmaster') && bodyText.includes('status is required');
