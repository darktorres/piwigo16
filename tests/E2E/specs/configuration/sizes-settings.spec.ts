...
test.describe('Configuration Sizes Settings @critical @admin @config', () => {
  ...
  test('should load sizes configuration section', async ({ adminPage }) => {
    await adminPage.goto(`${baseUrl}/admin.php?page=configuration&section=sizes`, { waitUntil: 'domcontentloaded' });
  ...
  test('should display all derivative size types', async ({ adminPage }) => {
    ...
    const sizeTypes = ['square', 'thumb', 'xxsmall', 'xsmall', 'small', 'medium', 'large', 'xlarge', 'xxlarge'];
  ...
  test('should change square thumbnail size and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const persistedValue = await adminPage.locator('input[name="d[square][w]"]').inputValue();
  ...
  test('should change medium image size and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const persistedWidth = await adminPage.locator('input[name="d[medium][w]"]').inputValue();
      const persistedHeight = await adminPage.locator('input[name="d[medium][h]"]').inputValue();
  ...
  test('should change large image size and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const persistedWidth = await adminPage.locator('input[name="d[large][w]"]').inputValue();
      const persistedHeight = await adminPage.locator('input[name="d[large][h]"]').inputValue();
  ...
  test('should change quality setting and persist', async ({ adminPage }) => {
    ...
    try {
      ...
      const persistedValue = await adminPage.locator('input[name="resize_quality"]').inputValue();
      expect(persistedValue).toBe(newValue);
    } finally {
      // Restore original
      await adminPage.locator('input[name="resize_quality"]').fill(originalValue);
  ...
  test('should have restore default settings button', async ({ adminPage }) => {
    ...
    const restoreLink = adminPage.locator('a[href*="action=restore_settings"], button:has-text("Restore"), a:has-text("Restore")');
...
test.describe('Configuration Sizes - New Upload Integration @critical @admin @config @upload @slow', () => {
  test('should apply new size settings to newly uploaded images', async ({ adminPage }) => {
    ...
    try {
      ...
      const imageId = await uploadImage(request, albumId, token, imagePath, 'Size Test Image');
