...
import { baseUrl, loginAndGetToken, createTestAlbum, uploadImage, getFixturePath } from '@helpers/api-helpers';
...
test.describe('Configuration Watermark - Image Upload Integration @critical @admin @config @upload @slow', () => {
  test('should apply watermark to newly uploaded images when enabled', async ({ adminPage }) => {
    ...
    try {
      ...
      const imageId = await uploadImage(request, albumId, token, imagePath, 'Watermark Test Image');
  ...
  test('should not apply watermark to new images when disabled', async ({ adminPage }) => {
    ...
    try {
      ...
      const imageId = await uploadImage(request, albumId, token, imagePath, 'No Watermark Test Image');
