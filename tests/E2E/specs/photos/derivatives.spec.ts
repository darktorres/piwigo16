...
test.describe('Photo Derivatives @admin @photos @derivatives', () => {
  test('should get derivative URLs for uploaded photo', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `DerivUrl_Test_${Date.now()}`);
  ...
  test('should have thumbnail derivative accessible', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `ThumbAccess_Test_${Date.now()}`);
    ...
    try {
      ...
      const info = await infoResp.json();
      const thumbUrl = info.result.derivatives?.thumb?.url || info.result.derivatives?.square?.url;
  ...
  test('should have different sizes for different derivatives', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `DerivSizes_Test_${Date.now()}`);
...
test.describe('Photo Derivative Quality @admin @photos @derivatives @quality', () => {
  test('should serve derivatives with correct content type', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `DerivContent_Test_${Date.now()}`);
  ...
  test('should handle small source image derivatives correctly', async ({ request }) => {
    // Create album
    const { id: albumId, token } = await createTestAlbum(request, `SmallSource_Test_${Date.now()}`);
...
test.describe('Photo Derivative URLs @admin @photos @derivatives @urls', () => {
  test('should have consistent derivative URL structure', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `DerivUrlStruct_Test_${Date.now()}`);
  ...
  test('should access original image URL', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `OriginalUrl_Test_${Date.now()}`);
