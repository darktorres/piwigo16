...
test.describe('Batch Manager Page @admin @photos @batch', () => {
  ...
  test('should display batch manager page @smoke', async ({ adminPage, request }) => {
    const result = await createTestAlbumWithPhotos(request, `BatchManager_Smoke_${Date.now()}`, 3);
    const { albumId, token } = result;

    try {
      const batchPage = new BatchManagerPage(adminPage);
      await adminPage.goto(`${baseUrl}/admin.php?page=batch_manager&filter=category-${albumId}`, { waitUntil: 'domcontentloaded' });
  ...
  test('should display filter section', async ({ adminPage, request }) => {
    const result = await createTestAlbumWithPhotos(request, `BatchFilter_Test_${Date.now()}`, 3);
    const { albumId, token } = result;

    try {
      await adminPage.goto(`${baseUrl}/admin.php?page=batch_manager&filter=category-${albumId}`, { waitUntil: 'domcontentloaded' });
      // Check for filter section elements
      const filterSection = adminPage.locator('.filterBlock, fieldset:has(legend), #filterList, [id*="filter"]');
  ...
  test('should display thumbnails for photos in album', async ({ adminPage, request }) => {
    const result = await createTestAlbumWithPhotos(request, `BatchThumbs_Test_${Date.now()}`, 3);
    const { albumId, token } = result;
    ...
    try {
      ...
      const thumbnailContainers = adminPage.locator('.thumbnails, ul.thumbnails, .thumbnail-container, .wrap1');
      const thumbnailItems = adminPage.locator('.thumbnails .wrap1, .thumbnail-item, li.wrap1, li[id*="cat_"], div[id*="cat_"]');

      // Wait for at least one thumbnail container to become visible (with timeout)
      await waitForSafe(thumbnailContainers.first(), { state: 'visible', timeout: TIMEOUTS.element.quick }, 'thumbnail container');
      ...
      const hasContainer = await isVisibleSafe(thumbnailContainers.first(), 'thumbnail container');
  ...
  test('should display action select dropdown when photos selected', async ({ adminPage, request }) => {
    const result = await createTestAlbumWithPhotos(request, `BatchAction_Test_${Date.now()}`, 3);
    const { albumId, token } = result;
    ...
    try {
      ...
      if (await selectAllLink.isVisible()) {
        ...
        await waitForSafe(actionSelect, { state: 'visible', timeout: TIMEOUTS.element.quick }, 'action select');
  ...
  test('should have selection controls', async ({ adminPage, request }) => {
    const result = await createTestAlbumWithPhotos(request, `BatchSelect_Test_${Date.now()}`, 3);
    const { albumId, token } = result;
...
test.describe.serial('Photo Delete Operations @admin @photos @delete @api', () => {
  test('should delete single photo via API', async ({ request }) => {
    const { albumId, imageIds, token } = await createTestAlbumWithPhotos(request, `DeleteSingle_Test_${Date.now()}`, 1);
    ...
    try {
      ...
      await safeCleanup(
        () => deleteTestAlbum(request, albumId, token),
        `album ${albumId}`
      );
    }
  });
  ...
  test('should delete multiple photos via API', async ({ request }) => {
    const { albumId, imageIds, token } = await createTestAlbumWithPhotos(request, `DeleteMulti_Test_${Date.now()}`, 3);
...
test.describe.serial('Photo Move Operations @admin @photos @move @api', () => {
  test('should move photo to different album via API', async ({ request }) => {
    ...
    const { albumId: sourceAlbumId, imageIds, token } = await createTestAlbumWithPhotos(
      request,
      `MoveOp1_${Date.now()}`,
      1
    );
    ...
    try {
      ...
      const photoInTarget = targetImages.result.images.some((img: any) => String(img.id) === String(imageId));
      ...
      const photoInSource = sourceImages.result.images.some((img: any) => String(img.id) === String(imageId));
  ...
  test('should copy photo to another album via API', async ({ request }) => {
    ...
    try {
      ...
      const inSource = sourceImages.result.images.some((img: any) => String(img.id) === String(imageId));
      ...
      const inTarget = targetImages.result.images.some((img: any) => String(img.id) === String(imageId));
  ...
  test('should dissociate photo from album via API', async ({ request }) => {
    ...
    const { albumId: album1Id, imageIds, token } = await createTestAlbumWithPhotos(
      request,
      `DissocOp3_${Date.now()}`,
      1
    );
    ...
    try {
      ...
      expect((await album2CheckResp.json()).result.images.some((img: any) => String(img.id) === String(imageId))).toBe(true);
      ...
      expect(album1Images.result.images.some((img: any) => String(img.id) === String(imageId))).toBe(false);
      ...
      expect(album2Images.result.images.some((img: any) => String(img.id) === String(imageId))).toBe(true);

    } finally {
      // Cleanup with retry logic - prevents orphaned test data
      await safeCleanup(
        () => deleteTestAlbum(request, album1Id, token),
        `album ${album1Id}`
      );
      await safeCleanup(
        () => deleteTestAlbum(request, album2Id, token),
        `album ${album2Id}`
      );
    }
  });
