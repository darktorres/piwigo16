...
test.describe('Photo Metadata Edit Page @admin @photos @metadata', () => {
  ...
  test('should display description field', async ({ adminPage, request }) => {
    ...
    try {
      ...
      await adminPage.waitForSelector('form#pictureModify, form[name="pictureModify"]', { timeout: TIMEOUTS.element.standard });
      // Try multiple possible selectors for the description field
      const descField = adminPage.locator('textarea[name="comment"], textarea.description, #photo-comment');
  ...
  test('should display privacy level select', async ({ adminPage, request }) => {
    ...
    try {
      ...
      await adminPage.waitForSelector('form#pictureModify, form[name="pictureModify"]', { timeout: TIMEOUTS.element.standard });
      await expect(photoEditPage.privacyLevelSelect).toBeVisible({ timeout: TIMEOUTS.element.standard });
    } finally {
      await safeCleanup(() => deleteTestAlbum(request, albumId, token), `album ${albumId}`);
    }
  });
  ...
  test('should display submit button', async ({ adminPage, request }) => {
    ...
    try {
      ...
      const submitBtn = adminPage.locator('input[type="submit"], button[type="submit"], input[name="submit"]').first();
...
test.describe.serial('Photo Metadata API Operations @admin @photos @metadata @api', () => {
  test('should update photo title via API', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `TitleUpdate_Test_${Date.now()}`);
  ...
  test('should update photo author via API', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `AuthorUpdate_Test_${Date.now()}`);
  ...
  test('should update photo description via API', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `DescUpdate_Test_${Date.now()}`);
  ...
  test('should update photo privacy level via API', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `PrivacyUpdate_Test_${Date.now()}`);
  ...
  test('should update multiple metadata fields at once via API', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `MultiUpdate_Test_${Date.now()}`);
  ...
  test('should get photo info via API', async ({ request }) => {
    const { albumId, imageId, token } = await createTestAlbumWithPhoto(request, `GetInfo_Test_${Date.now()}`);
...
test.describe.serial('Photo Album Association @admin @photos @metadata @association', () => {
  test('should associate photo with multiple albums via API', async ({ request }) => {
    ...
    const { albumId: album1Id, imageId, token } = await createTestAlbumWithPhoto(
      request,
      `MultiAlbum1_${Date.now()}`
    );
    ...
    try {
      ...
      const imageInAlbum2 = album2Images.result.images.some((img: any) => String(img.id) === String(imageId));
