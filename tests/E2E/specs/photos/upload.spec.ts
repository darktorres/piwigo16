...
import { baseUrl, loginAndGetToken, createTestAlbum, uploadImage, deleteTestAlbum, getFixturePath } from '@helpers/api-helpers';
...
test.describe('Photo Upload Page @admin @photos @upload', () => {
  ...
  test('should display add files button', async ({ adminPage }) => {
    ...
    if (isDisabled !== null) {
      ...
      const createAlbumPrompt = adminPage.locator('text=Piwigo requires an album, text=Create a first album');
      const hasPrompt = await isVisibleSafe(createAlbumPrompt.first(), 'create album prompt on upload page');
  ...
  test('should display album selection controls', async ({ adminPage }) => {
    ...
    const matchedAlbumSelector = await waitForMultipleSelectors(adminPage, albumSelectors, ELEMENT_TIMEOUTS.quick);
    if (matchedAlbumSelector) {
      console.log(`Album selector found: ${matchedAlbumSelector}`);
    }

    // At least one of these should be present
    const hasAlbumControl = selectButton || createButton || matchedAlbumSelector !== null || albumNameDisplay;
...
test.describe.serial('Photo Upload API @admin @photos @upload @api', () => {
  test('should upload photo via API', async ({ request }) => {
    ...
    try {
      ...
      const imageId = await uploadImage(request, albumId, token, imagePath, 'Test Upload Image');
  ...
  test('should upload multiple photos via API', async ({ request }) => {
    ...
    try {
      ...
      for (let i = 0; i < testImages.length; i++) {
        ...
        const imageId = await uploadImage(request, albumId, token, imagePath, `Test Image ${i + 1}`);
  ...
  test('should upload photo with metadata via API', async ({ request }) => {
    ...
    try {
      const imagePath = path.join(__dirname, '../../fixtures/images/test-image-1.jpg');
      const imageId = await uploadImage(request, albumId, token, imagePath, 'Photo With Metadata', {
  ...
  test('should upload photo with tags via API', async ({ request }) => {
    ...
    try {
      const imagePath = path.join(__dirname, '../../fixtures/images/test-image-1.jpg');
      const imageId = await uploadImage(request, albumId, token, imagePath, 'Photo With Tags', {
...
test.describe('Photo Upload Validation @admin @photos @upload @validation', () => {
  test('should reject upload without album via API', async ({ request }) => {
    ...
    const uploadResponse = await request.post(`${baseUrl}/ws.php?format=json&method=pwg.images.addSimple`, {
      multipart: {
        name: 'No Album Photo',
        pwg_token: token,
        image: {
  ...
  test('should handle duplicate upload via API', async ({ request }) => {
    ...
    try {
      ...
      const secondId = await uploadImage(request, albumId, token, imagePath, 'Duplicate Test 2');
