...
test.describe('Change Password - Success Cases @critical @auth', () => {
  ...
  test('should change password with correct old password @critical', async ({ adminPage, browser }) => {
  ...
  test('should verify password change persists across sessions', async ({ adminPage, browser }) => {
    const profilePage = new ProfilePage(adminPage);
    await profilePage.goto();
  ...
  test('should leave password fields empty after successful change', async ({ adminPage }) => {
    ...
    const errorElement = adminPage.locator('.error, .errors').first();
    const hasError = await isVisibleSafe(errorElement, 'error message in password field clearing test');
    ...
    if (!hasError) {
      ...
      const confirmPasswordValue = await (await profilePage.getPasswordConfirm()).inputValue();
...
test.describe('Change Password - Error Cases @critical @auth', () => {
  ...
  test('should reject incorrect old password @critical', async ({ adminPage }) => {
    ...
    const errorElement = adminPage.locator('.error, .errors, [role="alert"], text=wrong, text=incorrect, text=current password').first();
  ...
  test('should reject password mismatch @critical', async ({ adminPage }) => {
    ...
    const errorElement = adminPage.locator('.error, .errors, [role="alert"], text=match, text=same, text=confirmation').first();
    await expect(errorElement).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should not change password if only current password filled', async ({ adminPage, browser }) => {
    const profilePage = new ProfilePage(adminPage);
    await profilePage.goto();
...
test.describe('Change Password - Security @critical @auth', () => {
  ...
  test('should invalidate existing sessions after password change', async ({ adminPage, browser }) => {
    const profilePage = new ProfilePage(adminPage);
    ...
    const errorElement = adminPage.locator('.error, .errors').first();
    const hasError = await isVisibleSafe(errorElement, 'error element in session invalidation test');
...
test.describe('Change Password - Edge Cases @auth', () => {
  ...
  test('should handle special characters in password', async ({ adminPage }) => {
    ...
    const errorElement = adminPage.locator('.error, .errors').first();
    const hasError = await isVisibleSafe(errorElement, 'error element in special password test');
  ...
  test('should handle unicode characters in password', async ({ adminPage }) => {
    ...
    const errorElement = adminPage.locator('.error, .errors').first();
    const hasError = await isVisibleSafe(errorElement, 'error element in unicode password test');
