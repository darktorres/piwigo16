...
test.describe('Profile Page - View Profile @critical @auth', () => {
  ...
  test('should display preferences section', async ({ adminPage }) => {
    ...
    const isVisible = await isVisibleSafe(preferencesSection, 'preferences section heading on profile page');
  ...
  test('should redirect to login when not authenticated', async ({ browser }) => {
    ...
    try {
      ...
      await isolatedPage.waitForURL(/identification\.php|login|profile\.php/, { timeout: 5000 });
