...
function c13yInternalTestCheckIntegrity(): CheckIntegrity
{
    ...
    return new CheckIntegrity(LangTestFactory::get(), $repo, new Translator(CurrentConfigTestFactory::get(), new TranslationsCachePool(CacheFactory::create(namespace: 'piwigo.translations'))), EventDispatcherTestFactory::get(), PageStateTestFactory::get(), LayoutStateTestFactory::get());
}
...
test('c13yUser flags a configured webmaster_id that has no matching user row, and c13yCorrectionUser creates it', function (): void {
    ...
    expect($anomaly->correctionFct)
        ->toBeInstanceOf(Closure::class);
    expect($anomaly->isCallable)
        ->toBeTrue();
    expect($anomaly->correctionFctArgs)
        ->toBe([
            'id' => 999999,
            'action' => 'creation',
        ]);
    ...
    try {
        ...
        // now genuinely runs.
        $correctionFct = $anomaly->correctionFct;
        $correctionFctArgs = $anomaly->correctionFctArgs;
...
test('c13yUser flags a real user whose status does not match the expected one, and c13yCorrectionUser fixes it', function (): void {
    ...
    try {
        ...
        foreach ($c13y->retrieve_list as $anomaly) {
            if (($anomaly->correctionFctArgs['id'] ?? null) === 1) {
        ...
        expect($webmasterAnomaly->correctionFctArgs)
            ->toBe([
                'id' => 1,
                'action' => 'status',
            ]);
        ...
        // same way CheckIntegrity::check()'s own correction-running pass
        // does.
        $correctionFct = $webmasterAnomaly->correctionFct;
        $correctionFctArgs = $webmasterAnomaly->correctionFctArgs;
        if (! is_callable($correctionFct) || ! is_array($correctionFctArgs)) {
            throw new RuntimeException('Expected a callable correction_fct with array args');
        }
        $result = call_user_func_array($correctionFct, $correctionFctArgs);
