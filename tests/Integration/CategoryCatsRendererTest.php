...
final class CategoryCatsRendererTest extends IntegrationTestCase
{
    ...
    public function testRenderSkipsACategoryPresentInAStaleCachedTreeButDeletedFromTheDb(): void
    {
        $this->seedUser();
    ...
    public function testRenderPicksARandomRepresentativeWhenAllowRandomRepresentativeIsEnabled(): void
    {
        $this->seedUser();
        $currentConfig = Kernel::container()->get(CurrentConfig::class);
        if (! $currentConfig instanceof CurrentConfig) {
    ...
    public function testRenderPicksARandomRepresentativeAmongSubcategoriesWhenNoneIsSetDirectly(): void
    {
        $this->seedUser();
    ...
    public function testRenderSkipsACategoryWithNoResolvableRepresentative(): void
    {
        $this->seedUser();
        ...
        try {
            $result = $this->renderer->render(Section::Categories, CategoryInfoTestFactory::build(id: 1), 0);
    ...
    public function testRenderShowsTheDateRangeWhenDisplayFromtoIsEnabled(): void
    {
        $this->seedUser();
        $currentConfig = Kernel::container()->get(CurrentConfig::class);
        if (! $currentConfig instanceof CurrentConfig) {
    ...
    public function testRenderSubstitutesARepresentativePhotoWhosePrivacyLevelIsTooHigh(): void
    {
    ...
    public function testRenderCachesANullRepresentativeWhenNoPrivacySafeSubstituteExists(): void
    {
        // Both of category 2's own images (4 and 5) excluded this time --
        // no substitute exists anywhere in the category.
        $this->seedUser([
            'image_access_list' => '4,5',
        ]);
        $this->conn->executeStatement('UPDATE images SET level = 5 WHERE id = 4');
    ...
    public function testRenderSkipsACategoryWhoseFilteredCountImagesBecomesZero(): void
    {
        $this->seedUser();
