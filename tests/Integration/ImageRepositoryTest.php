...
final class ImageRepositoryTest extends IntegrationTestCase
{
    ...
    /**
     * Real happy-path coverage -- until now the only test exercising
     * massInsertFormats() was the empty-array no-op above. Covers a
     * varying `filesize` (including `null`) across 2 rows/2 images in one
     * call, and that each gets a real auto-generated `format_id`.
     */
    public function testMassInsertFormatsInsertsEveryRowWithARealAutoGeneratedFormatId(): void
    {
        $this->repo->massInsertFormats([
            [
                'image_id' => 1,
        ...
        $rows = $this->conn->createQueryBuilder()
            ->select('image_id', 'ext', 'filesize')
            ->from('image_format')
            ->where("ext IN ('p18-fmt-a', 'p18-fmt-b')")
        ...
        $formatId = $this->conn->createQueryBuilder()
            ->select('format_id')
            ->from('image_format')
            ->where("ext = 'p18-fmt-a'")
            ->executeQuery()
        ...
        self::assertGreaterThan(0, (int) $formatId);
    }

    /**
     * Regression proof for the batching itself: a future accidental
     * revert of massInsertFormats() to a per-row persist()/flush() loop
     * would still pass the correctness test above (still functionally
     * correct, just slow) -- this fails immediately instead, by counting
     * real round trips via a `Doctrine\DBAL\Logging\Middleware`-wrapped
     * connection. Uses real fixture image id 1 (multiple format rows per
     * image are a normal shape, `ext` is the differentiator) since this
     * method has no `ignore` option -- an invalid `image_id` would throw,
     * not silently skip.
     */
    public function testMassInsertFormatsIssuesOneStatementNotOnePerRow(): void
    {
        ...
        $loggedRepo = TypedRepository::narrow(EntityManagerFactory::build($loggedConn)->getRepository(ImageEntity::class), ImageRepository::class);

        try {
            $loggedRepo->massInsertFormats([
                [
                    'image_id' => 1,
                    'ext' => 'p18-count-a',
                    'filesize' => 1,
            ...
            $loggedConn->executeStatement("DELETE FROM image_format WHERE ext IN ('p18-count-a', 'p18-count-b', 'p18-count-c')");
            $loggedConn->close();
        }
    }
