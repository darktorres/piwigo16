...
test('fromArrays returns defaults for an empty GET/POST/REQUEST', function (): void {
    ...
    expect($request->requestMethod)
        ->toBeNull()
        ->and($request->requestAction)
        ->toBeNull()
        ->and($request->pageParam)
        ->toBeNull()
        ->and($request->destinationTagPresent)
        ->toBeFalse()
        ->and($request->destinationTag)
        ->toBeNull();
});
...
test('fromArrays reads the ws method and action from REQUEST', function (): void {
    ...
    expect($request->requestMethod)
        ->toBe('pwg.plugins.performAction')
        ->and($request->requestAction)
        ->toBe('restore');
});
