...
test('fromArray narrows both bounds to float', function (): void {
    ...
    $filter = FilesizeFilter::fromArray(['min' => '10', 'max' => '500.5']);

    expect($filter->min)->toBe(10.0)
        ->and($filter->max)->toBe(500.5);
});

test('fromArray defaults both bounds to null when absent or non-numeric', function (): void {
    $filter = FilesizeFilter::fromArray(['min' => 'not-a-number']);

    expect($filter->min)->toBeNull()
        ->and($filter->max)->toBeNull();
});

test('isEmpty is true only when both bounds are null', function (): void {
    expect(new FilesizeFilter()->isEmpty())->toBeTrue();
    expect(new FilesizeFilter(min: 10.0)->isEmpty())->toBeFalse();
    expect(new FilesizeFilter(max: 500.0)->isEmpty())->toBeFalse();
});
