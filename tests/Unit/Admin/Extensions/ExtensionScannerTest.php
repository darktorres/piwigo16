...
test('scan finds the real bundled en_UK language via its common.po header', function (): void {
    $found = new ExtensionScanner()->scan(ExtensionType::Language, UrlServiceTestFactory::build(), Lang::current(), CurrentPaths::get(), CurrentUser::current(), EventDispatcher::get(), CurrentConfig::current(), 'utf-8');
...
test('scan skips a plugin directory with no main.inc.php', function (): void {
    ...
    try {
        ...
        $found = new ExtensionScanner()->scan(ExtensionType::Plugin, UrlServiceTestFactory::build(), Lang::current(), CurrentPaths::get(), CurrentUser::current(), EventDispatcher::get(), CurrentConfig::current());
...
test('scan trims trailing whitespace from every regex-captured theme header value', function (): void {
    ...
    try {
        ...
        $found = new ExtensionScanner()->scan(ExtensionType::Theme, UrlServiceTestFactory::build(), Lang::current(), CurrentPaths::get(), CurrentUser::current(), EventDispatcher::get(), CurrentConfig::current());
