...
test('toArray flattens every property to its real Smarty template variable name', function (): void {
    $context = new UserActivityPageContext(
        ...
        pwgToken: 'abc123',
        inherit: true,
        cacheKeys: ['users' => 'def456'],
        ulist: [['id' => 1, 'username' => 'jane', 'nb_lines' => 5]],
        nbUsers: 3,
        activityDates: ['min' => '2026-01-01', 'max' => '2026-08-08'],
        additionalFiltType: 'photo',
        additionalFiltName: 'sunset.jpg',
        additionalFiltValue: '42',
        actions: [['object' => 'image', 'action' => 'add', 'counter' => 2, 'value' => 'image/add']],
    );
    ...
    expect($context->toArray())->toBe([
        ...
        'PWG_TOKEN' => 'abc123',
        'INHERIT' => true,
        'CACHE_KEYS' => ['users' => 'def456'],
        'ulist' => [['id' => 1, 'username' => 'jane', 'nb_lines' => 5]],
        'nb_users' => 3,
        'ACTIVITY_DATES' => ['min' => '2026-01-01', 'max' => '2026-08-08'],
        'ADDITIONAL_FILT' => ['type' => 'photo', 'name' => 'sunset.jpg', 'value' => '42'],
        'ACTIONS' => [['object' => 'image', 'action' => 'add', 'counter' => 2, 'value' => 'image/add']],
    ]);
});
...
test('toArray reflects a no-filter state (type=false, name/value=null)', function (): void {
    ...
    expect($context->toArray()['ADDITIONAL_FILT'])->toBe(['type' => false, 'name' => null, 'value' => null]);
});
