...
test('fromArrays returns defaults for an empty GET/POST', function (): void {
    ...
    expect($request->isSubmitted)
        ->toBeFalse()
        ->and($request->selectAction)
        ->toBe('')
        ->and($request->nbPhotosDeletedPresent)
        ->toBeFalse()
        ->and($request->nbPhotosDeleted)
        ->toBe(0)
        ->and($request->isSetSelected)
        ->toBeFalse()
        ->and($request->wholeSet)
        ->toBe('')
        ->and($request->selectionPresent)
        ->toBeFalse()
        ...
        ->and($request->page)
        ->toBe('')
        ->and($request->displayRequested)
        ->toBeFalse()
        ->and($request->displayRaw)
        ->toBeNull();
});
...
test('fromArrays parses nb_photos_deleted', function (): void {
    ...
    expect($request->nbPhotosDeletedPresent)
        ->toBeTrue()
        ->and($request->nbPhotosDeleted)
        ->toBe(5);
});
...
test('fromArrays rejects a non-digit nb_photos_deleted', function (): void {
    expect(fn (): BatchManagerGlobalRequest => BatchManagerGlobalRequest::fromArrays([], [
        'nb_photos_deleted' => 'abc',
    ], new InputValidator()))
...
test('fromArrays exposes the raw selection array', function (): void {
    ...
    expect($request->selectionPresent)
        ->toBeTrue()
        ->and($request->selection)
        ->toBe(['1', '2']);
});
...
test('fromArrays treats an absent or zero display as not requested', function (): void {
    ...
    $zero = BatchManagerGlobalRequest::fromArrays([
        'display' => '0',
    ], [], new InputValidator());

    expect($absent->displayRequested)
        ->toBeFalse()
        ->and($zero->displayRequested)
        ->toBeFalse();
});

test('fromArrays keeps a requested display value, including "all"', function (): void {
    $numeric = BatchManagerGlobalRequest::fromArrays([
        'display' => '50',
    ], [], new InputValidator());
    $all = BatchManagerGlobalRequest::fromArrays([
        'display' => 'all',
    ], [], new InputValidator());

    expect($numeric->displayRequested)
        ->toBeTrue()
        ->and($numeric->displayRaw)
        ->toBe('50')
        ->and($all->displayRequested)
        ->toBeTrue()
        ->and($all->displayRaw)
        ->toBe('all');
});
...
test('fromArrays treats a present but empty-string display as not requested', function (): void {
    ...
    expect($request->displayRequested)
        ->toBeFalse();
});
