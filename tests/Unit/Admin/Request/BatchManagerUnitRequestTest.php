...
test('fromArrays rejects a malformed element_ids', function (): void {
    expect(fn (): BatchManagerUnitRequest => BatchManagerUnitRequest::fromArrays([], [
        'submit' => '1',
        'element_ids' => '1; DROP TABLE',
    ], new InputValidator()))
...
test('fromArrays rejects a malformed nb_photos_deleted', function (): void {
    expect(fn (): BatchManagerUnitRequest => BatchManagerUnitRequest::fromArrays([], [
        'nb_photos_deleted' => '1; DROP TABLE',
    ], new InputValidator()))
        ->toThrow(RuntimeException::class);
});
...
test('fromArrays treats display=0 and display= as not requested', function (): void {
    expect(BatchManagerUnitRequest::fromArrays([
        'display' => '0',
    ], [], new InputValidator())->displayRequested)->toBeFalse()
        ->and(BatchManagerUnitRequest::fromArrays([
            'display' => '',
        ], [], new InputValidator())->displayRequested)->toBeFalse();
});
