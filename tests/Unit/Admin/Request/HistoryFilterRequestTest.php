...
test('fromArray passes ip/image_id straight through', function (): void {
    $request = HistoryFilterRequest::fromArray(['filter_ip' => '192.168.1.1', 'filter_image_id' => '42'], new InputValidator());
...
test('fromArray parses a numeric filter_user_id', function (): void {
    $request = HistoryFilterRequest::fromArray(['filter_user_id' => '7'], new InputValidator());
...
test('fromArray rejects a malformed filter_ip', function (): void {
    expect(fn (): HistoryFilterRequest => HistoryFilterRequest::fromArray(['filter_ip' => 'not-an-ip'], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArray rejects a malformed filter_image_id', function (): void {
    expect(fn (): HistoryFilterRequest => HistoryFilterRequest::fromArray(['filter_image_id' => 'abc'], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArray rejects a malformed filter_user_id', function (): void {
    expect(fn (): HistoryFilterRequest => HistoryFilterRequest::fromArray(['filter_user_id' => 'abc'], new InputValidator()))
        ->toThrow(RuntimeException::class);
});
...
test('fromArray defaults userId to -1 when filter_user_id is present but empty', function (): void {
    ...
    $request = HistoryFilterRequest::fromArray(['filter_user_id' => ''], new InputValidator());
...
test('fromArray reports hasAnyFilter true when only filter_ip is present', function (): void {
    ...
    $request = HistoryFilterRequest::fromArray(['filter_ip' => '192.168.1.1'], new InputValidator());
