...
test('fromArray detects an empty submit', function (): void {
    $request = MenubarSubmitRequest::fromArray(['submit' => '']);
...
test('isHidden reflects the per-block hide_ key', function (): void {
    $request = MenubarSubmitRequest::fromArray(['hide_42' => '']);

    expect($request->isHidden(42))->toBeTrue()
        ->and($request->isHidden(43))->toBeFalse();
});

test('positionFor parses the per-block pos_ key', function (): void {
    $request = MenubarSubmitRequest::fromArray(['pos_42' => '150']);

    expect($request->positionFor(42))->toBe(150);
});

test('positionFor defaults to 0 when the pos_ key is missing or not numeric', function (): void {
    $request = MenubarSubmitRequest::fromArray(['pos_42' => 'abc']);

    expect($request->positionFor(42))->toBe(0)
        ->and($request->positionFor(99))->toBe(0);
});
