...
test('fromArray parses a numeric image_id', function (): void {
    $request = PictureFormatsImageIdRequest::fromArray(['image_id' => '42'], new InputValidator());

    expect($request->imageId)->toEqual(ImageId::from(42));
});
...
test('fromArray rejects a non-digit image_id', function (): void {
    expect(fn (): PictureFormatsImageIdRequest => PictureFormatsImageIdRequest::fromArray(['image_id' => '1;DROP TABLE'], new InputValidator()))
