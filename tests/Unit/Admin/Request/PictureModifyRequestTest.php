...
test('fromArrays returns defaults for an empty GET/POST', function (): void {
    ...
    expect($request->imageId)
        ->toBe(0)
        ->and($request->deletePresent)
        ->toBeFalse()
        ->and($request->syncMetadataPresent)
        ->toBeFalse()
        ->and($request->isSubmitted)
        ->toBeFalse()
        ->and($request->postLevel)
        ->toBeNull()
        ->and($request->nameField)
        ->toBeNull()
        ->and($request->authorField)
        ->toBeNull()
        ->and($request->commentField)
        ->toBeNull()
        ->and($request->dateCreation)
        ->toBeNull()
        ->and($request->tagsRaw)
        ->toBeNull()
        ->and($request->associate)
        ->toBe([])
        ->and($request->represent)
        ->toBe([]);
});
...
test('fromArrays rejects a malformed image_id', function (): void {
    expect(fn (): PictureModifyRequest => PictureModifyRequest::fromArrays([
...
test('fromArrays distinguishes an empty-string name submission from an absent one', function (): void {
    $submitted = PictureModifyRequest::fromArrays([], [
        'name' => '',
    ], new InputValidator());
    $absent = PictureModifyRequest::fromArrays([], [], new InputValidator());

    expect($submitted->nameField)
        ->toBe('')
        ->and($absent->nameField)
        ->toBeNull();
});
