...
test('fromArray returns null revision/extension/pluginId and false flags when absent', function (): void {
    ...
    expect($request->revision)
        ...
        ->toBeNull()
        ->and($request->installStatusPresent)
        ->toBeFalse()
        ->and($request->installStatus)
        ->toBe('')
        ->and($request->pluginId)
        ->toBeNull()
        ->and($request->betaTest)
        ->toBeFalse();
});
...
test('fromArray reports installStatusPresent and the raw string value', function (): void {
    ...
    expect($request->installStatusPresent)
        ->toBeTrue()
        ->and($request->installStatus)
        ->toBe('ok');
});
...
test('fromArray sets betaTest true only for the literal string true', function (): void {
    expect(PluginsNewRequest::fromArray([
        'beta-test' => 'true',
    ])->betaTest)->toBeTrue()
        ->and(PluginsNewRequest::fromArray([
            'beta-test' => 'false',
        ])->betaTest)->toBeFalse()
