...
test('fromArray defaults minRates to 2 and consensusTopNumber to the given default when absent', function (): void {
    ...
    expect($request->minRates)->toBe(2)
        ->and($request->consensusTopNumber)->toBe(5)
        ->and($request->orderBy)->toBeNull();
});
...
test('fromArray parses numeric values when present', function (): void {
    ...
    expect($request->minRates)->toBe(10)
        ->and($request->consensusTopNumber)->toBe(20)
        ->and($request->orderBy)->toBe(3);
});

test('fromArray ignores non-numeric order_by', function (): void {
    $request = RatingUserFilterRequest::fromArray(['order_by' => 'abc'], 5);

    expect($request->orderBy)->toBeNull();
});

test('fromArray returns an out-of-range orderBy verbatim, leaving bounds-checking to the caller', function (): void {
    $request = RatingUserFilterRequest::fromArray(['order_by' => '99'], 5);

    expect($request->orderBy)->toBe(99);
});
