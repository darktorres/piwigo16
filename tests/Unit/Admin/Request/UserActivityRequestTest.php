...
test('fromArray recognizes the download_logs type', function (): void {
    $request = UserActivityRequest::fromArray([
        'type' => 'download_logs',
    ], new InputValidator());

    expect($request->isDownloadLogs)
        ->toBeTrue();
});
...
test('fromArray exposes filter presence and values for photo/album/group', function (): void {
    ...
    expect($request->hasFilter('photo'))
        ->toBeTrue()
        ->and($request->filterValue('photo'))
        ->toBe('42')
        ->and($request->hasFilter('album'))
        ->toBeFalse()
        ->and($request->filterValue('album'))
        ->toBeNull();
});
...
test('fromArray accepts a valid scalar album/group filter without throwing', function (): void {
    ...
    expect($request->filterValue('album'))
        ->toBe('7')
        ->and($request->filterValue('group'))
        ->toBe('3');
});

test('fromArray rejects a non-digit photo filter', function (): void {
    expect(fn (): UserActivityRequest => UserActivityRequest::fromArray([
        'photo' => '1; DROP TABLE',
    ], new InputValidator()))
