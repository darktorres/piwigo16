...
test('getDateObject builds a year-only DateTime when month is null', function (): void {
    $date = StatsPageRenderer::getDateObject(['year' => 2026, 'month' => null, 'day' => null, 'hour' => null, 'nb_pages' => 5]);

    expect($date->format('Y-m-d'))->toBe('2026-01-01');
});

test('getDateObject cascades through month/day/hour as each is provided', function (): void {
    $monthOnly = StatsPageRenderer::getDateObject(['year' => 2026, 'month' => 7, 'day' => null, 'hour' => null, 'nb_pages' => 5]);
    expect($monthOnly->format('Y-m-d'))->toBe('2026-07-01');

    $dayOnly = StatsPageRenderer::getDateObject(['year' => 2026, 'month' => 7, 'day' => 12, 'hour' => null, 'nb_pages' => 5]);
    expect($dayOnly->format('Y-m-d'))->toBe('2026-07-12');

    $withHour = StatsPageRenderer::getDateObject(['year' => 2026, 'month' => 7, 'day' => 12, 'hour' => 14, 'nb_pages' => 5]);
    expect($withHour->format('Y-m-d H:i'))->toBe('2026-07-12 14:00');
});
...
test('getDateObject accepts numeric-string columns the same as native ints', function (): void {
    ...
    $date = StatsPageRenderer::getDateObject(['year' => '2026', 'month' => '7', 'day' => '12', 'hour' => '14', 'nb_pages' => '5']);

    expect($date->format('Y-m-d H:i'))->toBe('2026-07-12 14:00');
});

test('setMissingValues fills every day bucket between first and last date with zero, then overlays real rows', function (): void {
    $data = [
        ['year' => 2026, 'month' => 7, 'day' => 12, 'hour' => null, 'nb_pages' => 10],
        ['year' => 2026, 'month' => 7, 'day' => 10, 'hour' => null, 'nb_pages' => 4],
    ];
...
test('setMissingValues sums multiple real rows landing in the same bucket', function (): void {
    $data = [
        ['year' => 2026, 'month' => 7, 'day' => 10, 'hour' => 14, 'nb_pages' => 3],
        ['year' => 2026, 'month' => 7, 'day' => 10, 'hour' => 15, 'nb_pages' => 5],
    ];
...
test('setMissingValues defaults first/last date from the data itself when neither is given', function (): void {
    ...
    $data = [
        ['year' => 2026, 'month' => null, 'day' => null, 'hour' => null, 'nb_pages' => 9],
        ['year' => 2024, 'month' => null, 'day' => null, 'hour' => null, 'nb_pages' => 3],
    ];
...
test('setMissingValues throws for an unrecognized unit', function (): void {
    expect(fn () => StatsPageRenderer::setMissingValues('century', [['year' => 2026, 'month' => null, 'day' => null, 'hour' => null, 'nb_pages' => 1]], new DateTime('2026-01-01'), new DateTime('2026-01-01')))
