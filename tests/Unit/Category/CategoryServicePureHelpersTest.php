...
test('removeComputedCategory does nothing when the category has no known parent in the map', function (): void {
    ...
    // ComputedCategoryRow is mutable, so a plain `$original = $cats[1]`
    // would just alias the same object -- clone to actually snapshot the
    // pre-call property values, or this assertion would trivially pass
    // even if removeComputedCategory() mutated the row in place.
    $original = clone $cats[1];
    $removed = catComputedRow(9, null, 0, 5, 0, nbImages: 5);

    CategoryService::removeComputedCategory($cats, $removed);

    expect($cats[1])->toEqual($original);
});
