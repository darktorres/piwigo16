...
test('fromRow narrows a real DB row into typed properties', function (): void {
    ...
    expect($row->id)->toBe(1)
        ...
        ->and($row->comment)->toBe('A description')
        ->and($row->permalink)->toBe('sample-album')
        ->and($row->status)->toBe('public')
        ->and($row->uppercats)->toBe('1')
        ->and($row->globalRank)->toBe('1')
        ->and($row->idUppercat)->toBeNull()
        ->and($row->representativePictureId)->toBe(4)
        ->and($row->imageOrder)->toBe('file ASC');
});
...
test('fromRow falls back to safe defaults for a missing/invalid row', function (): void {
    ...
    expect($row->id)->toBe(0)
        ->and($row->name)->toBe('')
        ->and($row->comment)->toBeNull()
        ->and($row->permalink)->toBeNull()
        ->and($row->status)->toBe('')
        ->and($row->uppercats)->toBe('')
        ->and($row->globalRank)->toBeNull()
        ->and($row->idUppercat)->toBeNull()
        ->and($row->representativePictureId)->toBeNull()
        ->and($row->imageOrder)->toBeNull();
});
