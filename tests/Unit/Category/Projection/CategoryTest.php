...
test('fromRow narrows a real DB row into typed properties', function (): void {
    ...
    expect($cat->id)->toEqual(CategoryId::from(3))
        ->and($cat->name)->toBe('Sample Album')
        ->and($cat->idUppercat)->toBe(1)
        ->and($cat->comment)->toBe('a description')
        ->and($cat->dir)->toBe('sample_dir')
        ->and($cat->rank)->toBe(2)
        ->and($cat->status)->toBe('private')
        ->and($cat->siteId)->toBe(1)
        ->and($cat->visible)->toBeTrue()
        ->and($cat->representativePictureId)->toBe(42)
        ->and($cat->uppercats)->toBe('1,3')
        ->and($cat->commentable)->toBeTrue()
        ->and($cat->globalRank)->toBe('1.2')
        ->and($cat->imageOrder)->toBe('name ASC')
        ->and($cat->permalink)->toEqual(Permalink::from('sample-album'))
        ->and($cat->lastmodified)->toBe('2026-08-01 00:00:00');
});
...
test('fromRow keeps already-hydrated CategoryId/Permalink/CategoryStatus instances as-is', function (): void {
    ...
    expect($cat->id)->toEqual(CategoryId::from(9))
        ->and($cat->permalink)->toEqual(Permalink::from('other-slug'))
        ->and($cat->status)->toBe('public');
});

test('fromRow defaults every nullable column to null/default when absent', function (): void {
    $cat = Category::fromRow(['id' => 1]);

    expect($cat->name)->toBe('')
        ->and($cat->idUppercat)->toBeNull()
        ->and($cat->comment)->toBeNull()
        ->and($cat->dir)->toBeNull()
        ->and($cat->rank)->toBeNull()
        ->and($cat->status)->toBe('public')
        ->and($cat->siteId)->toBeNull()
        ->and($cat->visible)->toBeTrue()
        ->and($cat->representativePictureId)->toBeNull()
        ->and($cat->uppercats)->toBe('')
        ->and($cat->commentable)->toBeTrue()
        ->and($cat->globalRank)->toBeNull()
        ->and($cat->imageOrder)->toBeNull()
        ->and($cat->permalink)->toBeNull()
        ->and($cat->lastmodified)->toBe('');
});
...
test('fromRow throws with the real debug type of a non-null but invalid id', function (): void {
    expect(fn () => Category::fromRow(['id' => 'not-a-number']))
