...
test('fromArrays accepts a well-formed array selection (validated as an array, not a scalar)', function (): void {
    ...
    expect(fn (): BatchManagerRequest => BatchManagerRequest::fromArrays([], ['selection' => ['5', '10']], [], new InputValidator()))
        ->not->toThrow(RuntimeException::class);
});

test('fromArrays rejects a selection item that is not a valid ID', function (): void {
    expect(fn (): BatchManagerRequest => BatchManagerRequest::fromArrays([], ['selection' => ['not-an-id']], [], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays rejects an invalid display value', function (): void {
    expect(fn (): BatchManagerRequest => BatchManagerRequest::fromArrays([], [], ['display' => 'bogus'], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays reads a real page value from GET', function (): void {
    $request = BatchManagerRequest::fromArrays(['page' => 'batch_manager'], [], [], new InputValidator());
...
test('fromArrays parses a valid start from REQUEST', function (): void {
    $request = BatchManagerRequest::fromArrays([], [], ['start' => '12'], new InputValidator());
...
test('fromArrays uses the real negative start value once past the reset boundary, not just 0', function (): void {
    ...
    // a mutated 1.)
    $request = BatchManagerRequest::fromArrays([], [], ['start' => '-1'], new InputValidator());
...
test('fromArrays resets start to 0 when the filter form is submitted, even with a positive REQUEST start', function (): void {
    $request = BatchManagerRequest::fromArrays([], ['submitFilter' => '1'], ['start' => '12', 'submitFilter' => '1'], new InputValidator());
...
test('fromArrays resets start to 0 when display is all', function (): void {
    $request = BatchManagerRequest::fromArrays([], [], ['start' => '12', 'display' => 'all'], new InputValidator());

    expect($request->start)->toBe(0);
});

test('fromArrays reads a valid mode as tab', function (): void {
    $request = BatchManagerRequest::fromArrays(['mode' => 'unit'], [], [], new InputValidator());

    expect($request->tab)->toBe('unit');
});

test('fromArrays rejects an invalid mode', function (): void {
    expect(fn (): BatchManagerRequest => BatchManagerRequest::fromArrays(['mode' => 'bogus'], [], [], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays validates and parses nb_orphans_deleted only for the delete_orphans action', function (): void {
    $matching = BatchManagerRequest::fromArrays(['action' => 'delete_orphans', 'nb_orphans_deleted' => '3'], [], [], new InputValidator());
    $wrongAction = BatchManagerRequest::fromArrays(['action' => 'sync_md5sum', 'nb_orphans_deleted' => '3'], [], [], new InputValidator());
...
test('fromArrays rejects a non-digit nb_orphans_deleted', function (): void {
    expect(fn (): BatchManagerRequest => BatchManagerRequest::fromArrays(['action' => 'delete_orphans', 'nb_orphans_deleted' => 'abc'], [], [], new InputValidator()))
        ->toThrow(RuntimeException::class);
});
...
test('fromArrays defaults nb_orphans_deleted to exactly 0 for an empty (non-numeric) value', function (): void {
    ...
    $request = BatchManagerRequest::fromArrays(['action' => 'delete_orphans', 'nb_orphans_deleted' => ''], [], [], new InputValidator());

    expect($request->nbOrphansDeleted)->toBe(0);
});

test('fromArrays validates and parses nb_md5sum_added only for the sync_md5sum action', function (): void {
    $request = BatchManagerRequest::fromArrays(['action' => 'sync_md5sum', 'nb_md5sum_added' => '7'], [], [], new InputValidator());
...
test('fromArrays defaults nb_md5sum_added to exactly 0 for an empty (non-numeric) value', function (): void {
    $request = BatchManagerRequest::fromArrays(['action' => 'sync_md5sum', 'nb_md5sum_added' => ''], [], [], new InputValidator());

    expect($request->nbMd5sumAdded)->toBe(0);
});

test('fromArrays rejects a non-digit nb_md5sum_added', function (): void {
    expect(fn (): BatchManagerRequest => BatchManagerRequest::fromArrays(['action' => 'sync_md5sum', 'nb_md5sum_added' => 'abc'], [], [], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays retains the full raw post bag', function (): void {
    $request = BatchManagerRequest::fromArrays([], ['filter_category_use' => '1', 'filter_category' => '5'], [], new InputValidator());
...
test('fromArrays normalizes a comma-separated filter string into tokens', function (): void {
    $request = BatchManagerRequest::fromArrays(['filter' => 'category-5,level-1'], [], [], new InputValidator());
...
test('fromArrays casts a non-string scalar array-filter element to string, and \'\' for a non-scalar element', function (): void {
    ...
    $request = BatchManagerRequest::fromArrays(['filter' => [5, ['nested']]], [], [], new InputValidator());

    expect($request->urlFilterTokens)->toBe(['5', '']);
});

test('fromArrays casts a non-string scalar filter value to string, and \'\' for a non-scalar value', function (): void {
    $intFilter = BatchManagerRequest::fromArrays(['filter' => 5], [], [], new InputValidator());
    expect($intFilter->urlFilterTokens)->toBe(['5']);

    $nonScalarFilter = BatchManagerRequest::fromArrays(['filter' => new stdClass()], [], [], new InputValidator());
    expect($nonScalarFilter->urlFilterTokens)->toBe(['']);
});
