...
test('fromArrays returns defaults for an empty GET/POST', function (): void {
    ...
    expect($request->catId)
        ->toBe(0)
        ->and($request->isSetPermalink)
        ->toBeFalse()
        ->and($request->permalink)
        ->toBe('')
        ->and($request->isSave)
        ->toBeFalse()
        ->and($request->deletePermanentPresent)
        ->toBeFalse()
        ->and($request->deletePermanent)
        ->toBe('')
        ->and($request->psfPresent)
        ->toBeFalse()
        ->and($request->psf)
        ->toBeNull()
        ->and($request->dpsfPresent)
        ->toBeFalse()
        ->and($request->dpsf)
        ->toBeNull();
});
...
test('fromArrays rejects a non-digit cat_id', function (): void {
    expect(fn (): PermalinksRequest => PermalinksRequest::fromArrays([], [
        'cat_id' => '1; DROP TABLE',
    ], new InputValidator()))
...
test('fromArrays reports deletePermanentPresent and its normalized value', function (): void {
    $request = PermalinksRequest::fromArrays([
        'delete_permanent' => 'abc',
    ], [], new InputValidator());

    expect($request->deletePermanentPresent)
        ->toBeTrue()
        ->and($request->deletePermanent)
        ->toBe('abc');
});
