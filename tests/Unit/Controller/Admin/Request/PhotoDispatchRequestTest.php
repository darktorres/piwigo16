...
test('fromArray parses a numeric image_id and a known tab', function (): void {
    $request = PhotoDispatchRequest::fromArray(['image_id' => '42', 'tab' => 'coi'], ['properties', 'coi', 'formats'], new InputValidator());
...
test('fromArray falls back to properties for an unknown tab', function (): void {
    $request = PhotoDispatchRequest::fromArray(['tab' => 'not-a-real-tab'], ['properties', 'coi', 'formats'], new InputValidator());

    expect($request->tab)->toBe('properties');
});

test('fromArray rejects a non-digit image_id', function (): void {
    expect(fn (): PhotoDispatchRequest => PhotoDispatchRequest::fromArray(['image_id' => '1; DROP TABLE'], ['properties', 'coi', 'formats'], new InputValidator()))
