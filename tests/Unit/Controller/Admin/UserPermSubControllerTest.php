...
/**
 * Piwigo\Controller\Admin\UserPermSubController -- 1 constructor dep, a
 * pure delegate to `UserPermPageRenderer::render()`. Resolved via
 ...
 * (see project memory). `UserPermPageRenderer::render()` calls
 ...
 * The real per-user category permission grant/deny needs real user and
 * category rows and is not attempted here.
 */
function userPermSubControllerTestRoot(): string
{
    $root = sys_get_temp_dir() . '/piwigo-user-perm-subcontroller-test-' . bin2hex(random_bytes(8)) . '/';
...
function userPermSubControllerTestRrmdir(string $dir): void
{
    if (! is_dir($dir)) {
        return;
    }
    ...
    foreach ($nodes !== false ? $nodes : [] as $node) {
        ...
        $path = $dir . '/' . $node;
        is_dir($path) ? userPermSubControllerTestRrmdir($path) : unlink($path);
    }
    rmdir($dir);
}

test('handle() denies access with a 401 for a logged-in non-admin user', function (): void {
    $root = userPermSubControllerTestRoot();
    ...
    try {
        ...
        $subController = Kernel::container()->get(UserPermSubController::class);
        if (! $subController instanceof UserPermSubController) {
            throw new LogicException('Container returned an unexpected type for ' . UserPermSubController::class);
        }
