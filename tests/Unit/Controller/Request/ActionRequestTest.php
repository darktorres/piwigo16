...
test('fromArray returns defaults for an empty GET', function (): void {
    ...
    expect($request->id)
        ...
        ->and($request->part)
        ->toBeNull()
        ->and($request->formatRequested)
        ->toBeFalse()
        ->and($request->formatId)
        ->toBeNull()
        ->and($request->pwgToken)
        ->toBeNull()
        ->and($request->downloadPresent)
        ->toBeFalse();
});

test('fromArray parses id/part for a valid direct request', function (): void {
    $request = ActionRequest::fromArray([
        'id' => '42',
        'part' => 'e',
...
test('fromArray accepts the "r" and "f" part values, not just "e"', function (): void {
    ...
    $r = ActionRequest::fromArray([
        'part' => 'r',
    ], true, new InputValidator());
    expect($r->part)
        ->toBe('r');

    $f = ActionRequest::fromArray([
        'part' => 'f',
    ], true, new InputValidator());
    expect($f->part)
        ->toBe('f');
...
test('fromArray rejects a malformed format when formats are enabled', function (): void {
    expect(fn (): ActionRequest => ActionRequest::fromArray([
        'format' => '1; DROP TABLE',
    ], true, new InputValidator()))
...
test('fromArray reports downloadPresent when present', function (): void {
    ...
    expect($request->downloadPresent)
        ->toBeTrue();
});
