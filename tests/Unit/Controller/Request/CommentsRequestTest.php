...
test('fromArrays keeps a real since value, but excludes empty-string and "0" sentinels', function (): void {
    $real = CommentsRequest::fromArrays(['since' => '2026-08-01'], [], 20, new InputValidator());
    expect($real->since)->toBe('2026-08-01');

    $empty = CommentsRequest::fromArrays(['since' => ''], [], 20, new InputValidator());
    expect($empty->since)->toBeNull();

    $zero = CommentsRequest::fromArrays(['since' => '0'], [], 20, new InputValidator());
    expect($zero->since)->toBeNull();
    ...
    $nonString = CommentsRequest::fromArrays(['since' => 123], [], 20, new InputValidator());
    expect($nonString->since)->toBeNull();
});

test('fromArrays falls back to 10 for a non-numeric, non-all items_number', function (): void {
    $request = CommentsRequest::fromArrays(['items_number' => 'lots'], [], 20, new InputValidator());

    expect($request->itemsNumber)->toBe(10);
});

test('fromArrays keeps a valid items_number override', function (): void {
    $request = CommentsRequest::fromArrays(['items_number' => '50'], [], 20, new InputValidator());

    expect($request->itemsNumber)->toBe('50');
});

test('fromArrays keeps all as items_number', function (): void {
    $request = CommentsRequest::fromArrays(['items_number' => 'all'], [], 20, new InputValidator());
...
test('fromArrays falls back to a real, present-but-neither-int-nor-string items_number to 10, not a mutated sentinel', function (): void {
    ...
    // final value.
    $request = CommentsRequest::fromArrays(['items_number' => true], [], 20, new InputValidator());
...
test('fromArrays ignores an unrecognized sort_by/sort_order', function (): void {
    $request = CommentsRequest::fromArrays(['sort_by' => 'evil', 'sort_order' => 'evil'], [], 20, new InputValidator());
...
test('fromArrays accepts a recognized sort_by/sort_order', function (): void {
    $request = CommentsRequest::fromArrays(['sort_by' => 'image_id', 'sort_order' => 'ASC'], [], 20, new InputValidator());
...
test('fromArrays skips the cat filter when cat is 0', function (): void {
    $request = CommentsRequest::fromArrays(['cat' => '0'], [], 20, new InputValidator());
...
test('fromArrays validates and keeps a numeric cat', function (): void {
    $request = CommentsRequest::fromArrays(['cat' => '5'], [], 20, new InputValidator());
...
test('fromArrays rejects a non-digit cat', function (): void {
    expect(fn (): CommentsRequest => CommentsRequest::fromArrays(['cat' => 'abc'], [], 20, new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays splits author filter (truthy-gated) from author display (presence-gated)', function (): void {
    $zero = CommentsRequest::fromArrays(['author' => '0'], [], 20, new InputValidator());
    expect($zero->authorFilter)->toBeNull()
        ->and($zero->authorDisplay)->toBe('0');

    $normal = CommentsRequest::fromArrays(['author' => 'alice'], [], 20, new InputValidator());
...
test('fromArrays treats an empty author as absent, same as "0"', function (): void {
    $request = CommentsRequest::fromArrays(['author' => ''], [], 20, new InputValidator());

    expect($request->authorFilter)->toBeNull()
        ->and($request->authorDisplay)->toBe('');
});
...
test('fromArrays rejects a non-string, non-empty-looking author for the filter', function (): void {
    ...
    $request = CommentsRequest::fromArrays(['author' => 123], [], 20, new InputValidator());

    expect($request->authorFilter)->toBeNull();
});

test('fromArrays splits keyword filter (truthy-gated) from keyword display (presence-gated)', function (): void {
    $zero = CommentsRequest::fromArrays(['keyword' => '0'], [], 20, new InputValidator());
...
test('fromArrays treats an empty keyword as absent, same as "0"', function (): void {
    $request = CommentsRequest::fromArrays(['keyword' => ''], [], 20, new InputValidator());

    expect($request->keywordFilter)->toBeNull()
        ->and($request->keywordDisplay)->toBe('');
});

test('fromArrays rejects a non-string, non-empty-looking keyword for the filter', function (): void {
    $request = CommentsRequest::fromArrays(['keyword' => 456], [], 20, new InputValidator());

    expect($request->keywordFilter)->toBeNull();
});

test('fromArrays validates and parses a numeric comment_id', function (): void {
    $request = CommentsRequest::fromArrays(['comment_id' => '42'], [], 20, new InputValidator());

    expect($request->commentId)->toBe(42);
});

test('fromArrays rejects a non-digit comment_id', function (): void {
    expect(fn (): CommentsRequest => CommentsRequest::fromArrays(['comment_id' => 'abc'], [], 20, new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays treats an empty comment_id as absent, not a validation failure', function (): void {
    $request = CommentsRequest::fromArrays(['comment_id' => ''], [], 20, new InputValidator());

    expect($request->commentId)->toBeNull();
});

test('fromArrays picks the first matching action in delete/validate/edit order', function (): void {
    $request = CommentsRequest::fromArrays(['validate' => '7', 'edit' => '9'], [], 20, new InputValidator());
...
test('fromArrays recognizes a bare delete action with no other action keys present', function (): void {
    ...
    // null instead.
    $request = CommentsRequest::fromArrays(['delete' => '3'], [], 20, new InputValidator());

    expect($request->action)->toBe('delete')
        ->and($request->actionCommentId)->toBe(3);
});
...
test('fromArrays recognizes a bare edit action with no other action keys present', function (): void {
    ...
    $request = CommentsRequest::fromArrays(['edit' => '5'], [], 20, new InputValidator());

    expect($request->action)->toBe('edit')
        ->and($request->actionCommentId)->toBe(5);
});
...
test('fromArrays rejects a non-digit action value', function (): void {
    ...
    expect(fn (): CommentsRequest => CommentsRequest::fromArrays(['delete' => 'abc'], [], 20, new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArrays parses start as an int', function (): void {
    $request = CommentsRequest::fromArrays(['start' => '30'], [], 20, new InputValidator());
...
test('fromArrays falls back to 0 when start is present but not scalar', function (): void {
    ...
    // array is 1, not 0).
    $request = CommentsRequest::fromArrays(['start' => ['nested']], [], 20, new InputValidator());
...
test('fromArrays treats an empty or zero content as absent', function (): void {
    $empty = CommentsRequest::fromArrays([], ['content' => ''], 20, new InputValidator());
    $zero = CommentsRequest::fromArrays([], ['content' => '0'], 20, new InputValidator());
