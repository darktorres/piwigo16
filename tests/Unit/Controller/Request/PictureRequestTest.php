...
test('fromArrays returns defaults for an empty GET/POST', function (): void {
    ...
    expect($request->metadataPresent)
        ->toBeFalse()
        ->and($request->action)
        ->toBeNull()
        ->and($request->rate)
        ->toBeNull()
        ->and($request->commentToEdit)
        ->toBeNull()
        ->and($request->content)
        ->toBeNull()
        ->and($request->postKey)
        ...
        ->toBeNull()
        ->and($request->commentToDelete)
        ->toBeNull()
        ->and($request->commentToValidate)
        ->toBeNull()
        ->and($request->contentPresent)
        ->toBeFalse()
        ->and($request->slideshowPresent)
        ->toBeFalse()
        ->and($request->slideshow)
        ->toBeNull();
});
...
test('fromArrays parses action and rate', function (): void {
    ...
    $requestWithAction = PictureRequest::fromArrays([
        'action' => 'add_to_favorites',
    ], [], new InputValidator());

    expect($request->rate)
        ->toBe('5')
        ->and($requestWithAction->action)
        ->toBe('add_to_favorites');
});
...
test('fromArrays rejects a malformed comment_to_edit when action is edit_comment', function (): void {
    expect(fn (): PictureRequest => PictureRequest::fromArrays([
        'action' => 'edit_comment',
        'comment_to_edit' => '1; DROP TABLE',
    ], [], new InputValidator()))
...
test('fromArrays validates comment_to_delete only when action is delete_comment', function (): void {
    ...
    expect($request->commentToDelete)
        ->toBe('7');
});
...
test('fromArrays validates comment_to_validate only when action is validate_comment', function (): void {
    $request = PictureRequest::fromArrays([
        ...
        'comment_to_validate' => '9',
    ], [], new InputValidator());

    expect($request->commentToValidate)
        ->toBe('9');
});
...
test('fromArrays reports slideshowPresent and the raw slideshow string', function (): void {
    ...
    expect($request->slideshowPresent)
        ->toBeTrue()
        ->and($request->slideshow)
        ->toBe('abc');
});
