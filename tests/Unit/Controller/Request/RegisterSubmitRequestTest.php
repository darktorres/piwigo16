...
test('fromArray reports not submitted and empty defaults for an empty POST', function (): void {
    ...
    expect($request->isSubmitted)->toBeFalse()
        ...
        ->and($request->password)->toBe('')
        ->and($request->passwordConf)->toBe('')
        ->and($request->login)->toBe('')
        ->and($request->mailAddress)->toBeNull()
        ->and($request->sendPasswordByMail)->toBeFalse();
});
...
test('fromArray parses a full registration submission', function (): void {
    ...
    expect($request->isSubmitted)->toBeTrue()
        ->and($request->key)->toBe('1:2:3')
        ->and($request->password)->toBe('secret')
        ->and($request->passwordConf)->toBe('secret')
        ->and($request->login)->toBe('alice')
        ->and($request->mailAddress)->toBe('alice@example.test')
        ->and($request->sendPasswordByMail)->toBeTrue();
});
...
test('fromArray normalizes a non-string mail_address to null', function (): void {
    $request = RegisterSubmitRequest::fromArray(['mail_address' => ['x']]);

    expect($request->mailAddress)->toBeNull();
});
