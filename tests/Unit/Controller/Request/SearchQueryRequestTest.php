...
test('fromArray rejects a non-digit cat_id', function (): void {
    expect(fn (): SearchQueryRequest => SearchQueryRequest::fromArray(['cat_id' => '1; DROP TABLE'], new InputValidator()))
        ->toThrow(RuntimeException::class);
});

test('fromArray reports hasTagId and passes through a valid comma-separated tag_id', function (): void {
    $request = SearchQueryRequest::fromArray(['tag_id' => '1,2,3'], new InputValidator());
...
test('fromArray rejects a malformed tag_id', function (): void {
    expect(fn (): SearchQueryRequest => SearchQueryRequest::fromArray(['tag_id' => 'abc'], new InputValidator()))
        ->toThrow(RuntimeException::class);
});
