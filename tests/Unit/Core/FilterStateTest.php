...
test('every getter throws before set() has ever been called', function (): void {
    ...
    expect($filterState->isInitialized())
        ->toBeFalse();
...
test('set() initializes every field and isInitialized flips to true', function (): void {
    ...
    $filterState->set(true, 'a,b,c', 'x,y', [
        '1' => [
            'name' => 'Holidays',
        ],
    ]);
    ...
    expect($filterState->isInitialized())
        ...
        ->toBeTrue()
        ->and($filterState->visibleCategories())
        ->toBe('a,b,c')
        ->and($filterState->visibleImages())
        ->toBe('x,y')
        ->and($filterState->categories())
        ->toBe([
            '1' => [
                'name' => 'Holidays',
            ],
...
test('reset() clears $enabled/$visibleCategories/$visibleImages/$categories to their real default values, not just $initialized', function (): void {
    ...
    expect($class->getProperty('enabled')->getValue($filterState))
        ->toBeFalse();
    expect($class->getProperty('visibleCategories')->getValue($filterState))
        ->toBe('');
    expect($class->getProperty('visibleImages')->getValue($filterState))
        ->toBe('');
    expect($class->getProperty('categories')->getValue($filterState))
        ->toBe([]);
});
