...
test('fromRoot composes every subdirectory from the given root, trailing slash normalized', function (): void {
    ...
    expect($paths->root)
        ->toBe('/var/www/piwigo/')
        ->and($paths->plugins)
        ->toBe('/var/www/piwigo/plugins/')
        ->and($paths->themes)
        ->toBe('/var/www/piwigo/themes/')
        ->and($paths->local)
        ->toBe('/var/www/piwigo/local/')
        ->and($paths->siteLocal)
        ->toBe('/var/www/piwigo/local/')
        ->and($paths->data)
        ->toBe('/var/www/piwigo/_data/')
        ->and($paths->derivatives)
        ->toBe('/var/www/piwigo/_data/i/')
        ->and($paths->logs)
        ->toBe('/var/www/piwigo/_data/logs/')
        ->and($paths->upload)
        ->toBe('/var/www/piwigo/upload/')
        ->and($paths->config)
        ->toBe('/var/www/piwigo/config/')
        ->and($paths->vendor)
        ->toBe('/var/www/piwigo/vendor/');
});
...
test('siteLocal honors a PIWIGO_LOCAL_DIR override, the multi-site-instance deployment mechanism, while local stays fixed', function (): void {
    ...
    try {
        ...
        expect($paths->siteLocal)
            ->toBe('/var/www/piwigo/local-site2/')
            ->and($paths->local)
            ->toBe('/var/www/piwigo/local/');
    } finally {
        putenv($original === null ? 'PIWIGO_LOCAL_DIR' : 'PIWIGO_LOCAL_DIR=' . $original);
