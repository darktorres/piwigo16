...
test('setStatusHeader sends a real HTTP/1.1 status line, with the given code and reason phrase, for a genuine HTTP/1.1 request', function (): void {
    ...
    file_put_contents(
        ...
        . '(new \Piwigo\Html\HtmlService(new \Piwigo\Config\CurrentConfig(), new \Piwigo\PluginConfig\EventDispatcher(), new \Piwigo\Core\ProcessCache(), new \Piwigo\Core\ErrorCollector(new \Piwigo\Config\DeploymentPolicy(), \Piwigo\Core\Paths::fromRoot(sys_get_temp_dir())), new \Piwigo\Users\CurrentUser(new \Piwigo\Config\CurrentConfig()), new \Piwigo\Template\CurrentTemplate(), new \Piwigo\Core\PageState(), new \Piwigo\Lang\Translator(new \Piwigo\Config\CurrentConfig(), new \Piwigo\Cache\TranslationsCachePool(\Piwigo\Cache\CacheFactory::create(namespace: \'piwigo.translations\'))), new \Piwigo\Category\CategoryRepository($em, new \Piwigo\Config\CurrentConfig()), $em))->setStatusHeader((int) ($_GET[\'code\'] ?? 404), (string) ($_GET[\'text\'] ?? \'\'));',
    );
