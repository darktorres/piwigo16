...
test('passes through a successful response unchanged', function (): void {
    ...
    $response = new ExceptionHandlerMiddleware($logger)
        ->process(new ServerRequest('GET', '/'), okHandler());
...
test('catches a downstream throwable and returns a generic 500', function (): void {
    ...
    $response = new ExceptionHandlerMiddleware($logger)
        ->process(new ServerRequest('GET', '/'), throwingHandler());
...
test('logs the caught throwable with structured context', function (): void {
    ...
    new ExceptionHandlerMiddleware($logger)
        ->process(new ServerRequest('GET', '/some/path'), throwingHandler());
