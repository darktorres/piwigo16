...
test('html sets an html content type with utf-8 charset', function (): void {
    ...
    expect((string) $response->getBody())->toBe('<p>hi</p>');
});
...
test('raw sends the body through with exactly the given headers', function (): void {
    ...
    expect($response->getStatusCode())->toBe(200);
    expect($response->getHeaderLine('Content-Type'))->toBe('application/rss+xml; charset=utf-8; filename=feed.xml');
    expect($response->getHeaderLine('Content-Disposition'))->toBe('inline; filename=feed.xml');
    expect((string) $response->getBody())->toBe('binary-ish content');
});

test('raw accepts a custom status code', function (): void {
    $response = ResponseFactory::raw('forbidden', ['Content-Type' => 'text/plain'], 403);
