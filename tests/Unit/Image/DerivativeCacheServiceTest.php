...
beforeEach(function (): void {
    ...
    // own tests exercise) pointed at whatever root that earlier boot bound
    // instead of this fixture root.
    Kernel::reset();
...
test('clearDerivativeCacheRecursive deletes only files matching the pattern', function (): void {
    ...
    new DerivativeCacheService(CurrentConfigTestFactory::get(), CurrentPathsTestFactory::get())->clearDerivativeCacheRecursive($dir, '#.*-th\.jpg$#');
...
test('clearDerivativeCache groups 2+ types into a single alternation pattern', function (): void {
    ...
    new DerivativeCacheService(CurrentConfigTestFactory::get(), CurrentPathsTestFactory::get())->clearDerivativeCache(['thumb', 'square']);
...
test('clearDerivativeCache falls back to the custom-type pattern for a type name that is neither "all" nor a standard ImageStdParams type', function (): void {
    ...
    new DerivativeCacheService(CurrentConfigTestFactory::get(), CurrentPathsTestFactory::get())->clearDerivativeCache('myCustomWidget');
...
test('deleteElementDerivatives rewrites the path to its pwg_representative form when representative_ext is given', function (): void {
    $derivDir = CurrentPathsTestFactory::get()->root . CurrentConfigTestFactory::get()->derivativeDir . '2026/07/pwg_representative';
    mkdir($derivDir, 0o777, true);
    file_put_contents($derivDir . '/photo-th.jpg', 'x');
...
test('clearDerivativeCache does nothing, without throwing, when the derivatives directory does not exist', function (): void {
    ...
    rmdir(CurrentPathsTestFactory::get()->root . CurrentConfigTestFactory::get()->derivativeDir);
...
test('clearDerivativeCache never removes the derivatives root directory itself, even once every year-folder is emptied out', function (): void {
    ...
    $derivRoot = CurrentPathsTestFactory::get()->root . CurrentConfigTestFactory::get()->derivativeDir;
...
test('clearDerivativeCacheRecursive returns false without throwing when the directory cannot be opened', function (): void {
    ...
    set_error_handler(static fn (): bool => true);
    try {
        $result = new DerivativeCacheService(CurrentConfigTestFactory::get(), CurrentPathsTestFactory::get())->clearDerivativeCacheRecursive($missing, '#.*#');
    } finally {
        restore_error_handler();
    }
