...
test('sizeToUrl collapses a square size to a single number, keeps a rectangular one as WxH', function (): void {
    expect(DerivativeUrlCodec::sizeToUrl(new Dimensions(100, 100)))->toBe(100);
    expect(DerivativeUrlCodec::sizeToUrl(new Dimensions(100, 200)))->toBe('100x200');
});
...
test('urlToSize parses both the single-number and WxH token shapes, the exact inverse of sizeToUrl', function (): void {
    expect(DerivativeUrlCodec::urlToSize('100'))->toEqual(new Dimensions(100, 100));
    expect(DerivativeUrlCodec::urlToSize('100x200'))->toEqual(new Dimensions(100, 200));
});

test('sizeEquals compares both dimensions', function (): void {
    expect(DerivativeUrlCodec::sizeEquals(new Dimensions(100, 200), new Dimensions(100, 200)))->toBeTrue();
    expect(DerivativeUrlCodec::sizeEquals(new Dimensions(100, 200), new Dimensions(100, 201)))->toBeFalse();
    expect(DerivativeUrlCodec::sizeEquals(new Dimensions(100, 200), new Dimensions(101, 200)))->toBeFalse();
});
