...
test('decodeSlideshowParams with null input returns the defaults', function (): void {
    $decoded = new ImageService(imageServiceTestLang(), EntityManagerFactory::build(DbConnection::build())->getRepository(ImageEntity::class), new ActivityService(EntityManagerFactory::build(DbConnection::build())->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()))->decodeSlideshowParams(null);
...
test('encodeSlideshowParams encodes a changed boolean', function (): void {
    $encoded = new ImageService(imageServiceTestLang(), EntityManagerFactory::build(DbConnection::build())->getRepository(ImageEntity::class), new ActivityService(EntityManagerFactory::build(DbConnection::build())->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()))->encodeSlideshowParams(['period' => 4, 'repeat' => false, 'play' => true]);
...
test('decodeSlideshowParams parses more than one key-value token from the same string', function (): void {
    $decoded = new ImageService(imageServiceTestLang(), EntityManagerFactory::build(DbConnection::build())->getRepository(ImageEntity::class), new ActivityService(EntityManagerFactory::build(DbConnection::build())->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()))->decodeSlideshowParams('period-6+repeat-false+play-false');
...
test('countPdfPages counts page markers', function (): void {
    ...
    expect(new ImageService(imageServiceTestLang(), EntityManagerFactory::build(DbConnection::build())->getRepository(ImageEntity::class), new ActivityService(EntityManagerFactory::build(DbConnection::build())->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()))->countPdfPages($tmp))->toBe(2);

    unlink($tmp);
});

test('countPdfPages returns false for a missing file', function (): void {
    expect(new ImageService(imageServiceTestLang(), EntityManagerFactory::build(DbConnection::build())->getRepository(ImageEntity::class), new ActivityService(EntityManagerFactory::build(DbConnection::build())->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()))->countPdfPages('/no/such/file.pdf'))->toBeFalse();
});
...
{
    return new ImageService(imageServiceTestLang(), $repo, new ActivityService(EntityManagerFactory::build($conn)->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), $paths ?? Paths::fromRoot(sys_get_temp_dir()));
}
...
test('deleteElements() fires begin_delete_elements and delete_elements with the exact real ids, deletes the image row, and records the activity', function (): void {
    ...
    EventDispatcher::get()->addTypedHandler(BeginDeleteElements::class, $beginHandler);
    EventDispatcher::get()->addTypedHandler(DeleteElements::class, $deleteHandler);

    try {
        $service = new ImageService(imageServiceTestLang(), $repo, $activityService, SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()));
        ...
        EventDispatcher::get()->removeEventHandler(BeginDeleteElements::class, $beginHandler);
        EventDispatcher::get()->removeEventHandler(DeleteElements::class, $deleteHandler);
    }
});
...
test('deleteElements() with an empty id list never fires begin_delete_elements at all', function (): void {
    ...
    EventDispatcher::get()->addTypedHandler(BeginDeleteElements::class, $handler);
    ...
    try {
        ...
        EventDispatcher::get()->removeEventHandler(BeginDeleteElements::class, $handler);
    }
});
...
test('emptyLounge() clears a stale lock, logs the API-suffixed begin/win/end messages exactly, and empties every real lounge row into image_category', function (): void {
    ...
    try {
        ...
        EventDispatcher::get()->addTypedHandler(EmptyLounge::class, $handler);
...
test('countPdfPages() returns false when the path passes is_file()/is_readable() but file_get_contents() itself fails', function (): void {
    ...
    try {
        set_error_handler(static fn (): bool => true);
        try {
            $result = new ImageService(imageServiceTestLang(), EntityManagerFactory::build(DbConnection::build())->getRepository(ImageEntity::class), new ActivityService(EntityManagerFactory::build(DbConnection::build())->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), TranslatorTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()))->countPdfPages($scheme . '://fake.pdf');
        } finally {
            restore_error_handler();
        }
