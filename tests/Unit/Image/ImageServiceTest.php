...
test('logger() throws when the container returns an unexpected type for CurrentLogger', function (): void {
    ...
    KernelContainerOverride::withWrongTypeFor(CurrentLogger::class, function () use ($service): void {
        $service->emptyLounge(SessionServiceTestFactory::get());
    });
...
test('deleteElements() with physical deletion removing zero files never fires delete_elements or records the activity', function (): void {
    ...
    try {
        $service = new ImageService($repo, $activityService, EventDispatcherTestFactory::get(), CurrentConfigTestFactory::get(), Paths::fromRoot($root), imageServiceTestCategoryService($conn));
        $urlService = new ImageServiceTestFakeUrlService();
...
test('deleteElements() fires begin_delete_elements and delete_elements with the exact real ids, deletes the image row, and records the activity', function (): void {
    ...
    try {
        $service = new ImageService($repo, $activityService, EventDispatcherTestFactory::get(), CurrentConfigTestFactory::get(), Paths::fromRoot(sys_get_temp_dir()), imageServiceTestCategoryService($conn));
...
test('emptyLounge() clears a stale lock, logs the API-suffixed begin/win/end messages exactly, and empties every real lounge row into image_category', function (): void {
    ...
    try {
        ...
        try {
            ...
            $result = $service->emptyLounge(SessionServiceTestFactory::get(), invalidateUserCache: false);
...
test('emptyLounge() invalidates the permission cache (and its orphan-count cache entry) when asked to', function (): void {
    ...
    try {
        ...
        try {
            ...
            $service->emptyLounge(SessionServiceTestFactory::get(), invalidateUserCache: true);
...
test('emptyLounge() treats a lock that is exactly 60 seconds old as still fresh, not yet stale', function (): void {
    ...
    try {
        ...
        try {
            ...
            $result = $service->emptyLounge(SessionServiceTestFactory::get(), invalidateUserCache: false, now: $t0);
...
test('emptyLounge() returns null when a different, still-fresh execution already holds the lock', function (): void {
    ...
    try {
        ...
        try {
            ...
            expect($service->emptyLounge(SessionServiceTestFactory::get()))
                ->toBeNull();
        } finally {
            $conn->executeStatement("DELETE FROM config WHERE param = 'empty_lounge_running'");
            CurrentConfigTestFactory::get()->reset();
