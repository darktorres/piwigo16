...
{
    return new ImageService(imageServiceTestLang(), $repo, new ActivityService(EntityManagerFactory::build($conn)->getRepository(ActivityEntity::class)), SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), Translator::get(), $paths ?? Paths::fromRoot(sys_get_temp_dir()));
}
...
test('deleteElements() fires begin_delete_elements and delete_elements with the exact real ids, deletes the image row, and records the activity', function (): void {
    ...
    try {
        $service = new ImageService(imageServiceTestLang(), $repo, $activityService, SessionServiceTestFactory::get(), EventDispatcher::get(), CurrentConfig::current(), Translator::get(), Paths::fromRoot(sys_get_temp_dir()));
