...
test('fromArray reads the ws method', function (): void {
    $request = EmptyLoungeRequest::fromArray(['method' => 'pwg.images.emptyLounge']);

    expect($request->requestMethod)->toBe('pwg.images.emptyLounge');
});

test('fromArray narrows a non-string value to null', function (): void {
    $request = EmptyLoungeRequest::fromArray(['method' => ['nested']]);
