...
beforeEach(function (): void {
    // A real Paths is required, not a bare boot: __invoke() resolves
    // UploadService via Bootstrap\AdminAccessor::uploadService(), whose
    // own StorageRegistry dependency's factory always calls fromConfig(),
    // which requires() config/storage.php -- that file unconditionally
    // calls CurrentPathsTestFactory::get().
...
test('__invoke returns the existing image id and deletes the newly uploaded file when its md5sum already exists (duplicate detection)', function (): void {
    ...
    try {
        ...
        $handler = new BatchUploadHandler(UrlServiceTestFactory::build());
