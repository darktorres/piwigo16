...
test('fromRow narrows a real DB row into typed properties', function (): void {
    ...
    expect($rate->userId)->toEqual(UserId::from(3))
        ->and($rate->elementId)->toEqual(ImageId::from(7))
        ->and($rate->anonymousId)->toBe('')
        ->and($rate->rate)->toBe(4)
        ->and($rate->date)->toBe('2026-08-01');
});
...
test('fromRow keeps already-hydrated UserId/ImageId instances as-is', function (): void {
    ...
    expect($rate->userId)->toEqual(UserId::from(5))
        ->and($rate->elementId)->toEqual(ImageId::from(9));
});

test('fromRow defaults anonymous_id/rate/date when absent', function (): void {
    $rate = Rate::fromRow(['user_id' => 1, 'element_id' => 1]);

    expect($rate->anonymousId)->toBe('')
        ->and($rate->rate)->toBe(0)
        ->and($rate->date)->toBeNull();
});

test('fromRow throws when user_id is missing or invalid', function (): void {
    expect(fn () => Rate::fromRow(['element_id' => 1]))
        ->toThrow(InvalidArgumentException::class, 'Expected a positive user id, got null');

    expect(fn () => Rate::fromRow(['user_id' => 'not-a-number', 'element_id' => 1]))
...
test('fromRow throws when element_id is missing or invalid', function (): void {
    expect(fn () => Rate::fromRow(['user_id' => 1]))
        ->toThrow(InvalidArgumentException::class, 'Expected a positive element id, got null');

    expect(fn () => Rate::fromRow(['user_id' => 1, 'element_id' => 'not-a-number']))
