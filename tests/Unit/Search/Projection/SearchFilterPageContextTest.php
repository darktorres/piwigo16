...
test('toArray flattens every fixed property, and omits every optional key when null', function (): void {
    $result = makeSearchFilterPageContextForTest()->toArray();
...
test('toArray includes every optional key when set', function (): void {
    ...
    expect($result['TAGS'])->toBe([['id' => 1, 'name' => 'sunset']])
        ...
        ->and($result['FILESIZE'])->toBe(['list' => '0.0,1.0', 'bounds' => ['min' => '0.0', 'max' => '1.0'], 'selected' => ['min' => '0.0', 'max' => '1.0']])
        ->and($result['RATIOS'])->toBe(['Portrait' => 1, 'square' => 0, 'Landscape' => 2, 'Panorama' => 0])
        ->and($result['HEIGHT'])->toBe(['list' => '100,200', 'bounds' => ['min' => '100', 'max' => '200'], 'selected' => ['min' => '100', 'max' => '200']])
        ->and($result['WIDTH'])->toBe(['list' => '100,200', 'bounds' => ['min' => '100', 'max' => '200'], 'selected' => ['min' => '100', 'max' => '200']]);
});

test('toArray preserves a false fullnameOf/gp value distinctly from null', function (): void {
    $result = makeSearchFilterPageContextForTest(fullnameOf: false, gp: false)->toArray();
