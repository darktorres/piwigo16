...
function fullSearchRow(): array
{
    return [
        ...
        'created_by' => '3',
        'forked_from' => '2',
        'rules' => json_encode(['q' => 'nature', 0 => 'stray-int-key']),
    ];
}
...
test('fromRow decodes a JSON rules string, keeping only string-keyed entries', function (): void {
    ...
    expect($search->id)->toBe(7)
        ->and($search->searchUuid)->toBe('psk-20260712-abcdefghij')
        ->and($search->createdOn)->toBe('2026-07-12 00:00:00')
        ->and($search->createdBy)->toBe(3)
        ->and($search->forkedFrom)->toBe(2)
        ...
        ->and($search->rules)->toBe(['q' => 'nature']);
});
...
test('fromRow defaults every nullable/malformed column to null, and id/rules to their zero value', function (): void {
    ...
    expect($search->id)->toBe(0)
        ->and($search->searchUuid)->toBeNull()
        ->and($search->createdOn)->toBeNull()
        ->and($search->createdBy)->toBeNull()
        ->and($search->forkedFrom)->toBeNull()
        ->and($search->rules)->toBeNull();
});
