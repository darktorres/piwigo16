...
test('computeForce stays false for a guest even when the file type would otherwise qualify', function (): void {
    ...
    try {
        $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'css', UrlServiceTestFactory::build(), Paths::fromRoot('/tmp/piwigo-file-combiner-test'), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), []);
...
test('computeForce is true for an admin combining JS with templateCompileCheck off and a cache-busting header', function (): void {
    ...
    try {
        $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot('/tmp/piwigo-file-combiner-test'), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), []);
...
test('initialKey seeds the scheme-less absolute root URL for CSS combining, so a scheme change alone busts the cache', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'css', $urlService, Paths::fromRoot('/tmp/piwigo-file-combiner-test'), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), []);
...
test('combine returns a single non-template combinable unchanged', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot('/tmp/piwigo-file-combiner-test'), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [$combinable]);
...
test('combine passes remote combinables through without combining them', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot('/tmp/piwigo-file-combiner-test'), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [$remote, $local]);
...
test('combine flushes pending items before appending a remote combinable, preserving declaration order', function (): void {
    ...
    $remote = new Combinable('remote-script', 'https://cdn.example.com/foo.js');
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot('/tmp/piwigo-file-combiner-test'), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [$first, $remote]);
...
test('combine merges 2+ non-template files into a single combined output on disk, respecting declaration order', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot($root), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [$first, $second]);
...
test('combine does not rewrite an already-combined file on a second call (cache respected, not force-rebuilt)', function (): void {
    ...
    $combinerFirst = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot($root), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [
    ...
    try {
        ...
        $combinerSecond = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot($root), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [
            new Combinable('a', 'themes/default/js/a.js'),
            new Combinable('b', 'themes/default/js/b.js'),
        ]);
...
test('combine merges 2+ CSS files into a single combined output, with a stripped @import promoted to the shared header', function (): void {
    ...
    $urlService = UrlServiceTestFactory::build();
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'css', $urlService, Paths::fromRoot($root), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), [
...
test('combine reaches process_combinable for a single template combinable via flush_pending\'s own count===1 branch, updating its path to the cached file', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot($root), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), []);
...
test('process_combinable builds and writes a new combined CSS file on a cache miss, dispatching to process_css', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'css', UrlServiceTestFactory::build(), Paths::fromRoot($root), new EventDispatcher(), CurrentTemplate::current(), CurrentConfig::current(), []);
...
test('process_css throws when a combined_css_postfilter listener returns something other than a CombinedCssPostfilter instance', function (): void {
    ...
    $combinable = new Combinable('foo-css', 'themes/default/css/foo.css');
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'css', UrlServiceTestFactory::build(), Paths::fromRoot($root), EventDispatcher::get(), CurrentTemplate::current(), CurrentConfig::current(), []);
...
test('process_combinable notifies combinable_preparse listeners before parsing a template combinable', function (): void {
    ...
    $combiner = new FileCombiner(fileCombinerTestAccessControl(), 'js', UrlServiceTestFactory::build(), Paths::fromRoot($root), EventDispatcher::get(), CurrentTemplate::current(), CurrentConfig::current(), []);
