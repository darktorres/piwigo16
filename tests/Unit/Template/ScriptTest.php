...
test('constructor sets load_mode, precedents, and initializes extra empty', function (): void {
    ...
    expect($script->load_mode)->toBe(1)
        ->and($script->precedents)->toBe(['jquery'])
        ->and($script->extra)->toBe([])
        ->and($script->id)->toBe('my-script')
        ->and($script->path)->toBe('themes/default/js/foo.js');
});
