...
function fullDefaultUserInfoRow(): array
{
    return [
        ...
        'preferences' => ['maxwidth' => '100', 'maxheight' => '100'],
    ];
}
...
test('fromArray narrows every column to its real type', function (): void {
    ...
    expect($info->nbImagePage)->toBe(20)
        ->and($info->language)->toBe('en_US')
        ->and($info->expand)->toBeTrue()
        ->and($info->showNbComments)->toBeTrue()
        ->and($info->showNbHits)->toBeTrue()
        ->and($info->recentPeriod)->toBe(7)
        ->and($info->theme)->toBe('default')
        ->and($info->enabledHigh)->toBeTrue()
        ->and($info->level)->toBe(3)
        ->and($info->activationKey)->toBe('abc123')
        ->and($info->activationKeyExpire)->toBe('2026-08-08 00:00:00')
        ->and($info->lastmodified)->toBe('2026-08-01 12:00:00')
        ->and($info->preferences)->toBe(['maxwidth' => '100', 'maxheight' => '100']);
});
...
test('fromArray defaults absent scalar columns to their zero value', function (): void {
    ...
    expect($info->nbImagePage)->toBe(0)
        ->and($info->language)->toBe('')
        ->and($info->expand)->toBeFalse()
        ->and($info->showNbComments)->toBeFalse()
        ->and($info->showNbHits)->toBeFalse()
        ->and($info->recentPeriod)->toBe(0)
        ->and($info->theme)->toBe('')
        ->and($info->enabledHigh)->toBeFalse()
        ->and($info->level)->toBe(0)
        ->and($info->activationKey)->toBeNull()
        ->and($info->activationKeyExpire)->toBeNull()
        ->and($info->lastmodified)->toBe('')
        ->and($info->preferences)->toBeNull();
});
...
test('fromArray drops any non-string-keyed entry from a corrupted preferences cache value', function (): void {
    ...
    $row['preferences'] = ['maxwidth' => '100', 0 => 'stray-int-key', 1 => 'another'];

    $info = DefaultUserInfo::fromArray($row);

    expect($info->preferences)->toBe(['maxwidth' => '100']);
});
