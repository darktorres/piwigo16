...
test('constructs with distinct values for every property', function (): void {
    ...
    expect($row->userId)->toBeInstanceOf(UserId::class)
        ->and($row->userId?->value)->toBe(5)
        ->and($row->status)->toBe('normal')
        ->and($row->language)->toBe('en_UK')
        ->and($row->email)->toBe('user@example.com')
        ->and($row->username)->toBe('fixture_user');
});
...
test('accepts a null userId/status/language/email/username', function (): void {
    ...
    expect($row->userId)->toBeNull()
        ...
        ->and($row->language)->toBeNull()
        ->and($row->email)->toBeNull()
        ->and($row->username)->toBeNull();
});
