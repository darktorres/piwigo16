...
use Piwigo\Ws\Groups\DeleteHandler;
use Piwigo\Ws\Server;
use Piwigo\Ws\WsErrorResponse;

/**
 * Piwigo\Ws\Groups\DeleteHandler -- `pwg.groups.delete` (admin_only,
 ...
 * pattern as PermissionsTest.php. The real group deletion needs real
 * group rows and is not attempted here.
 */
function pwgGroupsDeleteHandlerTestSubject(): DeleteHandler
{
...
function pwgGroupsDeleteHandlerTestServer(): Server
{
...
test('returns a 403 WsErrorResponse when the submitted pwg_token does not match the real CSRF token', function (): void {
    $handler = pwgGroupsDeleteHandlerTestSubject();
    $server = pwgGroupsDeleteHandlerTestServer();
