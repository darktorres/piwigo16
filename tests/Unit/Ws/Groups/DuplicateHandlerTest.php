...
use Piwigo\Ws\Groups\DuplicateHandler;
use Piwigo\Ws\Server;
use Piwigo\Ws\WsErrorResponse;

/**
 * Piwigo\Ws\Groups\DuplicateHandler -- `pwg.groups.duplicate`
 ...
 * token is therefore a cheap, DB-free 403 branch. The real duplicate
 * needs a real group row and is not attempted here.
 */
function pwgGroupsDuplicateHandlerTestSubject(): DuplicateHandler
{
...
function pwgGroupsDuplicateHandlerTestServer(): Server
{
...
test('returns a 403 WsErrorResponse when the submitted pwg_token does not match the real CSRF token', function (): void {
    $handler = pwgGroupsDuplicateHandlerTestSubject();
    $server = pwgGroupsDuplicateHandlerTestServer();
