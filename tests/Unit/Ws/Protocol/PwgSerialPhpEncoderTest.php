...
test('encodeResponse serializes a PwgError as a fail/err/message triple', function (): void {
    ...
    expect($result)
        ->toBe('a:3:{s:4:"stat";s:4:"fail";s:3:"err";i:1003;s:7:"message";s:17:"Invalid param foo";}')
        ->and(unserialize($result))
        ->toBe([
            'stat' => 'fail',
            'err' => 1003,
...
test('encodeResponse flattens a PwgNamedStruct, merging its attributes_xml_ marker key into the result', function (): void {
    ...
    expect($result)
        ->toBe('a:2:{s:4:"stat";s:2:"ok";s:6:"result";a:3:{s:2:"id";i:7;s:4:"name";s:4:"Alps";s:7:"visible";i:1;}}')
        ->and(unserialize($result))
        ->toBe([
            'stat' => 'ok',
            'result' => [
...
test('encodeResponse flattens a PwgNamedArray to its plain list content, with no attributes merge', function (): void {
    ...
    expect($result)
        ->toBe('a:2:{s:4:"stat";s:2:"ok";s:6:"result";a:3:{i:0;i:10;i:1;i:20;i:2;i:30;}}')
        ->and(unserialize($result))
        ->toBe([
            'stat' => 'ok',
            'result' => [10, 20, 30],
