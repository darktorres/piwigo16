...
test('addMethod with a plain param-name list registers a shorthand signature with default flags/type', function (): void {
    ...
    expect($server->hasMethod('test.method'))->toBeTrue()
        ->and($server->getMethodDescription('test.method'))->toBe('A test method')
        ->and($server->getMethodSignature('test.method'))->toBe([
            'foo' => ['flags' => 0, 'type' => 0],
            'bar' => ['flags' => 0, 'type' => 0],
        ]);
});
...
test('addMethod with a detailed param options map preserves flags/type and sets OPTIONAL when a default is present', function (): void {
    ...
    $server->addMethod('test.method', fn (array $params, PwgServer &$service): array => [], [
        'id' => ['type' => WsParamType::ID],
        'name' => ['default' => 'x'],
    ]);

    $signature = $server->getMethodSignature('test.method');

    expect($signature['id'])->toBe(['type' => WsParamType::ID, 'flags' => 0])
...
test('addMethod treats a null description as an empty string and null params as no params', function (): void {
    ...
    $server->addMethod('test.method', fn (array $params, PwgServer &$service): array => [], null, null, ['admin_only' => true]);

    expect($server->getMethodDescription('test.method'))->toBe('')
        ->and($server->getMethodSignature('test.method'))->toBe([])
        ->and($server->getMethodOptions('test.method'))->toBe(['admin_only' => true]);
});
...
test('hasMethod/getMethodDescription/getMethodSignature/getMethodOptions fall back cleanly for an unregistered method', function (): void {
    ...
    expect($server->hasMethod('nope'))->toBeFalse()
        ->and($server->getMethodDescription('nope'))->toBe('')
        ->and($server->getMethodSignature('nope'))->toBe([])
        ->and($server->getMethodOptions('nope'))->toBe([]);
});
...
test('checkType rejects an invalid scalar of each type with a descriptive PwgError', function (): void {
    ...
    expect($error)->toBeInstanceOf(PwgError::class);
    if ($error instanceof PwgError) {
        expect($error->code())->toBe(WsError::INVALID_PARAM)
            ->and($error->message())->toBe('flag must be a boolean');
    }
    ...
    expect($error)->toBeInstanceOf(PwgError::class);
    if ($error instanceof PwgError) {
        expect($error->message())->toBe('ratio must be a float');
    }
});
...
test('checkType enforces POSITIVE/NOTNULL as a minimum range of 1, appending the right message suffix', function (): void {
    ...
    expect($error)->toBeInstanceOf(PwgError::class);
    if ($error instanceof PwgError) {
        expect($error->message())->toBe('category_id must be an positive and not null integer');
    }
...
test('checkType validates every element when the param is an array', function (): void {
    ...
    expect($error)->toBeInstanceOf(PwgError::class);
    if ($error instanceof PwgError) {
        expect($error->message())->toBe('ids must only contain integers');
    }
...
test('invoke returns INVALID_METHOD for an unregistered method name', function (): void {
    ...
    if ($result instanceof PwgError) {
        expect($result->code())->toBe(WsError::INVALID_METHOD);
    }
});
...
test('invoke returns a 405 for a post_only method called without POST data', function (): void {
    ...
    $_POST = [];
    $server = pwgServerTestServer();
    $server->addMethod('test.postOnly', fn (array $params, PwgServer &$service): array => [], null, null, ['post_only' => true]);
...
test('invoke returns a 401 for an admin_only method called by a non-admin', function (): void {
    ...
    $server->addMethod('test.adminOnly', fn (array $params, PwgServer &$service): array => [], null, null, ['admin_only' => true]);
...
test('invoke returns a 401 when an active API key request targets a config-forbidden method', function (): void {
    ...
    $server->addMethod('test.forbidden', fn (array $params, PwgServer &$service): array => ['ok' => true]);
...
test('invoke returns MISSING_PARAM when a required param is absent, and again when it is present but empty', function (): void {
    ...
    expect($absent)->toBeInstanceOf(PwgError::class);
    if ($absent instanceof PwgError) {
        expect($absent->code())->toBe(WsError::MISSING_PARAM)
            ->and($absent->message())->toBe('Missing parameters: name');
    }

    $empty = $server->invoke('test.method', ['name' => '']);
    expect($empty)->toBeInstanceOf(PwgError::class);
    if ($empty instanceof PwgError) {
        expect($empty->code())->toBe(WsError::MISSING_PARAM);
    }
});
...
test('invoke applies a registered default value for a missing optional param', function (): void {
    ...
    expect($received)->toBe(['name' => 'anonymous']);
});
...
test('invoke rejects an array value for a param that does not accept arrays', function (): void {
    ...
    $result = $server->invoke('test.method', ['name' => ['a', 'b']]);

    expect($result)->toBeInstanceOf(PwgError::class);
    if ($result instanceof PwgError) {
        expect($result->code())->toBe(WsError::INVALID_PARAM)
            ->and($result->message())->toBe('name must be scalar');
    }
});
...
test('invoke force-wraps a scalar into an array when FORCE_ARRAY is set', function (): void {
    ...
    $server->addMethod('test.method', function (array $params, PwgServer &$service) use (&$received): array {
        $received = $params;
        return [];
    }, ['ids' => ['flags' => WsParamFlag::FORCE_ARRAY]]);

    $server->invoke('test.method', ['ids' => '7']);

    expect($received)->toBe(['ids' => ['7']]);
});

test('invoke rejects a param that fails its declared type check', function (): void {
    $server = pwgServerTestServer();
    $server->addMethod('test.method', fn (array $params, PwgServer &$service): array => [], ['category_id' => ['type' => WsParamType::ID]]);

    $result = $server->invoke('test.method', ['category_id' => 'not-an-id']);
...
test('invoke clamps a param above maxValue down to maxValue', function (): void {
    ...
    $server->addMethod('test.method', function (array $params, PwgServer &$service) use (&$received): array {
        $received = $params;
        return [];
    }, ['per_page' => ['type' => WsParamType::INT, 'maxValue' => 100]]);

    $server->invoke('test.method', ['per_page' => '500']);

    expect($received)->toBe(['per_page' => 100]);
});
...
test('invoke calls the real registered callback with the checked params and a reference to the service itself', function (): void {
    ...
    $server->addMethod('test.method', function (array $params, PwgServer &$service): array {
        return ['echo' => $params, 'sameService' => $service->hasMethod('test.method')];
    }, ['name']);

    $result = $server->invoke('test.method', ['name' => 'Alps']);

    expect($result)->toBe(['echo' => ['name' => 'Alps'], 'sameService' => true]);
});
...
test('ws_getMethodList lists only non-hidden methods', function (): void {
    ...
    $server->addMethod('test.hidden', fn (array $params, PwgServer &$service): array => [], null, null, ['hidden' => true]);
...
test('ws_getMethodDetails returns INVALID_PARAM for a non-existent method name', function (): void {
    ...
    $result = PwgServer::ws_getMethodDetails(['methodName' => 'does.not.exist'], $server);
...
test('ws_getMethodDetails describes a real method\'s full param signature', function (): void {
    $server = pwgServerTestServer();
    $server->addMethod('test.method', fn (array $params, PwgServer &$service): array => [], [
        'category_id' => ['type' => WsParamType::ID],
        'name' => ['default' => 'x', 'info' => 'a name'],
    ], 'Does a thing', ['admin_only' => true]);

    $result = PwgServer::ws_getMethodDetails(['methodName' => 'test.method'], $server);

    expect($result)->toBeArray();
    if (is_array($result)) {
        expect($result['name'])->toBe('test.method')
            ->and($result['description'])->toBe('Does a thing')
            ->and($result['options'])->toBe(['admin_only' => true]);
...
test('isAuthorizedMethodForAPIKEY allows any method when no API key is active', function (): void {
    ...
    expect($server->isAuthorizedMethodForAPIKEY('pwg.users.setInfo'))->toBeTrue();
});
...
test('isAuthorizedMethodForAPIKEY blocks a config-forbidden method once the API key flag is active', function (): void {
    ...
    expect($server->isAuthorizedMethodForAPIKEY('pwg.users.setInfo'))->toBeFalse()
        ->and($server->isAuthorizedMethodForAPIKEY('pwg.categories.getList'))->toBeTrue();
});
