...
test.describe('tag management', () => {
    test('create tag, assign to photo, delete tag', async ({ page, request }) => {
        ...
        // Create tag
        const tagCreate = await request.post('/ws.php?format=json', {
            headers: { Cookie: cookie },
            form: { method: 'pwg.tags.add', name: 'e2e-unique-tag' },
        });
        ...
        // Assign tag to photo
        const setInfo = await request.post('/ws.php?format=json', {
            headers: { Cookie: cookie },
            form: {
        ...
        const tagImages = await request.get(
            `/ws.php?format=json&method=pwg.tags.getImages&tag_id=${tagId}`,
            { headers: { Cookie: cookie } },
        );
        ...
        // Delete tag
        const deleteTag = await request.post('/ws.php?format=json', {
            headers: { Cookie: cookie },
            form: { method: 'pwg.tags.delete', tag_id: String(tagId) },
        });
        expect((await deleteTag.json()).stat).toBe('ok');

        // Tag no longer in list
        const tagList = await request.get('/ws.php?format=json&method=pwg.tags.getList', {
            headers: { Cookie: cookie },
        });
