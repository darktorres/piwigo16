...
{foreach from=$languages item=language name=languages_loop}
  <tr class="{if $smarty.foreach.languages_loop.index is odd}row1{else}row2{/if}">
    ...
    <td class="u-text-center"><a href="{$language.EXT_URL}" class="externalLink cluetip" title="{$language.EXT_NAME}|{$language.VER_DESC|@htmlspecialchars|@nl2br}">{$language.VERSION}</a></td>
    <td>{$language.DATE}</td>
    <td>{$language.AUTHOR}</td>
    <td class="u-text-center"><a href="{$language.URL_INSTALL}">{'Install'|@translate}</a>
