    ...
    if (infos.details) {
      $.each(infos.details, function(ext: string, data: any) {
        ...
        if (detail_size > 1048576) {
          ...
          detail_size_nb =  Number((detail_size / 1024).toFixed(0)) < 1 ? (detail_size / 1024).toFixed(2) : (detail_size / 1024).toFixed(0);
        }
        const detail_str_size = detail_str_size_type_string.replace("%s", String(detail_size_nb));
...
function resizeStorageTooltips(resize: boolean = false) {
  $('.storage-chart span').each(function () {
    ...
    const arrow = $('.storage-tooltips #'+$(this).data("type")+' .tooltip-arrow');
    ...
    const tooltip_height = $('.storage-tooltips #'+$(this).data("type")).innerHeight()! + str_chart_height!;
...
function resizeActivityTooltips() {
  $('.activity_tooltips').has('.tooltip').each(function() {
    ...
    const left = $(this).position().left + ($(this).innerWidth()! / 2) + (tooltip.innerWidth()! / 2);
    if (left > max_width) {
