...
{combine_script id='LocalStorageCache' load='footer' path='themes/admin/default/js/LocalStorageCache.js'}
...
<div class="filterBlock">
  <ul id="filterList">
    <li id="filter_prefilter" {if !isset($filter.prefilter)}style="display:none"{/if}>
      ...
      <a id="empty_caddie" href="admin.php?page=batch_manager&amp;action=empty_caddie&amp;pwg_token={$CSRF_TOKEN}" style="{if !isset($filter.prefilter) or $filter.prefilter ne 'caddie'}display:none{/if}">{'Empty caddie'|translate}</a>
{if $NB_ORPHANS > 0}
