...
{if isset($loaded_plugins['rv_gmaps']) && $BODY_ID == "theMapListPage"}
  {footer_script require='jquery'}<script>
    $(document).ready(function() {
      ...
      $('#theMapListPage ul.categoryActions').addClass('nav flex-column').find('li').addClass('nav-item').find('a')
        .addClass('nav-link');
...
{if isset($loaded_plugins['oAuth'])}
  {footer_script require='jquery'}<script>
    $(document).ready(function() {
      ...
      $('#oAuthQuickconnect legend').addClass('dropdown-header').appendTo('#identificationDropdown>.dropdown-menu')
        .changeElementType('li');
...
{if isset($loaded_plugins['BatchDownloader'])}
  {footer_script require='jquery'}<script>
    $(document).ready(function() {
      ...
      $('.batch-downloader-icon').addClass('fas fa-cloud-download-alt fa-fw').removeClass('pwg-icon').after(
        '<span class="d-lg-none"> ' + $('#batchDownloadLink').attr('title') + '</span>');
    });
  ...
  {footer_script require='jquery'}<script>
    $(document).ready(function() {
      if (window.jQuery && window.RVTS && RVTS.start > 0) {
        ...
        $('#rvtsUp').html('<div id="rvtsUp" style="text-align:center;font-size:120%;margin:10px"><a href="' + $f.attr(
          "href") + '">' + $f.html() + '</a> | <a href="javascript:RVTS.loadUp()">' + RVTS.prevMsg + "</a></div>");
      }
    });
  </script>{/footer_script}
{/if}