...
<div class="contact">
  <form method="post" action="{$F_ACTION}">
    <div class="form-group">
      ...
      <input class="form-control" type="text" name="author" id="author" {if $contact.is_logged}
        value="{$contact.author|escape:html}" readonly{else} placeholder="{'Your name'|translate}"
          {/if}{if $contact.mandatory_name and !$contact.is_logged} required{/if}>
      </div>
      <div class="form-group">
        <label for="email">{'Your e-mail'|translate}</label>
        <input class="form-control" type="email" name="email" id="email" {if $contact.is_logged}
          value="{$contact.email|escape:html}" readonly{else} placeholder="{'Your e-mail'|translate}"
            {/if}{if $contact.mandatory_mail and (!$contact.is_logged or empty($contact.email))} required{/if}>
        </div>
        <div class="form-group">
