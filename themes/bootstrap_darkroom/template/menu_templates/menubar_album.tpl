<li id="AMMalbumMenu{$block->data.album_id}" class="nav-item dropdown AMMalbumMenu">
  <a href="{$block->data.link}" class="nav-link{if count($block->data.album) > 0} dropdown-toggle"
    data-bs-toggle="dropdown{/if}">{$block->data.name}</a>
    ...
    {foreach $block->data.album as $cat}
      <a class="dropdown-item{if $cat.SELECTED} active{/if}" data-level="{($cat.LEVEL -1)}" href="{$cat.URL}">
        ...
        {if !empty($cat.icon_ts)}
          <span class="album-recent ms-2" title="{$cat.icon_ts.TITLE}">✻</span>
        {/if}
      </a>
    {/foreach}
