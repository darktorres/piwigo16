<li class="nav-item dropdown" id="identificationDropdown">
    ...
    <div class="dropdown-menu dropdown-menu-right" role="menu">
        ...
        {/if}
        {if isset($U_REGISTER)}
            <a class="dropdown-item" href="{$U_REGISTER}" title="{'Create a new account'|translate}"
                rel="nofollow">{'Register'|translate}</a>
        {/if}
        {if isset($U_LOGOUT)}
        ...
        {/if}
        {if isset($U_ADMIN)}
            <a class="dropdown-item" href="{$U_ADMIN}"
                title="{'available for administrators only'|translate}">{'Administration'|translate}</a>
        {/if}
    </div>
</li>