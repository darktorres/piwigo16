...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll<HTMLElement>('.linkedAlbumPopInContainer .ClosePopIn').forEach(el => {
    el.classList.add(prefix_icon + 'cancel');
  });
  document.querySelectorAll<HTMLElement>('.linkedAlbumPopInContainer .searching').forEach(el => {
  ...
  if (global_params.fields.allwords) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.word').forEach(el => {
      el.checked = true;
    });
    ...
    if (global_params.fields.allwords.words && global_params.fields.allwords.words.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-word').forEach(el => el.classList.add('filter-filled'));
    ...
    Object.keys(word_search_fields).forEach((field_key: string) => {
      const inp = document.querySelector<HTMLInputElement>('#' + word_search_fields[field_key]);
      if (inp) inp.checked = true;
    });

    word_search_mode = global_params.fields.allwords.mode;
    document.querySelectorAll<HTMLInputElement>('.word-search-options input[value="' + word_search_mode + '"]').forEach(el => {
      el.checked = true;
    });
    ...
    document.querySelectorAll<HTMLElement>('.filter-word .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        const ws = document.querySelector<HTMLInputElement>('.filter-word #word-search');
        if (ws) ws.value = '';
        document.querySelectorAll<HTMLInputElement>('.filter-word .search-params input').forEach(inp => {
          inp.checked = true;
        });
        document.querySelectorAll<HTMLInputElement>(".filter-word .word-search-options input[value='AND']").forEach(inp => {
          inp.checked = true;
        });
      });
    });
  ...
  if (global_params.fields.tags) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.tags').forEach(el => {
      el.checked = true;
    });
    document.querySelectorAll<HTMLInputElement>('.filter-tag-form .search-params input[value="' + global_params.fields.tags.mode + '"]').forEach(el => {
      el.checked = true;
    });
    ...
    if (tagSelectTs) {
      (tagSelectTs.getValue() as string[]).forEach((id: string) => {
        const itemText = tagSelectTs!.getItem(id)?.textContent?.replace(/\(\d+ \w+\)×/, '').trim() ?? '';
        tag_search_str += itemText + ', ';
      });
    }
    if (global_params.fields.tags.words && global_params.fields.tags.words.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-tag').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-tag .search-words').forEach(el => {
    ...
    document.querySelectorAll<HTMLElement>('.filter-tag .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        if (tagSelectTs) tagSelectTs.clear();
        document.querySelectorAll<HTMLInputElement>(".filter-tag .search-params input[value='AND']").forEach(inp => {
          inp.checked = true;
        });
      });
    });
  ...
  if (global_params.fields.date_posted) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.date_posted').forEach(el => {
      el.checked = true;
    });
    ...
    if (global_params.fields.date_posted.preset != null && global_params.fields.date_posted.preset != '') {
      ...
      const presetInp = document.querySelector<HTMLInputElement>('#date_posted-' + global_params.fields.date_posted.preset);
      if (presetInp) presetInp.checked = true;
      const datePeriodEl = document.querySelector<HTMLElement>('.date_posted-option label#' + global_params.fields.date_posted.preset + ' .date-period');
      ...
      if ('custom' == global_params.fields.date_posted.preset && global_params.fields.date_posted.custom != null) {
        ...
        customArray.forEach((item: string, index: number) => {
          ...
          const customInp = document.querySelector<HTMLInputElement>('#date_posted_' + customValue);
          if (customInp) {
            customInp.checked = true;
            customInp.classList.add('selected');
            // show the checked-icon in siblings label
            customInp.parentElement?.querySelectorAll<HTMLElement>('label .checked-icon').forEach(icon => {
              icon.style.display = '';
            });
          }

          const periodEl = document.querySelector<HTMLElement>('.date_posted-option label#' + customValue + ' .date-period');
      ...
      document.querySelectorAll<HTMLElement>('.filter-date_posted').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-date_posted .search-words').forEach(el => {
        el.textContent = date_posted_str;
      });
    }

    document.querySelectorAll<HTMLElement>('.filter-date_posted .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        updateFilters('date_posted', 'add');
        document.querySelectorAll<HTMLInputElement>('.date_posted-option input').forEach(inp => {
          inp.checked = false;
    ...
    document.querySelectorAll<HTMLElement>('.custom_posted_date_toggle').forEach(el => {
      el.addEventListener('click', () => {
        document.querySelectorAll<HTMLElement>('.custom_posted_date').forEach(d => toggleDisplay(d));
        document.querySelectorAll<HTMLElement>('.preset_posted_date').forEach(d => toggleDisplay(d));
      });
    });

    // Handle accordion features in custom options
    document.querySelectorAll<HTMLElement>('.custom_posted_date .accordion-toggle').forEach(el => {
      el.addEventListener('click', () => {
        const clickedOption = el.parentElement;
        if (clickedOption) {
          clickedOption.classList.toggle('show-child');
          if ('year' == el.dataset['type']) {
            clickedOption.parentElement?.querySelectorAll<HTMLElement>('.date_posted-option.month').forEach(m => toggleDisplay(m));
          } else if ('month' == el.dataset['type']) {
            clickedOption.parentElement?.querySelectorAll<HTMLElement>('.date_posted-option.day').forEach(d => toggleDisplay(d));
          }
        }
      });
    });
    ...
    document.querySelectorAll<HTMLInputElement>('.custom_posted_date .date_posted-option input').forEach(inp => {
      inp.addEventListener('change', () => {
        ...
        if (document.querySelectorAll('.custom_posted_date input:checked').length > 0) {
          ...
          document.querySelectorAll<HTMLInputElement>('.preset_posted_date input').forEach(pi => {
  ...
  if (global_params.fields.date_created) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.date_created').forEach(el => {
      el.checked = true;
    });

    if (global_params.fields.date_created.preset != null && global_params.fields.date_created.preset != '') {
      // If filter is used and not empty check preset date option
      const presetInp = document.querySelector<HTMLInputElement>('#date_created-' + global_params.fields.date_created.preset);
      if (presetInp) presetInp.checked = true;
      const datePeriodEl = document.querySelector<HTMLElement>('.date_created-option label#' + global_params.fields.date_created.preset + ' .date-period');
      ...
      if ('custom' == global_params.fields.date_created.preset && global_params.fields.date_created.custom != null) {
        ...
        customArray.forEach((item: string, index: number) => {
          ...
          const customInp = document.querySelector<HTMLInputElement>('#date_created_' + customValue);
          if (customInp) {
            customInp.checked = true;
            customInp.classList.add('selected');
            customInp.parentElement?.querySelectorAll<HTMLElement>('label .checked-icon').forEach(icon => {
          ...
          const periodEl = document.querySelector<HTMLElement>('.date_created-option label#' + customValue + ' .date-period');
        ...
        document.querySelectorAll<HTMLElement>('.date_created-option.year').forEach(el => {
          updateDateFilters(`.custom_created_date #${el.id}`);
        });
      }

      // change badge label if filter not empty
      document.querySelectorAll<HTMLElement>('.filter-date_created').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-date_created .search-words').forEach(el => {
        el.textContent = date_created_str;
      });
    }

    document.querySelectorAll<HTMLElement>('.filter-date_created .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        updateFilters('date_created', 'add');
        document.querySelectorAll<HTMLInputElement>('.date_created-option input').forEach(inp => {
          inp.checked = false;
          inp.dispatchEvent(new Event('change'));
        });
      });
    ...
    document.querySelectorAll<HTMLElement>('.custom_created_date_toggle').forEach(el => {
      el.addEventListener('click', () => {
        document.querySelectorAll<HTMLElement>('.custom_created_date').forEach(d => toggleDisplay(d));
        document.querySelectorAll<HTMLElement>('.preset_created_date').forEach(d => toggleDisplay(d));
      });
    });

    // Handle accordion features in custom options
    document.querySelectorAll<HTMLElement>('.custom_created_date .accordion-toggle').forEach(el => {
      el.addEventListener('click', () => {
        const clickedOption = el.parentElement;
        if (clickedOption) {
          clickedOption.classList.toggle('show-child');
          if ('year' == el.dataset['type']) {
            clickedOption.parentElement?.querySelectorAll<HTMLElement>('.date_created-option.month').forEach(m => toggleDisplay(m));
          } else if ('month' == el.dataset['type']) {
            clickedOption.parentElement?.querySelectorAll<HTMLElement>('.date_created-option.day').forEach(d => toggleDisplay(d));
          }
        }
      });
    });
    ...
    // On custom date input select
    document.querySelectorAll<HTMLInputElement>('.custom_created_date .date_created-option input').forEach(inp => {
      inp.addEventListener('change', () => {
        const currentYear = inp.dataset['year'];
        ...
        if (document.querySelectorAll('.custom_created_date input:checked').length > 0) {
          ...
          if (customChk) customChk.checked = true;
          document.querySelectorAll<HTMLInputElement>('.preset_created_date input').forEach(pi => {
            pi.setAttribute('disabled', 'disabled');
          });
        } else {
          const customChk = document.querySelector<HTMLInputElement>('#date_created-custom');
  ...
  if (global_params.fields.cat) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.album').forEach(el => {
      el.checked = true;
    });
    ...
    document.querySelectorAll<HTMLElement>('.filter-album .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        // (no-op selector retained as comment: .filter-album .search-params input[value='AND'])
        ab.resetAll();
        const selCatsCont = document.querySelector<HTMLElement>('.selected-categories-container');
        if (selCatsCont) selCatsCont.innerHTML = '';
        const searchSubCatsInp = document.querySelector<HTMLInputElement>('#search-sub-cats');
  ...
  if (authorsEl) {
    ...
    if (global_params.fields.author) {
      ...
      document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.author').forEach(el => {
        el.checked = true;
      });

      author_search_str = '';
      (authorSelectTs.getValue() as string[]).forEach((id: string) => {
        const itemText = authorSelectTs!.getItem(id)?.textContent?.replace(/\(\d+ \w+\)×/, '').trim() ?? '';
        author_search_str += itemText + ', ';
      });

      if (global_params.fields.author.words && global_params.fields.author.words.length > 0) {
        document.querySelectorAll<HTMLElement>('.filter-authors').forEach(el => el.classList.add('filter-filled'));
        document.querySelectorAll<HTMLElement>('.filter.filter-authors .search-words').forEach(el => {
      ...
      document.querySelectorAll<HTMLElement>('.filter-authors .filter-actions .clear').forEach(el => {
        el.addEventListener('click', () => {
  ...
  if (global_params.fields.added_by) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.added_by').forEach(el => {
      el.checked = true;
    });

    if (global_params.fields.added_by && global_params.fields.added_by.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-added_by').forEach(el => el.classList.add('filter-filled'));
      ...
      document.querySelectorAll<HTMLElement>('.filter.filter-added_by .search-words').forEach(el => {
    ...
    document.querySelectorAll<HTMLElement>('.filter-added_by .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        document.querySelectorAll<HTMLInputElement>('.filter-added_by .added_by-option input').forEach(inp => {
          inp.checked = false;
        });
      });
    });
  ...
  if (global_params.fields.filetypes) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.filetypes').forEach(el => {
      el.checked = true;
    });
    ...
    if (global_params.fields.filetypes && global_params.fields.filetypes.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-filetypes').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-filetypes .search-words').forEach(el => {
        el.textContent = filetypes_search_str.toUpperCase().slice(0, -2);
      });

      document.querySelectorAll<HTMLInputElement>('.filetypes-option input').forEach(inp => {
    ...
    document.querySelectorAll<HTMLElement>('.filter-filetypes .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        document.querySelectorAll<HTMLInputElement>('.filter-filetypes .filetypes-option input').forEach(inp => {
          inp.checked = false;
        });
      });
    });
  ...
  if (global_params.fields.ratios) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.ratios').forEach(el => {
      el.checked = true;
    });
    ...
    if (global_params.fields.ratios && global_params.fields.ratios.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-ratios').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-ratios .search-words').forEach(el => {
    ...
    document.querySelectorAll<HTMLElement>('.filter-ratios .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        document.querySelectorAll<HTMLInputElement>('.filter-ratios .ratios-option input').forEach(inp => {
          inp.checked = false;
        });
      });
    });
  ...
  if (global_params.fields.ratings && show_filter_ratings) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.ratings').forEach(el => {
      el.checked = true;
    });
    ...
    if (global_params.fields.ratings && global_params.fields.ratings.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-ratings').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-ratings .search-words').forEach(el => {
    ...
    document.querySelectorAll<HTMLElement>('.filter-ratings .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        document.querySelectorAll<HTMLInputElement>('.filter-ratings .ratings-option input').forEach(inp => {
          inp.checked = false;
        });
      });
    });
  ...
  if (global_params.fields.filesize_min != null && global_params.fields.filesize_max != null) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.filesize').forEach(el => {
      el.checked = true;
    });
    ...
    document.querySelectorAll<HTMLElement>('[data-slider=filesizes]').forEach(sliderEl => {
      sliderEl.addEventListener('slidestop', () => {
        ...
        const fsMinText = document.querySelector<HTMLInputElement>('input[name=filter_filesize_min_text]');
        if (fsMinText) {
          fsMinText.value = min;
          fsMinText.dispatchEvent(new Event('change'));
        }
        const fsMaxText = document.querySelector<HTMLInputElement>('input[name=filter_filesize_max_text]');
        if (fsMaxText) {
          fsMaxText.value = max;
    ...
    document.querySelectorAll<HTMLElement>('.filter-filesize .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (filterFilesizeEl && filterFilesizeEl.classList.contains('filter-filled')) {
          ...
          document.querySelectorAll<HTMLElement>('.filter.filter-filesize .search-words').forEach(sw => {
  ...
  if (global_params.fields.height_min != null && global_params.fields.height_max != null) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.height').forEach(el => {
      el.checked = true;
    });
    ...
    document.querySelectorAll<HTMLElement>('.filter-height .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (filterHeightEl && filterHeightEl.classList.contains('filter-filled')) {
          ...
          document.querySelectorAll<HTMLElement>('.filter.filter-height .search-words').forEach(sw => {
  ...
  if (global_params.fields.width_min != null && global_params.fields.width_max != null) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.width').forEach(el => {
      el.checked = true;
    });
    ...
    document.querySelectorAll<HTMLElement>('.filter-width .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (filterWidthEl && filterWidthEl.classList.contains('filter-filled')) {
          ...
          document.querySelectorAll<HTMLElement>('.filter.filter-width .search-words').forEach(sw => {
  ...
  if (global_params.fields.expert) {
    ...
    document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.expert').forEach(el => {
      el.checked = true;
    });
    ...
    if (global_params.fields.expert.string && global_params.fields.expert.string.length > 0) {
      document.querySelectorAll<HTMLElement>('.filter-expert').forEach(el => el.classList.add('filter-filled'));
      document.querySelectorAll<HTMLElement>('.filter.filter-expert .search-words').forEach(el => {
    ...
    document.querySelectorAll<HTMLElement>('.filter-expert .filter-actions .clear').forEach(el => {
      el.addEventListener('click', () => {
        const expInp = document.querySelector<HTMLInputElement>('.filter-expert #expert-search');
  ...
  if (!empty_filters_list.every((param: any) => param === '' || param === null || (typeof param == 'undefined'))) {
    document.querySelectorAll<HTMLElement>('.clear-all').forEach(el => el.classList.add('clickable'));
    document.querySelectorAll<HTMLElement>('.clear-all.clickable').forEach(el => {
      el.addEventListener('click', () => {
        const exclude_params = ['search_id', 'allwords_mode', 'allwords_fields', 'tags_mode', 'categories_withsubs'];
  ...
  document.addEventListener('keyup', (e: KeyboardEvent) => {
    // 27 is 'Escape'
    if (e.keyCode === 27) {
      document.querySelector<HTMLElement>('.filter-manager-popin .filter-manager-close')?.dispatchEvent(new Event('click'));
      document.querySelector<HTMLElement>('#closeModalQuickSearch')?.dispatchEvent(new Event('click'));
    }
    // 13 is 'Enter'
    if (e.keyCode === 13) {
      document.querySelectorAll<HTMLElement>('.filter-form .filter-validate').forEach(el => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-manager-popin').forEach(popinEl => {
    popinEl.addEventListener('click', (e: Event) => {
      if (e.target === popinEl) {
        popinEl.querySelector<HTMLElement>('.filter-manager-close')?.dispatchEvent(new Event('click'));
      }
    });
  });

  document.querySelectorAll<HTMLElement>('.filter-manager-popin .filter-cancel, .filter-manager-popin .filter-manager-close').forEach(el => {
    el.addEventListener('click', () => {
      const popin = document.querySelector<HTMLElement>('.filter-manager-popin');
      if (popin) popin.style.display = 'none';
      document.querySelectorAll<HTMLInputElement>('.filter-manager-controller-container input').forEach(inp => {
        if (inp.checked) {
          const widFilter = document.querySelector<HTMLElement>('.filter.filter-' + inp.dataset['wid']);
          if (widFilter && widFilter.style.display === 'none') {
            inp.checked = false;
          }
        } else {
          const widFilter = document.querySelector<HTMLElement>('.filter.filter-' + inp.dataset['wid']);
          if (widFilter && widFilter.style.display !== 'none' && widFilter.offsetParent !== null) {
            inp.checked = true;
          }
        }
      });
  ...
  document.querySelectorAll<HTMLElement>('.filter-manager-popin .filter-validate').forEach(el => {
    el.addEventListener('click', () => {
      document.querySelectorAll<HTMLInputElement>('.filter-manager-controller-container input').forEach(inp => {
        if (inp.checked) {
          const widFilter = document.querySelector<HTMLElement>('.filter.filter-' + inp.dataset['wid']);
  ...
  document.addEventListener('keyup', (e: KeyboardEvent) => {
    // 27 is 'Escape'
    if (e.keyCode === 27) {
      document.querySelector<HTMLElement>('.tags-found-popin .tags-found-close')?.dispatchEvent(new Event('click'));
      document.querySelector<HTMLElement>('.albums-found-popin .albums-found-close')?.dispatchEvent(new Event('click'));
    }
  });
  ...
  document.querySelectorAll<HTMLElement>('.tags-found-popin').forEach(popinEl => {
    popinEl.addEventListener('click', (e: Event) => {
      if (e.target === popinEl) {
        popinEl.querySelector<HTMLElement>('.tags-found-close')?.dispatchEvent(new Event('click'));
      }
    });
  });
  ...
  document.querySelectorAll<HTMLElement>('.albums-found-popin').forEach(popinEl => {
    popinEl.addEventListener('click', (e: Event) => {
      if (e.target === popinEl) {
        popinEl.querySelector<HTMLElement>('.albums-found-close')?.dispatchEvent(new Event('click'));
      }
    });
  });
  ...
  if (filterWordEl) {
    filterWordEl.addEventListener('click', (e: Event) => {
      ...
      const target = e.target as HTMLElement;
      if ((filterForm && filterForm.contains(target)) || target.classList.contains('filter-form')) {
        return;
      }
      ...
      // toggle visibility
      const isVisible = filterWordForm.style.display !== 'none' && filterWordForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        global_params.fields.allwords.words = (document.querySelector<HTMLInputElement>('#word-search'))?.value ?? '';
        global_params.fields.allwords.mode = (document.querySelector<HTMLInputElement>('.word-search-options input:checked'))?.getAttribute('value') ?? '';

        PS_params.allwords = (document.querySelector<HTMLInputElement>('#word-search'))?.value ?? '';
        PS_params.allwords_mode = (document.querySelector<HTMLInputElement>('.word-search-options input:checked'))?.getAttribute('value') ?? '';

        const new_fields: string[] = [];
        document.querySelectorAll<HTMLInputElement>('.filter-word-form .search-params input:checked').forEach(inp => {
          if (inp.getAttribute('name') == 'tags') {
            global_params.fields.search_in_tags = true;
          }
          new_fields.push(inp.getAttribute('name') ?? '');
        });
        if (document.querySelectorAll(".filter-word-form .search-params input[name='tags']:checked").length == 0) {
  ...
  document.querySelectorAll<HTMLElement>('.filter-word .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterWordEl && !filterWordEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.word').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterTagEl) {
    filterTagEl.addEventListener('click', (e: Event) => {
      ...
      const target = e.target as HTMLElement;
      if ((filterForm && filterForm.contains(target)) || target.classList.contains('filter-form') || target.classList.contains('remove')) {
        return;
      }
      const filterTagForm = document.querySelector<HTMLElement>('.filter-tag-form');
      if (!filterTagForm) return;

      const isVisible = filterTagForm.style.display !== 'none' && filterTagForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        global_params.fields.tags.mode = (document.querySelector<HTMLInputElement>('.filter-tag-form .search-params input:checked'))?.value ?? '';
        global_params.fields.tags.words = tagSelectTs ? tagSelectTs.getValue() as string[] : [];

        const tsVal = tagSelectTs ? tagSelectTs.getValue() as string[] : [];
        PS_params.tags = tsVal.length > 0 ? tsVal : '';
        PS_params.tags_mode = (document.querySelector<HTMLInputElement>('.filter-tag-form .search-params input:checked'))?.value ?? '';
      }
    });
  }
  ...
  document.querySelectorAll<HTMLElement>('.filter-tag .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterTagEl && !filterTagEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.tags').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterDatePostedEl) {
    filterDatePostedEl.addEventListener('click', (e: Event) => {
      ...
      const filterDatePostedForm = document.querySelector<HTMLElement>('.filter-date_posted-form');
      if (!filterDatePostedForm) return;

      const isVisible = filterDatePostedForm.style.display !== 'none' && filterDatePostedForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        const presetValue = (document.querySelector<HTMLInputElement>('.preset_posted_date .date_posted-option input:checked'))?.value;
        ...
        if ('custom' == presetValue) {
          ...
          document.querySelectorAll<HTMLInputElement>('.custom_posted_date .date_posted-option input:checked').forEach(inp => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-date_posted .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterDatePostedEl && !filterDatePostedEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.date').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterDateCreatedEl) {
    filterDateCreatedEl.addEventListener('click', (e: Event) => {
      ...
      const filterDateCreatedForm = document.querySelector<HTMLElement>('.filter-date_created-form');
      if (!filterDateCreatedForm) return;

      const isVisible = filterDateCreatedForm.style.display !== 'none' && filterDateCreatedForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        const presetValue = (document.querySelector<HTMLInputElement>('.preset_created_date .date_created-option input:checked'))?.value;
        ...
        if ('custom' == presetValue) {
          ...
          document.querySelectorAll<HTMLInputElement>('.custom_created_date .date_created-option input:checked').forEach(inp => {
            customDates.push(inp.value);
          });
  ...
  document.querySelectorAll<HTMLElement>('.filter-date_created .filter-validate').forEach(el => {
    el.addEventListener('click', () => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-date_created .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      updateFilters('date_created', 'del');
      performSearch(PS_params, true);
      if (filterDateCreatedEl && !filterDateCreatedEl.classList.contains('filter-filled')) {
  ...
  if (filterAlbumEl) {
    filterAlbumEl.addEventListener('click', (e: Event) => {
      ...
      const target = e.target as HTMLElement;
      if ((filterForm && filterForm.contains(target)) || target.classList.contains('filter-form') || target.classList.contains('remove-item')) {
        return;
      }
      const filterAlbumForm = document.querySelector<HTMLElement>('.filter-album-form');
      if (!filterAlbumForm) return;

      const isVisible = filterAlbumForm.style.display !== 'none' && filterAlbumForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        global_params.fields.cat.sub_inc = document.querySelectorAll("input[name='search-sub-cats']:checked").length != 0;

        PS_params.categories = ab.get_selected_albums().length > 0 ? ab.get_selected_albums() : '';
        PS_params.categories_withsubs = document.querySelectorAll("input[name='search-sub-cats']:checked").length != 0;
      }
    });
  }
  ...
  document.querySelectorAll<HTMLElement>('.filter-album .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      updateFilters('album', 'del');
      performSearch(PS_params, true);
      if (filterAlbumEl && !filterAlbumEl.classList.contains('filter-filled')) {
        filterAlbumEl.style.display = 'none';
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.album').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterAuthorsEl) {
    filterAuthorsEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterAuthorForm.style.display !== 'none' && filterAuthorForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        global_params.fields.author.words = authorSelectTs ? authorSelectTs.getValue() as string[] : [];
  ...
  document.querySelectorAll<HTMLElement>('.filter-authors .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterAuthorsEl && !filterAuthorsEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.author').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterAddedByEl) {
    filterAddedByEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterAddedByForm.style.display !== 'none' && filterAddedByForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        document.querySelectorAll<HTMLInputElement>('.added_by-option input:checked').forEach(inp => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-added_by .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterAddedByEl && !filterAddedByEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.added_by').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterFiletypesEl) {
    filterFiletypesEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterFiletypesForm.style.display !== 'none' && filterFiletypesForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filetypes-option input:checked').forEach(inp => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-filetypes .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterFiletypesEl && !filterFiletypesEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.filetypes').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterRatiosEl) {
    filterRatiosEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterRatiosForm.style.display !== 'none' && filterRatiosForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        document.querySelectorAll<HTMLInputElement>('.ratios-option input:checked').forEach(inp => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-ratios .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterRatiosEl && !filterRatiosEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.ratios').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterRatingsEl) {
    filterRatingsEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterRatingsForm.style.display !== 'none' && filterRatingsForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        document.querySelectorAll<HTMLInputElement>('.ratings-option input:checked').forEach(inp => {
  ...
  document.querySelectorAll<HTMLElement>('.filter-ratings .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterRatingsEl && !filterRatingsEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.ratings').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterFilesizeEl) {
    filterFilesizeEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterFilesizeForm.style.display !== 'none' && filterFilesizeForm.offsetParent !== null;
  ...
  document.querySelectorAll<HTMLElement>('.filter-filesize .filter-validate').forEach(el => {
    el.addEventListener('click', () => {
      filesize_min = Math.floor((parseFloat((document.querySelector<HTMLInputElement>('input[name=filter_filesize_min]'))?.value ?? '0')) * 1024);
      filesize_max = Math.ceil((parseFloat((document.querySelector<HTMLInputElement>('input[name=filter_filesize_max]'))?.value ?? '0')) * 1024);
  ...
  document.querySelectorAll<HTMLElement>('.filter-filesize .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      updateFilters('filesize', 'del');
      performSearch(PS_params, true);
      if (filterFilesizeEl && !filterFilesizeEl.classList.contains('filter-filled')) {
        filterFilesizeEl.style.display = 'none';
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.filesize').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterHeightEl) {
    filterHeightEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterHeightForm.style.display !== 'none' && filterHeightForm.offsetParent !== null;
  ...
  document.querySelectorAll<HTMLElement>('.filter-height .filter-validate').forEach(el => {
    el.addEventListener('click', () => {
      height_min = (document.querySelector<HTMLInputElement>('input[name=filter_height_min]'))?.value ?? '';
      height_max = (document.querySelector<HTMLInputElement>('input[name=filter_height_max]'))?.value ?? '';
  ...
  document.querySelectorAll<HTMLElement>('.filter-height .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      updateFilters('height', 'del');
      performSearch(PS_params, true);
      if (filterHeightEl && !filterHeightEl.classList.contains('filter-filled')) {
        filterHeightEl.style.display = 'none';
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.height').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterWidthEl) {
    filterWidthEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterWidthForm.style.display !== 'none' && filterWidthForm.offsetParent !== null;
  ...
  document.querySelectorAll<HTMLElement>('.filter-width .filter-validate').forEach(el => {
    el.addEventListener('click', () => {
      width_min = (document.querySelector<HTMLInputElement>('input[name=filter_width_min]'))?.value ?? '';
      width_max = (document.querySelector<HTMLInputElement>('input[name=filter_width_max]'))?.value ?? '';
  ...
  document.querySelectorAll<HTMLElement>('.filter-width .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      updateFilters('width', 'del');
      performSearch(PS_params, true);
      if (filterWidthEl && !filterWidthEl.classList.contains('filter-filled')) {
        filterWidthEl.style.display = 'none';
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.width').forEach(inp => {
          inp.checked = false;
        });
      }
    });
  ...
  if (filterExpertEl) {
    filterExpertEl.addEventListener('click', (e: Event) => {
      ...
      const isVisible = filterExpertForm.style.display !== 'none' && filterExpertForm.offsetParent !== null;
      ...
      if (!isVisible) {
        ...
        global_params.fields.expert = (document.querySelector<HTMLInputElement>('#expert-search'))?.value ?? '';
        PS_params.expert = (document.querySelector<HTMLInputElement>('#expert-search'))?.value ?? '';
      }
    });
  }
  ...
  document.querySelectorAll<HTMLElement>('.filter-expert .filter-actions .delete').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (filterExpertEl && !filterExpertEl.classList.contains('filter-filled')) {
        ...
        document.querySelectorAll<HTMLInputElement>('.filter-manager-controller.expert').forEach(inp => {
          inp.checked = false;
        });
      }
    });
...
function performSearch(params: Record<string, any>, reload: boolean = false): void {
  fetch('ws.php?format=json&method=pwg.images.filteredSearch.create', {
    ...
    .catch((e: any) => {
      ...
      document.querySelectorAll<HTMLElement>('.filter-validate .validate-text').forEach(el => {
...
function add_related_category({ album, addSelectedAlbum }: { album: { id: string; name: string }; addSelectedAlbum: () => void }): void {
  display_related_category(album.id, album.name);
  document.querySelector<HTMLElement>('.invisible-related-categories-select')?.insertAdjacentHTML(
    'beforeend',
...
function updateDateFilters(selector: string): void {
  ...
  if (inputYear && inputYear.checked) {
    ...
    yearIsCheck = true;
    ctx.querySelectorAll<HTMLInputElement>(':not(:checked)').forEach(inp => inp.setAttribute('disabled', 'true'));
    if (iconYear) {
    ...
    ctx.querySelectorAll<HTMLInputElement>(':is(input)').forEach(inp => inp.setAttribute('disabled', 'false'));
    if (iconYear) {
  ...
  allMonth.forEach(monthEl => {
    ...
    if (monthInput && monthInput.checked) {
      ...
      allDays.forEach(dayEl => {
        dayEl.querySelectorAll<HTMLInputElement>(':not(:checked)').forEach(inp => inp.setAttribute('disabled', 'true'));
      });
      if (iconMonth) {
    ...
    } else if (Array.from(allDays).some(dayEl => dayEl.querySelectorAll(':checked').length > 0)) {
      if (iconMonth) {
...
function resize_filter_form(): void {
  ...
  document.querySelectorAll<HTMLElement>('.filter').forEach(filterEl => {
    ...
    if (left_distance + filter_form_width > window_width) {
      ...
      const replace_form_width = -filter_form_width + filterEl.offsetWidth + check_left - mobile_marg;
      filterForm.style.left = replace_form_width + 'px';
    }
    if (!is_desktop) {
      const left_arrow = left_distance + (filterEl.offsetWidth / 2);
      filterForm.insertAdjacentHTML('afterbegin', '<svg width="10" height="10" viewBox="0 0 14 14" class="form_mobile_arrow" style="left:' + left_arrow + 'px"><polygon class="arrow-border" points="7,0 14,14 0,14"/><polygon class="arrow-fill" points="7,1 13.5,14 0.5,14"/></svg>');
    }
  });
}
