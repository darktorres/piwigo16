...
function makeNiceRatingForm(options: PwgRatingOptions)
{
	...
	gRatingButtons = form.getElementsByTagName('input') as HTMLCollectionOf<RatingButton>;
	gUserRating = "";
	...
	for (let i=0; i<gRatingButtons.length; i++)
	{
		...
		try { rateButton.type = "button"; } catch { /* avoid normal submit (use ajax); not working in IE6 */ }
...
function updateRating(e: Event)
{
	...
	$.ajax({
		...
		error: function(jqXHR) {
			...
			document.location.href = rateButton.form!.action + "&rate="+rateButton.initialRateValue;
		},
...
if (typeof window._pwgRatingAutoQueue!="undefined" && window._pwgRatingAutoQueue.length)
{
