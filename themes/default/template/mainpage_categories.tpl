...
{/if}
  <li class="{if $smarty.foreach.cat_loop.index is odd}odd{else}even{/if}">
		<div class="thumbnailCategory">
			...
			<a href="{$cat.URL}">
				<img {if !$cat.representative}src="{$ROOT_URL}{$themeconf.icon_dir}/img_small.png"{elseif $derivative->isCached()}src="{$derivative->getUrl()}"{else}src="{$ROOT_URL}{$themeconf.icon_dir}/img_small.png" data-src="{$derivative->getUrl()}"{/if} alt="{$cat.TN_ALT}" title="{$cat.NAME|@replace:'"':' '|@strip_tags:false} - {'display this album'|@translate}">
			</a>
			</div>
