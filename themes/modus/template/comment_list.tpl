	...
	{foreach from=$comments item=comment name=comment_loop}
		<li class="commentElement {if $smarty.foreach.comment_loop.index is odd}odd{else}even{/if}">
			...
			<div class="description">
				...
				<span class="commentAuthor">{if $comment.WEBSITE_URL}<a href="{$comment.WEBSITE_URL}" class="external"
						target="_blank" rel="nofollow">{$comment.AUTHOR}</a>{else}{$comment.AUTHOR}
					{/if}</span>
				...
				{if isset($comment.IN_EDIT)}
					...
					<form method="post" action="{$comment.U_EDIT}" id="editComment">
						...
						<p><textarea name="content" id="contenteditid" rows="5" cols="80">{$comment.CONTENT|@escape}</textarea>
						</p>
