...
{if !empty($blocks)}
	<aside id=menubar>
		{foreach $blocks as $id => $block}
			{if ($id!="mbIdentification" && $id!="mbMenu")}
				<dl id={$id}>
					{if $id=="mbLinks"}
						{* ============ mbLinks ========== *}
				...
				{elseif $id=="mbSpecials"}
					...
					<dd>
						<ul>
							{foreach $block->data as $link}
								<li><a href="{$link.URL}" title="{$link.TITLE}" {if isset($link.REL)} {$link.REL}{/if}>{$link.NAME}</a>
								</li>
							{/foreach}
	...
	{if isset($blocks.mbSpecials->data.most_visited)}
		<dl id="mbMostVisited">
			<dt><a href="{$blocks.mbSpecials->data.most_visited.URL}"
					title="{$blocks.mbSpecials->data.most_visited.TITLE}">{$blocks.mbSpecials->data.most_visited.NAME}</a>
			</dt>
		</dl>
	{/if}
	{if isset($blocks.mbSpecials->data.best_rated)}
		<dl id="mbBestRated">
			<dt><a href="{$blocks.mbSpecials->data.best_rated.URL}"
					title="{$blocks.mbSpecials->data.best_rated.TITLE}">{$blocks.mbSpecials->data.best_rated.NAME}</a></dt>
		</dl>
	{/if}
	{if isset($blocks.mbSpecials->data.recent_pics)}
		<dl>
			<dt><a href="{$blocks.mbSpecials->data.recent_pics.URL}"
					title="{$blocks.mbSpecials->data.recent_pics.TITLE}">{$blocks.mbSpecials->data.recent_pics.NAME}</a>
			</dt>
		</dl>
	{/if}
	<dl style="float:none">
