...
pub fn gps_coordinates(tags: &[Tag]) -> Vec<Tag> {
    ...
    for (coord, reftag, pos_ref, neg_ref) in [
        ...
        // GPS.pm's own GPSLatitude/GPSLongitude ValueConv is
        // `Image::ExifTool::GPS::ToDegrees($val)` -- it runs BEFORE this
        // composite, converting the 3-rational DMS value to a plain decimal
        // degrees float, so `$val[0]` in this composite's own ValueConv
        // (quoted above) is already that unsigned float, not the raw
        // rational triple. This crate's GPS tag never gets that ValueConv
        // applied to its own raw_value (only to print_value, via
        // metadata/exif.rs's DMS-reformatting pass), so it has to be
        // recomputed here from `src.raw_value`'s space-joined rational
        // components rather than reused -- otherwise this composite's own
        // raw_value stays the unconverted rational list forever (no
        // ExifTool consumer, including Piwigo's own `-GPSLatitude#`
        // integration, can get a decimal-degrees number out of it at all).
        let magnitude = src
            .raw_value
            .to_display_string()
            .split_whitespace()
            ...
            .collect::<Vec<f64>>();
        let raw = match magnitude.as_slice() {
            [deg, min, sec] => {
                let decimal = deg + min / 60.0 + sec / 3600.0;
                Value::F64(if negative { -decimal } else { decimal })
            }
            // Not a 3-component DMS rational (e.g. the "undefined coordinate"
            // 0/0 0/0 0/0 case, which stringifies to "undef undef undef" and
            // fails the f64 parse) -- fall back to whatever the plain GPS tag
            // itself carries rather than fabricate a number.
            _ => src.raw_value.clone(),
        };
        let mut t = mk_composite_raw(coord, src.description.clone().as_str(), raw, print);
        t.group.family2 = "Location".into();
        // GPS.pm's own Priority => 1 (quoted in this function's docblock
        // above) is what actually makes this composite outrank the plain
        // GPS:GPSLatitude/GPSLongitude tag of the same name in real
        // ExifTool -- mk_composite_raw()'s generic `priority: 0` default
        // ties with the plain tag's own default priority (also 0), and a
        // tie keeps whichever tag came first (the plain one, built before
        // composites are appended), silently discarding this composite's
        // correct signed-decimal value the moment `-a`/`-duplicates`
        // re-exposes the plain tag alongside it (confirmed live: without
        // this, `-a -GPSLatitude#` -- exactly what ExifToolProcess always
        // sends -- returned the plain tag's raw "41 54 9.72" instead of the
        // composite's signed -41.9027).
        t.priority = 1;
        out.push(t);
    }
    out
