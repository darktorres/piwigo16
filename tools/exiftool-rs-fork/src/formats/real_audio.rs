...
fn read_ra4(data: &[u8]) -> Result<Vec<Tag>> {
    ...
    if pos + copy_len <= d.len() && copy_len > 0 {
        // Decode UTF-8 when valid, else Latin-1 — like Title/Artist above — so
        // an accented byte like 0xFC surfaces as `ü`, matching the characters
        // ExifTool shows (it passes the raw bytes through).
        let copyright = crate::encoding::decode_utf8_or_latin1(&d[pos..pos + copy_len]);
        tags.push(mktag(
            "Real",
            "Copyright",
            "Copyright",
