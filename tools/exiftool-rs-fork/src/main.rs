    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            ...
            arg if arg.starts_with('-') && arg.len() > 1 && !arg.starts_with("--") => {
                apply_tag_request(&arg[1..], &mut options);
            }
...
/// Parse a `-TAG`, `-GROUP:TAG` or `-TAG#`/`-GROUP:TAG#` request (the `-`
/// already stripped) into `options.requested_tags`/`numeric_tags`. Shared
/// between one-shot CLI parsing and `run_stay_open()`'s per-batch parsing so
/// the two can't drift apart.
fn apply_tag_request(arg: &str, options: &mut Options) {
    let mut tag_req = arg.to_string();
    // ExifTool's per-tag numeric-format suffix: `-TAG#` or `-GROUP:TAG#`
    // requests TAG's raw (non-print-converted) value specifically,
    // independent of the global `-n`. The `#` must be stripped before the
    // name is compared in tag_matches_request(), or it's compared verbatim
    // against real tag names and never matches anything.
    if let Some(stripped) = tag_req.strip_suffix('#') {
        let bare_name = stripped.rsplit(':').next().unwrap_or(stripped);
        options.numeric_tags.insert(bare_name.to_lowercase());
        tag_req = stripped.to_string();
    }
    options.requested_tags.push(tag_req);
}

/// Real ExifTool's `-stay_open` protocol (exiftool's own `application.pl`
/// `Main::` loop) reads a per-request option/tag/file batch from stdin,
/// terminated by `-executeN`, and echoes a `{readyN}` marker matching
/// whatever numeric (or empty) suffix the client sent -- not a fixed bare
/// `{ready}` -- so a client pipelining several requests can tell which
/// response belongs to which, and stays silent until the first command
/// completes (no eager startup `{ready}`).
///
/// This crate's original implementation replicated none of that: it matched
/// only the bare literal `-execute` line (so any real caller's `-executeN`,
/// this workaround's own `ExifToolProcess` included, was silently dropped as
/// an unrecognized line), printed an unnumbered `{ready}` unconditionally on
/// startup, and -- the deeper gap -- never applied any stdin option line at
/// all: `-json`, `-a`, `-TAG` all fell through untouched, so every request
/// was answered with whatever `Options` the process happened to start with
/// (i.e. unfiltered, every tag, every time) rather than that request's own.
///
/// Rebuilt here to parse each batch's own accumulated option lines back into
/// a fresh, request-scoped `Options` (via `apply_tag_request()`, the same
/// parser one-shot CLI invocation uses) before extracting -- matching what a
/// real `-stay_open` client (this crate's own `ExifToolProcess` included)
/// actually sends per request: a full tag list, `-json`, `-a`, then the file
/// path. Scoped to exactly that: this does not attempt real ExifTool's own
/// sticky-until-changed option semantics across `-execute` calls, since
/// nothing in this codebase relies on that -- every request resets to
/// `base_options` (whatever was set at process startup) and applies only
/// that batch's own lines on top.
fn run_stay_open(
    ...
    let mut batch_options = base_options.clone();
    let mut batch_json = default_json;
    ...
    for line in stdin.lock().lines() {
        ...
        if let Some(marker) = line.strip_prefix("-execute") {
            if !current_files.is_empty() {
                let et = ExifTool::with_options(batch_options.clone());
                // Real ExifTool's -stay_open JSON output is still wrapped in
                // `[...]` per request, exactly like its one-shot -j mode
                // (print_json_all() below does the same) -- this crate's
                // original run_stay_open() called print_json_tags() bare,
                // with no enclosing brackets, so a client doing the standard
                // `json_decode($response)[0]` (real ExifTool's own documented
                // stay-open JSON contract, and what Piwigo's ExifToolProcess
                // does) got a decode of a bare object with no `[0]` to index
                // -- silently null, not an error, on every single request.
                if batch_json {
                    print!("[");
                }
            ...
            batch_options = base_options.clone();
            batch_json = default_json;
        } else if line == "-json" || line == "-j" {
            batch_json = true;
        } else if line == "-a" || line == "-duplicates" {
            batch_options.duplicates = true;
        } else if line.starts_with('-') && line.len() > 1 {
            apply_tag_request(&line[1..], &mut batch_options);
