 ...
 * effect only after a full server restart). Activated via a system-level
 * `php.ini` kept outside this repo (see docs/REFERENCE.md). Eagerly
 * declares every Piwigo\-namespaced class/interface/trait/enum once at
 * PHP worker startup, into OPcache's shared, cross-request "preloaded"
 * table, so requests never pay per-request class-declaration cost for
 * them again.
 *
 * A real Xdebug profile of one admin.php bootstrap found Composer's
 * autoload include closure (a bare `include $file` -- genuine
 ...
 * request's server-side time. Measured impact: admin.php ~30-45% faster,
 * index.php ~40-50% faster, identification.php ~65-68% faster.
 *
 ...
 *
 * Requiring the real vendor/autoload.php unconditionally pulls in every
 * Composer package's `autoload.files` entries (see
 * vendor/composer/autoload_files.php), including nunomaduro/collision's
 * PHPUnit adapter bootstrap. That bootstrap declares an anonymous class
 * whose parent isn't itself preloaded ("Can't preload unlinked class
 * ...@anonymous"), and opcache aborts preloading for the ENTIRE process
 * the moment even one class fails to link this way -- not specific to
 * Piwigo\Tests\* classes; excluding those alone doesn't fix it. Fixed by
 * never loading the real Composer autoloader at all here: a minimal
 * classmap-only autoloader below resolves exactly the Piwigo\ classes
 * this script preloads and nothing else, so no other package's
 * files-autoload side effects ever run.
 *
 ...
 *
 * SEC-02 guard below deliberately isn't the plain `PHP_SAPI !== 'cli'`
 * every other tools/*.php script uses: opcache.preload executes this
 * script under the REAL target SAPI (`apache2handler` here), not `cli`
 * -- the plain guard would have exited before preloading a single class,
 * breaking the whole mechanism. Also, $_SERVER['REQUEST_METHOD'] is NOT
 * set during real preload execution even though PHP_SAPI already reports
 * 'apache2handler' at that point -- preload runs before Apache populates
 * per-request superglobals, which is what actually distinguishes
...
// bootstrap phase (relying on it throws "Undefined constant STDERR"
// here, even though the identical fwrite(STDERR, ...) call works fine
// anywhere else in the app) -- php://stderr as a stream path works
// regardless of that phase. Reached under both CLI dry-runs and the real
// Apache preload (the guard above only blocks a genuine HTTP request):
// under the real Apache restart this becomes preload's own confirmation
